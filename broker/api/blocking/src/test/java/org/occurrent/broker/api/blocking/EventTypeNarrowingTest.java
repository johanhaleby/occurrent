 ...
 * {@link EventTypeNarrowing#narrow(Filter)} directly, so this tests the shared walk once here rather than
 * duplicating the same filter-tree cases in each transport module.
 */
class EventTypeNarrowingTest {
    ...
    void an_in_condition_type_filter_resolves_to_every_value() {
        Filter filter = Filter.type(Condition.in(EVENT_A_TYPE, EVENT_B_TYPE));
    ...
    void an_or_of_two_type_filters_unions_the_types() {
        Filter filter = Filter.type(EVENT_A_TYPE).or(Filter.type(EVENT_B_TYPE));
    ...
    void an_or_with_one_unconstrained_branch_cannot_narrow() {
        Filter filter = Filter.type(EVENT_A_TYPE).or(Filter.streamId("some-stream"));
    ...
    void an_and_narrows_to_the_type_conjunct_even_though_the_other_conjunct_is_not_a_type_filter() {
        Filter filter = Filter.type(EVENT_A_TYPE).and(Filter.streamId("some-stream"));
    ...
    void an_and_of_two_type_filters_intersects_them() {
        Filter filter = Filter.type(Condition.in(EVENT_A_TYPE, EVENT_B_TYPE)).and(Filter.type(EVENT_A_TYPE));
    ...
    void an_in_condition_with_no_values_cannot_narrow() {
        Filter filter = Filter.type(Condition.in(List.of()));
    ...
    void an_and_of_two_type_filters_with_no_type_in_common_cannot_narrow() {
        Filter filter = Filter.type(Condition.in(EVENT_A_TYPE, EVENT_B_TYPE)).and(Filter.type(EVENT_C_TYPE));
    ...
    void an_or_of_two_disjoint_ands_cannot_narrow() {
        Filter filter = Filter.type(EVENT_A_TYPE).and(Filter.type(EVENT_B_TYPE))
    ...
    @Test
    void a_range_condition_on_the_type_cannot_narrow() {
        Filter filter = Filter.type(Condition.gt(EVENT_A_TYPE));
    ...
    @Test
    void a_negated_equality_condition_on_the_type_cannot_narrow() {
        Filter filter = Filter.type(Condition.ne(EVENT_A_TYPE));
    ...
    @Test
    void a_not_wrapped_around_an_equality_condition_on_the_type_cannot_narrow() {
        Filter filter = Filter.type(Condition.not(Condition.eq(EVENT_A_TYPE)));
    ...
    void a_filter_on_an_unrelated_field_cannot_narrow() {
        Filter filter = Filter.subject("some-subject");
    ...
    /**
     * {@link EventTypeNarrowing#narrow(Filter)} is only ever called from a resolver's
     * {@code destinationsFor(Filter)}, never from {@code destinationFor(CloudEvent)}, so the warning
