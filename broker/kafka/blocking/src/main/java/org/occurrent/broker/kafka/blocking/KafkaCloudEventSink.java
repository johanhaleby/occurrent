...
public final class KafkaCloudEventSink implements CloudEventSink, AutoCloseable {
    ...
    private void publishOnce(CloudEvent cloudEvent) {
        ...
        if (destination.topicIsPattern()) {
            // A resolver's own bug, not a transient publish failure, so this is refused here rather than handed
            // to the retry strategy, which would only repeat the same mistake against the same event. The shipped
