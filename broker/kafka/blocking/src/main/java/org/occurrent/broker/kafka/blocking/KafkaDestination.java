 ...
 * per-message components. {@link KafkaTopicPerTypeDestinationResolver#catchAllDestination()} is the one place
 * {@code topic} is not a literal topic name but a Kafka topic-matching pattern instead, meant for
 * {@code KafkaConsumer.subscribe(java.util.regex.Pattern)} rather than for publishing, exactly as
 ...
 *                ({@code io.cloudevents:cloudevents-kafka}) reserves for the CloudEvent attributes themselves,
 *                since a colliding key would silently overwrite one of them, {@code streamid} included, without
 *                anything failing.
 */
public record KafkaDestination(String topic, @Nullable String key, Map<String, String> headers) implements EventDestination {
    ...
    /**
     * Create a destination with no message key and no application headers.
     */
    public static KafkaDestination of(String topic) {
        return new KafkaDestination(topic, null, Map.of());
    }
    ...
    public static KafkaDestination of(String topic, @Nullable String key) {
        return new KafkaDestination(topic, key, Map.of());
    }
    ...
    public KafkaDestination withHeaders(Map<String, String> headers) {
        return new KafkaDestination(topic, key, headers);
    }
}
