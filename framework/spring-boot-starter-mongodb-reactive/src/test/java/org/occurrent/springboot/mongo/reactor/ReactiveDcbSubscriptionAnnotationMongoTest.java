...
/**
 * Proves that a {@link DcbSubscription} method is wired reactively in DCB-only mode: it replays the DCB history appended
 * before startup via {@code ReactorDcbCatchupSubscriptionModel}, then receives live events, and its {@code eventTypes}
 * narrowing keeps non-matching events out. The reactive counterpart of the blocking {@code DcbSubscriptionAnnotationMongoTest}.
 * The history is appended by a bean the subscriber {@link DependsOn}, so it exists before the annotation subscription starts.
 */
@DisplayName("Reactive DcbSubscription annotation (DCB-only mode)")
@DisplayNameGeneration(ReplaceUnderscores.class)
@SpringBootTest(
        classes = ReactiveDcbSubscriptionAnnotationMongoTest.DcbOnlyApplication.class,
        properties = {
                "occurrent.event-store.capabilities=dcb",
                "occurrent.cloud-event-converter.cloud-event-source=urn:occurrent:reactive-dcb-annotation-test"
        }
)
@Import(ReactiveDcbSubscriptionAnnotationMongoTest.MongoDbContainerConfiguration.class)
@Testcontainers
@Timeout(60)
class ReactiveDcbSubscriptionAnnotationMongoTest {

    static final String TAG = "test:reactive-annotation";
    private static final URI SOURCE = URI.create("urn:occurrent:reactive-dcb-annotation-test");
    ...
    void replays_history_then_receives_live_events_and_excludes_non_matching_types() {
        // The two Included events were appended before startup by HistoryAppender, so the BEGINNING subscription replays them.
    ...
    static class RecordingDashboard {
        ...
        @DcbSubscription(id = "reactive-dcb-annotation-dashboard", eventTypes = Included.class, startAt = DcbStartPosition.BEGINNING)
