...
/**
 * The reactive counterpart of {@code StreamSubscriptionAnnotationPrivateHandlerCglibProxyMongoTest}: proves that a
 * {@link StreamSubscription} handler still receives events when it is a private method and the
 * {@code ApplicationContext} hands the registrar a CGLIB subclass proxy at delivery time.
 */
@DisplayName("Reactive StreamSubscription private handler on a CGLIB proxy")
@DisplayNameGeneration(ReplaceUnderscores.class)
@SpringBootTest(
        classes = ReactiveStreamSubscriptionAnnotationPrivateHandlerCglibProxyMongoTest.AnnotationApplication.class,
        properties = {
                "occurrent.event-store.capabilities=stream",
                "occurrent.cloud-event-converter.cloud-event-source=urn:occurrent:reactive-stream-subscription-private-handler-cglib-proxy-test"
        }
)
@Import(ReactiveStreamSubscriptionAnnotationPrivateHandlerCglibProxyMongoTest.MongoDbContainerConfiguration.class)
@Testcontainers
@Timeout(60)
class ReactiveStreamSubscriptionAnnotationPrivateHandlerCglibProxyMongoTest {

    private static final URI SOURCE = URI.create("urn:occurrent:reactive-stream-subscription-private-handler-cglib-proxy-test");
    ...
    static class PrivateHandlerSubscriber {
        ...
        @StreamSubscription(id = "reactive-private-handler-cglib-proxy-stream-subscriber")
        private Mono<Void> on(TestEvent event) {
            received.add(event);
            return Mono.empty();
        }
