...
@SpringBootTest(
        classes = ProjectionAnnotationCrudStoreMongoTest.CrudStoreApplication.class,
        properties = {
                "occurrent.event-store.capabilities=dcb",
                "occurrent.cloud-event-converter.cloud-event-source=urn:occurrent:projection-crud-store-test"
        }
)
@Import(ProjectionAnnotationCrudStoreMongoTest.MongoDbContainerConfiguration.class)
@Testcontainers
@Timeout(60)
class ProjectionAnnotationCrudStoreMongoTest {

    static final String TAG = "counter:crud";
    private static final URI SOURCE = URI.create("urn:occurrent:projection-crud-store-test");
    ...
    @Autowired
    private OrderCountCrudRepository orderCountRepository;

    @Test
    void materializes_into_a_crud_repository_backed_store() {
        await().atMost(ofSeconds(30)).pollInterval(ofMillis(100)).untilAsserted(() ->
                assertThat(orderCountRepository.findById("orders")).get().extracting(OrderCount::count).isEqualTo(2));
    }
    ...
    @EnableOccurrent
    static class CrudStoreApplication {
        @Bean
        ...
        @Bean
        OrderCountCrudRepository orderCountRepository() {
            return new OrderCountCrudRepository();
        }
    ...
    // A minimal in-memory CrudRepository, enough to prove the CrudRepository store-resolution branch (only findById and
    // save are exercised by the projection's load-evolve-save).
    static class OrderCountCrudRepository implements CrudRepository<OrderCount, String> {
        private final ConcurrentHashMap<String, OrderCount> store = new ConcurrentHashMap<>();

        @Override
        public <S extends OrderCount> S save(S entity) {
        ...
        @Override
        public <S extends OrderCount> Iterable<S> saveAll(Iterable<S> entities) {
        ...
        @Override
        public Iterable<OrderCount> findAll() {
            throw new UnsupportedOperationException();
        }

        @Override
        public Iterable<OrderCount> findAllById(Iterable<String> ids) {
            throw new UnsupportedOperationException();
        }
        ...
        @Override
        public void delete(OrderCount entity) {
            throw new UnsupportedOperationException();
        }
        ...
        @Override
        public void deleteAll(Iterable<? extends OrderCount> entities) {
            throw new UnsupportedOperationException();
        }
    ...
    record OrderCount(String id, int count) {
    }
