...
/**
 * Pins {@code occurrent.subscription.mode=manual} across a restart, which is the case where a subscription no longer
 * goes straight to live delivery. A first boot consumes an event and stores a checkpoint, so the second boot resumes
 * from it and takes the catch-up path instead. Catch-up replays out of the event store rather than the change stream,
 * so it is the one route that could deliver events to a subscription nobody has started.
 * <p>
 ...
 */
@DisplayName("Subscription mode manual (across restart)")
@DisplayNameGeneration(ReplaceUnderscores.class)
@Testcontainers
@Timeout(180)
class SubscriptionModeManualRestartMongoTest {

    private static final String SUBSCRIPTION_ID = "manualModeRestartSubscriber";
    private static final URI SOURCE = URI.create("urn:occurrent:subscription-mode-manual-restart");
    ...
    void a_stored_checkpoint_does_not_let_a_manual_subscription_replay_before_it_is_resumed() {
        ...
        try {
            ...
            subscriptionModel.resumeSubscription(SUBSCRIPTION_ID).waitUntilStarted(ofSeconds(30));
    ...
    private static void runFirstBoot(TestEvent beforeRestart) {
        ...
        try {
            ApplicationService<TestEvent> applicationService = applicationServiceOf(context);
            applicationService.execute(UUID.randomUUID().toString(), __ -> List.of(beforeRestart));
    ...
    private static String[] bootArgs(String mode) {
        return new String[]{
                "--spring.data.mongodb.uri=" + mongoDBContainer.getReplicaSetUrl("subscription-mode-manual-restart"),
    ...
    static class OfflineAppender {
        static final String APPENDED_WHILE_DOWN = "appended-while-down";
        ...
        void appendWhileDown() {
            applicationService.execute(UUID.randomUUID().toString(),
                    __ -> List.of(new TestEvent(APPENDED_WHILE_DOWN, new Date(), "name")));
        }
    }
