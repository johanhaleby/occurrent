...
@SpringBootTest(
        ...
        properties = {
                ...
                "occurrent.cloud-event-converter.cloud-event-source=urn:occurrent:push-projection-annotation-test"
        }
)
@Import(ProjectionAnnotationPushSourceMongoTest.MongoDbContainerConfiguration.class)
@Testcontainers
@Timeout(60)
class ProjectionAnnotationPushSourceMongoTest {

    private static final URI SOURCE = URI.create("urn:occurrent:push-projection-annotation-test");
    ...
    void catches_up_history_from_the_event_store_then_materializes_pushed_live_events() {
        ...
        // A live event arrives through the push model (as a broker listener would deliver it) and is materialized.
    ...
    static class PushProjectionApplication {
        ...
        // The external push feed a broker listener would drive. The @Projection binds to it via source = PUSH.
        @Bean
    ...
    static class OrderCountProjection {
        @Projection(id = "push-order-count", source = Source.PUSH, subscriptionModelName = "pushModel", storeName = "orderCountStore")
