...
@SpringBootTest(
        classes = SubscriptionModeManualDomainPushProjectionMongoTest.ManualDomainPushProjectionApplication.class,
        properties = {
                "occurrent.event-store.capabilities=stream",
                "occurrent.subscription.mode=manual",
                "occurrent.cloud-event-converter.cloud-event-source=urn:occurrent:manual-mode-domain-push-projection-test"
        }
)
@Import(SubscriptionModeManualDomainPushProjectionMongoTest.MongoDbContainerConfiguration.class)
@Testcontainers
@Timeout(60)
class SubscriptionModeManualDomainPushProjectionMongoTest {

    private static final URI SOURCE = URI.create("urn:occurrent:manual-mode-domain-push-projection-test");
    private static final String PROJECTION_ID = "manual-domain-push-order-count";
    ...
    void a_domain_push_source_projection_is_not_registered_until_started_then_catches_up_and_goes_live() {
        ...
        // The two historic events written before startup are only visible now, through the catch-up this start ran.
        ...
        // Starting it again is a no-op, not a second register-and-catch-up.
        manualStartProjections.start(PROJECTION_ID);
    ...
    @EnableOccurrent
    static class ManualDomainPushProjectionApplication {
        @Bean
    ...
    static class OrderCountProjection {
        @Projection(id = PROJECTION_ID, source = Source.PUSH, subscriptionModelName = "ordersFeed", storeName = "orderCountStore")
