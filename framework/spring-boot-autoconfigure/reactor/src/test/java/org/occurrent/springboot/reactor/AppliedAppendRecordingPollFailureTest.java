...
/**
 * The reactor twin of the blocking {@code AppliedAppendRecordingPollFailureTest}. Each tick of the applied-append
 * recording poll is the only thing that schedules the one after it, so a store that throws has to leave the poll
 * running or the projection stops recording for the rest of the application's life.
 */
...
class AppliedAppendRecordingPollFailureTest {
    ...
    private static void assertThatTheClearIsRetried(Exception clearFailure) {
        ...
        when(((ReplayAwareSubscriptions) model).listenForCatchup(eq(PROJECTION_ID), any(CatchupListener.class))).thenAnswer(invocation -> {
        ...
        when(model.subscribe(anyString(), any(), any(StartAt.class), any())).thenReturn(subscription);
        ...
        new ApplicationContextRunner()
                ...
                .withBean("recordingProjection", RecordingProjection.class, RecordingProjection::new)
