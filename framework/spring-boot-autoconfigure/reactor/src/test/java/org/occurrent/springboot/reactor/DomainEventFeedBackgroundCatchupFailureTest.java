 ...
 * background path is separate code in {@code ProjectionAnnotationRegistrar} ({@code catchUpCollectedFeeds()}), so a
 * regression there would otherwise be uncovered.
 * <p>
 * Container-free: a fake reader whose replay errors, a mocked {@code Subscribable} (required only so annotation
 * processing does not skip early, since a {@code DomainEventFeed} is not itself a {@code Subscribable}), and the
 * real {@code DomainEventFeed}.
 */
...
class DomainEventFeedBackgroundCatchupFailureTest {
    ...
    void a_background_domain_feed_catch_up_that_fails_does_not_fail_the_context_and_the_failure_lands_in_background_catchup_failures() {
        new ApplicationContextRunner()
                ...
                .run(context -> {
                    ...
                    while (!(status.of("domain-feed-push-background-failing-reactive") instanceof PushCatchupStatus.Failed) && System.nanoTime() < deadline) {
                        Thread.sleep(10);
                    }
                    assertThat(status.of("domain-feed-push-background-failing-reactive")).isInstanceOfSatisfying(PushCatchupStatus.Failed.class, failed ->
                    ...
                    assertThat(status.isCaughtUp("domain-feed-push-background-failing-reactive")).isFalse();
                });
    }
    ...
    static class FailingBackgroundProjection {
        @Projection(id = "domain-feed-push-background-failing-reactive", source = Source.PUSH, startupMode = StartupMode.BACKGROUND)
