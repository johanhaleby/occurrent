...
/**
 * The reactor twin of the blocking {@code DescriptorFactorySwitchingPrototypeTest}, see that class for the
 * mechanism. Only the reactive collaborator types and the mocked {@link Subscribable}, which the reactive bean post
 * processor needs present before it scans for annotations at all, differ from the blocking version.
 */
...
class DescriptorFactorySwitchingPrototypeTest {
    ...
    static class CheckedProjectionFactory implements ProjectionFactory {
        @Projection(id = "checked-projection-reactive", source = Source.PUSH)
    ...
    static class UncheckedProjectionFactory implements ProjectionFactory {
        @Projection(id = "unchecked-projection-reactive", source = Source.PUSH)
        org.occurrent.dsl.projection.Projection<Integer, TestEvent, String> projection() {
