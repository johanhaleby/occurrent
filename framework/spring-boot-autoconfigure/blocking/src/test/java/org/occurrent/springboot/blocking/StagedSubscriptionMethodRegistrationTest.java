 ...
 * refused. Reproduced without a running store (no Docker).
 */
...
class StagedSubscriptionMethodRegistrationTest {
    ...
    void a_prototype_registers_the_handler_the_scan_read_and_runs_it_on_a_subclass_that_inherits_it() {
        runner.withUserConfiguration(InheritingPrototypeConfiguration.class).run(context -> {
            ...
            ArgumentCaptor<Function2<EventMetadata, TestEvent, Unit>> handler = ArgumentCaptor.forClass(Function2.class);
            verify(subscriptions).subscribe(eq("checked-handler"), any(AgnosticSubscriptionFilter.class), any(), anyBoolean(), handler.capture());
