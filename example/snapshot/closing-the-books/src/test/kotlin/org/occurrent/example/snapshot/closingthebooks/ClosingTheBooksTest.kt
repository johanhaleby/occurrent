...
class ClosingTheBooksTest {
    ...
    private lateinit var snapshots: SnapshotDeciderApplicationService<LedgerEvent>
    ...
    fun setUp() {
        ...
        val converter: CloudEventConverter<LedgerEvent> =
            JacksonCloudEventConverter.Builder<LedgerEvent>(tools.jackson.module.kotlin.jacksonObjectMapper(), URI.create("urn:occurrent:example:snapshot"))
                ...
                .build()
        val applicationService: ApplicationService<LedgerEvent> = GenericApplicationService(eventStore, converter)
        snapshots = SnapshotDeciderApplicationService(applicationService)
    ...
    fun `technical snapshot is taken every N events and a later command resumes from it`() {
        ...
        repeat(250) { snapshots.execute("account-1", Deposit(1), decider, store, options) }
        ...
        val state = snapshots.executeAndReturnState("account-1", Deposit(5), decider, store, options)
    ...
    fun `closing the books snapshots the terminal state and carries the balance into the next period`() {
        ...
        snapshots.execute("period-2026-Q1", Deposit(100), decider, store, options)
        snapshots.execute("period-2026-Q1", Withdraw(30), decider, store, options)
        snapshots.execute("period-2026-Q1", CloseBooks("2026-Q1"), decider, store, options)
        ...
        val openingState = snapshots.executeAndReturnState("period-2026-Q2", SetOpeningBalance(closingBalance), decider, store, options)
        ...
        val afterDeposit = snapshots.executeAndReturnState("period-2026-Q2", Deposit(10), decider, store, options)
