...
/** Defines a course with a seat capacity. Single-entity boundary (the course). */
@Service
class DefineCourse(
    ...
    operator fun invoke(courseId: CourseId, title: String, capacity: Int, occurredAt: Instant = Instant.now()) {
        applicationService.execute(
            CourseEnrollmentDcbQueries.courseDecisionContext(courseId),
            CourseEnrollmentCommand.DefineCourse(UUID.randomUUID(), occurredAt, courseId, title, capacity),
            decider
        )
    }
}
