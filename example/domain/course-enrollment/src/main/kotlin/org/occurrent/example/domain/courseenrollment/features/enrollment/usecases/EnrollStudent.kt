...
/**
 * Enrolls a student in a course. The decision boundary spans the course AND the student (see
 * [CourseEnrollmentDcbQueries.enrollmentDecisionContext]), so the capacity invariant and the per-student limit are both
 * enforced by one conditional append. A concurrent enrollment that would break either invariant loses the optimistic
 * conflict and is retried by [Retryable].
 *
 * This use case is wired for you. Make it work by implementing the query and the decider it calls.
 */
@Service
class EnrollStudent(
    ...
    operator fun invoke(courseId: CourseId, studentId: StudentId, occurredAt: Instant = Instant.now()) {
        applicationService.execute(
            CourseEnrollmentDcbQueries.enrollmentDecisionContext(courseId, studentId),
            CourseEnrollmentCommand.EnrollStudent(UUID.randomUUID(), occurredAt, courseId, studentId),
            decider
        )
    }
}
