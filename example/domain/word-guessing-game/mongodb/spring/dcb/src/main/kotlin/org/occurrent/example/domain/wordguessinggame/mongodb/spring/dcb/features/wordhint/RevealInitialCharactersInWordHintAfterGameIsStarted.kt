...
class RevealInitialCharactersInWordHintAfterGameIsStarted(
    ...
    @Bean
    fun whenGameWasStartedThenRevealInitialCharactersInWordHint() =
        subscriptions.subscribe<GameWasStarted>("WhenGameWasStartedThenRevealInitialCharactersInWordHintPolicy") { gameWasStarted ->
    ...
    operator fun invoke(gameWasStarted: GameWasStarted) {
        applicationService.execute(GameDcbQueries.wordHintCriteria(gameWasStarted.gameId)) { events ->
