        ...
        for (int i = 0; i < ITERATIONS; i++) {
            ...
            List<Outcome<String>> outcomes = ConcurrentRendezvous.collide(threadCount, index -> {
                ...
                return () -> {
                    ...
                    // concurrent writers does not fail a write within the store's retry limit (15 attempts), and
                    // threadCount writers racing on one stream is well inside that, so any failure observed here is
                    // a real defect in the store rather than an expected loser.
