 ...
 * can be told apart from a scan incapable of finding anything. {@link ReactiveSuiteNeverSkipsTest} points the scan at
 * this class and requires it to be reported.
 * <p>
 * It is compiled into {@code target/test-classes} rather than {@code target/classes}, a different code source from the
 * suite, so it can never leak into the scan that matters. It is not named {@code *Test} either, so Surefire does not
 * run it.
 */
