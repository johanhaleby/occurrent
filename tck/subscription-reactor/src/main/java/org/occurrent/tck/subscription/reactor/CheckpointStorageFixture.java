...
public interface CheckpointStorageFixture {
     ...
     * comparison. Answered by the storage itself, through {@link CheckpointStorage#evaluatesWriteConditions()}, so the
     * capability a storage advertises is the one the suite holds it to. A storage that advertises {@code true} and then
     * refuses a condition fails the acceptance tests, and one that advertises {@code false} while evaluating conditions
     * fails the refusal tests. Override only to test a storage against the opposite of what it claims.
     */
