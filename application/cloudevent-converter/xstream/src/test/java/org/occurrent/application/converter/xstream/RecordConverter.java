...
/**
 *
 * Copied from https://github.com/oracle/xstream/blob/5a20ac6ca268d51fc55a6a4c634f2d04dbe2ed2f/xstream/src/java/com/thoughtworks/xstream/converters/extended/RecordConverter.java.
 * Can be removed when upgrading to xstream 1.5
 *
 * Converts a Record.
 *
...
public final class RecordConverter implements Converter {
    ...
    public void marshal(Object source,
        ...
        Arrays.stream(recordComponents(cls))
              .forEach(rc -> writeItem(rc, componentValue(source, rc), context, writer));
    }

    private void writeItem(RecordComponent recordComponent,
                           Object compValue,
                           MarshallingContext context,
                           HierarchicalStreamWriter writer) {
        if (compValue == null) {
            ...
            writer.startNode(recordComponent.name());
    ...
    /**
     * Unmarshals a record. Reconstitutes all stream values first (if any),
     * then matches the stream value to the corresponding component parameter
     * position of the record's canonical constructor, lastly, invokes the
    ...
    public Object unmarshal(HierarchicalStreamReader reader,
        ...
        while (reader.hasMoreChildren()) {
            ...
            if (reader.getAttribute("class") != null) {
                ...
                c = Arrays.stream(recordComponents)
                          .filter(rc -> rc.name().equals(name))
                          .map(RecordComponent::type)
                          .findFirst()
                          .orElseThrow(() -> new ConversionException("Type not found for " + name));
            }
    ...
    /**
     * Returns a array containing values ordered by that of the record components.
     * Individual values are retrieved (matched by name) from the given valueMap.
     * This order follows that of the record components, which also matches the
     * order of parameters of the canonical constructor. Where no matching value
     * is found, the default value of the component type is inserted.
     */
    private static Object[] orderValues(HashMap<String,Object> valueMap,
                                        RecordComponent[] recordComponents) {
        return Arrays.stream(recordComponents)
                     .map(rc -> valueMap.getOrDefault(rc.name(), defaultValueFor(rc.type())))
                     .toArray(Object[]::new);
    }
     ...
     */
    private static Object invokeCanonicalConstructor(Class<?> recordClass,
                                                     RecordComponent[] recordComponents,
                                                     Object[] args) {
        try {
            ...
            MethodHandle MH_canonicalConstructor =
                    LOOKUP.findConstructor(recordClass, methodType(void.class, paramTypes))
