 ...
 * default, which reads the stored checkpoint). The decision is made when the subscription starts by checking
 * {@link CheckpointStorage} for an existing checkpoint, so it reflects whether this subscription has run before.
 * <p>
 * This is the Spring-free building block behind the framework's {@code resumeBehavior = DEFAULT} handling. It lets a
 * plain (non-Spring) caller that wires its own catch-up-capable subscription model get the same catch-up-then-resume
 * behavior, and the annotation processors reuse the same rule.
 */
public final class ResumeStartPositions {
     ...
     * @param checkpointStorage the storage consulted, when the subscription starts, for an existing checkpoint
    ...
    public static StartAt replayThenResume(String subscriptionId, CheckpointStorage checkpointStorage, StartAt replayStart) {
        ...
        return StartAt.dynamic(() -> checkpointStorage.exists(subscriptionId) ? StartAt.subscriptionModelDefault() : replayStart);
    }
    ...
    public static DcbStartAt replayThenResumeDcb(String subscriptionId, CheckpointStorage checkpointStorage, DcbStartAt replayStart) {
        ...
        requireNonNull(checkpointStorage, "checkpointStorage cannot be null");
        requireNonNull(replayStart, "replayStart cannot be null");
        return DcbStartAt.dynamic(ctx -> checkpointStorage.exists(subscriptionId) ? DcbStartAt.subscriptionModelDefault() : replayStart);
    }
}
