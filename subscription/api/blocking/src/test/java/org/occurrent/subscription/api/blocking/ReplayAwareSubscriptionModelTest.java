...
/**
 * Covers {@link ReplayAwareSubscriptionModel#of(Object)}, the lookup a caller uses instead of casting to a concrete
 * catch-up model, since the model that knows about the replay is usually behind a {@link DelegatingSubscriptionModel}
 * such as {@code DurableSubscriptionModel(CatchupSubscriptionModel(..))}.
 */
@DisplayNameGeneration(ReplaceUnderscores.class)
class ReplayAwareSubscriptionModelTest {

    @Test
    void finds_the_model_itself_when_it_is_replay_aware() {
        ReplayAwareModel model = new ReplayAwareModel(Set.of("orders"));

        Optional<ReplayAwareSubscriptionModel> found = ReplayAwareSubscriptionModel.of(model);
    ...
    void unwraps_a_delegating_model_to_reach_the_replay_aware_one() {
        ...
        Optional<ReplayAwareSubscriptionModel> found = ReplayAwareSubscriptionModel.of(new Wrapper(inner));
    ...
    void unwraps_through_several_layers_of_wrapping() {
        ...
        Optional<ReplayAwareSubscriptionModel> found = ReplayAwareSubscriptionModel.of(new Wrapper(new Wrapper(inner)));
    ...
    void is_empty_when_nothing_in_the_chain_replays() {
        Optional<ReplayAwareSubscriptionModel> found = ReplayAwareSubscriptionModel.of(new Wrapper(new PlainModel()));
    ...
    private static final class ReplayAwareModel extends PlainModel implements ReplayAwareSubscriptionModel {
