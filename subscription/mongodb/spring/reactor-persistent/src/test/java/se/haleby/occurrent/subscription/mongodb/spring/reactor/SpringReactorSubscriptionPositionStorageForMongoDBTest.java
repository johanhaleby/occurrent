...
public class SpringReactorSubscriptionPositionStorageForMongoDBTest {
    ...
    void reactive_persistent_spring_subscription_allows_cancelling_subscription() throws InterruptedException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        CopyOnWriteArrayList<CloudEvent> state = new CopyOnWriteArrayList<>();
        String subscriberId = UUID.randomUUID().toString();
        ...
        subscription.cancelSubscription(subscriberId).block();
