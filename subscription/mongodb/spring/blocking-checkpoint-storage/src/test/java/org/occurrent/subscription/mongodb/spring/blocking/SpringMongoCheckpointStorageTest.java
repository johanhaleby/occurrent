...
@Testcontainers
public class SpringMongoCheckpointStorageTest {
    ...
    void create_mongo_event_store() {
        ...
        SpringMongoCheckpointStorage storage = new SpringMongoCheckpointStorage(mongoTemplate, RESUME_TOKEN_COLLECTION);
    ...
    class LegacyCheckpointFieldMigration {
        ...
        void reads_legacy_subscription_position_field_as_string_based_checkpoint() {
            ...
            mongoTemplate.getCollection(RESUME_TOKEN_COLLECTION).insertOne(
        ...
        void save_removes_legacy_subscription_position_field_and_writes_new_checkpoint_field() {
            ...
            // Then
            Document rawDocument = mongoTemplate.getCollection(RESUME_TOKEN_COLLECTION).find(Filters.eq(ID_FIELD, subscriptionId)).first();
            assertAll(
    ...
    void blocking_spring_subscription_stores_every_event_by_default() {
        ...
        CheckpointStorage storage = new CheckpointStorage() {
            ...
            @Override
            public Checkpoint read(@NonNull String subscriptionId) {
                return null;
            }
        ...
        await().atMost(4, SECONDS).with().pollInterval(Duration.of(20, MILLIS)).untilAsserted(() -> {
            ...
            assertThat(numberOfWritesToBlockingSubscriptionStorage).hasValue(4); // 3 events and one for global checkpoint
        });
    }
    ...
    void blocking_spring_subscription_stores_every_n_events_was_defined_config() {
        ...
        await().atMost(4, SECONDS).with().pollInterval(Duration.of(20, MILLIS)).untilAsserted(() -> {
            assertThat(state).hasSize(3);
            assertThat(numberOfWritesToBlockingSubscriptionStorage).hasValue(2); // 1 event and one for global checkpoint
        });
    }
