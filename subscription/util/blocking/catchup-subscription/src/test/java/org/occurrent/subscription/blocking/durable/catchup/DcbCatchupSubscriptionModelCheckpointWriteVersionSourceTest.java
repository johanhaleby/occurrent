 ...
 * {@link DcbCatchupSubscriptionModel} writes with {@code notOlderThan(version)}. No source, or a source answering
 * empty, leaves every write {@code any()}, exactly the behaviour before ADR 116. Also proves the same threading
 * works through the public {@link CatchupSubscriptionModel} dispatcher, which is what the Spring starter actually
 * constructs (it never builds this package-private class directly). Run over {@link InMemoryCheckpointStorage},
 * which evaluates a {@code CheckpointWriteCondition} for real rather than refusing it, so the stored version proves
 * which condition the model actually stamped the write with.
 */
@DisplayNameGeneration(ReplaceUnderscores.class)
class DcbCatchupSubscriptionModelCheckpointWriteVersionSourceTest {
    ...
    void a_configured_source_stamps_the_periodic_catchup_phase_write_not_older_than_the_version_it_answers() {
        ...
        CatchupSubscriptionModelConfig config = new CatchupSubscriptionModelConfig(
                useCheckpointStorage(storage, id -> OptionalLong.of(11)).andPersistCheckpointDuringCatchupPhaseForEveryNEvents(1));
        ...
        subscription.subscribe(SUBSCRIPTION_ID, StartAt.checkpoint(GlobalCheckpoint.of(0)), event -> {
        }).waitUntilStarted();

        await().untilAsserted(() -> assertThat(storage.writeVersion(SUBSCRIPTION_ID)).hasValue(11L));
    }

    @Test
    void a_configured_source_also_stamps_the_catchup_to_live_handover_write_through_the_public_catchup_subscription_model_dispatcher() {
        appendTagged("name:1", nameDefined("position1"));
        CheckpointStorage storage = new InMemoryCheckpointStorage();
        ...
        // Built through CatchupSubscriptionModel, the dispatcher the Spring starter actually constructs, to prove
        // the source is threadable from its public constructors through to this inner model's checkpoint writes.
        CatchupSubscriptionModelConfig config = new CatchupSubscriptionModelConfig(useCheckpointStorage(storage, id -> OptionalLong.of(4)));
        CatchupSubscriptionModel subscription = new CatchupSubscriptionModel(subscriptionModel, eventStore, DcbCriteria.tags(Tag.parse("name:1")), config);

        subscription.subscribe(SUBSCRIPTION_ID, StartAt.checkpoint(GlobalCheckpoint.of(0)), event -> {
        }).waitUntilStarted();

        await().untilAsserted(() -> assertThat(storage.writeVersion(SUBSCRIPTION_ID)).hasValue(4L));
    }
     ...
     * in-memory model only supports {@code now}/{@code default}, and a stub global position is enough for the
     * catch-up to take its normal handover path.
     */
