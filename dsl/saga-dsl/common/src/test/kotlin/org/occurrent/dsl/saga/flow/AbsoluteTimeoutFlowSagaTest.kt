...
class AbsoluteTimeoutFlowSagaTest {
    ...
    private val auctionSaga: Saga<AuctionEvent, FlowState<AuctionEvent>, AuctionCommand> = saga {
        ...
        step("bidding") {
            ...
            timeout(at = { received -> received.initiating(AuctionStarted::class.java).endsAt }, then = end) { received ->
                issue(CloseAuction(received.initiating(AuctionStarted::class.java).auctionId))
            }
        }
    }
    ...
    fun firesAbsoluteTimeout() {
        ...
        val step = auctionSaga.step(afterStart, SagaInput.timeout(SagaTimeout("a1", "step:bidding")))
