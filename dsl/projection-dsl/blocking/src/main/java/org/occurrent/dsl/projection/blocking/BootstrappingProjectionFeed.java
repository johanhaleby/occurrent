 ...
 * guarantee holds for the live phase. During the bootstrap window {@link #accept(Object)} buffers the event and returns
 * before it is folded, so a message may be acknowledged before it is applied. That is safe because the marker is written
 * only after the drain, so a crash mid-bootstrap re-replays the whole history from the store, the backstop for any event
