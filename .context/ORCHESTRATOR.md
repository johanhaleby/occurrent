...
- **Reactive dual-mode (combined STREAM+DCB) catch-up: NOT implemented (deferred, from unified-position ADR 45).** Blocking has one `CatchupSubscriptionModel` routing per-subscription between stream-filter and dcb-query reads; reactive has two separate models (`ReactorDcbCatchupSubscriptionModel` + `ReactorStreamCatchupSubscriptionModel`), each accepting only its own filter type, so they cannot be linearly chained. Result: on a combined STREAM+DCB reactive store, DCB history replay works but STREAM history replay is fail-loud. No design blocker, pure scope: fix is a reactive dual-mode model or a per-subscription router dispatching by filter type (`DcbSubscriptionFilter` to DCB reader, else stream reader), mirroring the blocking model. Single-mode reactive stores are unaffected.
...
- Reactive dual-mode catch-up (above) and reactive competing-consumer remain the main open reactive gaps.
