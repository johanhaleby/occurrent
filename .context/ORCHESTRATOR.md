...
- Reactive STREAM catch-up model: now EXISTS as `ReactorStreamCatchupSubscriptionModel` (built by the unified-position work, ADR 45). On a reactive STREAM-only store with position on, `@StreamSubscription` history replay works; the fail-loud path now fires only when position is off. This supersedes the old "does not exist" note (ADR 44).
- **Reactive dual-mode (combined STREAM+DCB) catch-up: NOT implemented (deferred future work).** The blocking side has a single `CatchupSubscriptionModel` that routes per-subscription between stream-filter and dcb-query reads inside one model. The reactive side has two separate models (`ReactorDcbCatchupSubscriptionModel` + `ReactorStreamCatchupSubscriptionModel`), each of which only accepts its own filter type, so they cannot be linearly chained. Result: on a combined STREAM+DCB reactive store, DCB history replay works but STREAM history replay is fail-loud. No design blocker, purely scope: the fix is a reactive dual-mode model or a per-subscription router that dispatches by filter type (`DcbSubscriptionFilter` to the DCB reader, else the stream reader), mirroring the blocking `CatchupSubscriptionModel`. Single-mode reactive stores (STREAM-only or DCB-only) are unaffected and fully work. Documented in ADR 45 and the changelog.
...
- Reactive stream catch-up / competing-consumer parity items above remain the main open reactive gaps.
