...
- Add Kotlin Sequence-returning extensions to `DcbDomainEventQueries` (for example `queryForSequence`), mirroring the DCB Kotlin extensions already on `DomainEventQueries` (`queryForSequence`, `queryForList`, `queryWithPosition` in `dsl/dcb-dsl/blocking`). Today `DcbDomainEventQueries.query(DcbQuery)` returns a `java.util.stream.Stream`, so Kotlin callers either consume the Java stream or bridge via `kotlin.streams.asSequence` (the example's `CourseDetail` does the latter). A Sequence-returning extension would give DCB query callers the same ergonomics as the stream side. Small DSL ergonomics addition. Requested by the user on 2026-06-26.
