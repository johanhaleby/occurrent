...
Reactive DCB support is complete and stacked as PRs #242 (store) → #243 (application service) → #244 (DSL) → #245 (subscriptions) → #246 (catch-up) → #247 (shared EventMetadata), all pushed and awaiting the user's `av-land`. This closes DCB across all four event stores. #247 shared the common DCB types so the blocking and reactive stacks stop duplicating them. New modules: `subscription-dsl-common` (holds `EventMetadata`, package `org.occurrent.dsl.subscription`), `dcb-dsl-common` (holds the shared `DcbEventMetadata`, the `DcbDomainEventStream` read result, and the `EventMetadata.dcbPosition`/`dcbTags` Kotlin accessors, package `org.occurrent.dsl.dcb`), and `application-service-common` (holds the shared `TagGenerator`, package `org.occurrent.application.service.dcb`). Left duplicated on purpose: `DcbExecuteOptions`, the `DcbApplicationService` interface, and the query/subscription DSLs, which differ by sync-versus-reactive effect type. Breaking: `org.occurrent.dsl.subscription.blocking.EventMetadata` moved to `org.occurrent.dsl.subscription.EventMetadata` (the only released type moved, a typealias shim was tried and dropped because Kotlin treats a cross-module typealias parameter type as a distinct type). `TagGenerator` and `DcbDomainEventStream` are unreleased DCB, so repackaging them is free. Per-PR `/code-review` found no behavior bugs, only test-coverage gaps and one real semantic issue in the catch-up empty-token path. Fixes applied: added regression tests on each PR (duplicate-event-not-retried #242, side-effect-once-across-retry + retry-exhaustion #243, conflict-retry decision #244, adapter floor/start pass-through #245, fail-loud empty-token + small-window/cache replay #246), and changed reactive DCB catch-up to FAIL LOUDLY when the model reports no resume token (was a best-effort fallback that could silently drop events between replay and live). The phased plan is `.context/reactive-dcb-plan.md`.
...
- `DcbSubscriptions` does no server-side filtering. `subscribe` always uses `OccurrentSubscriptionFilter.filter(Filter.all())` and post-filters every CloudEvent in-process by the `DcbQuery` (`DcbCloudEvents.matches`, position greater than 0). Stream `Subscriptions` instead accept a `Filter` that becomes a server-side MongoDB change-stream match (the `@Subscription` annotation pushes `Filter.type(...)` down). So a DCB read model that cares about only a few event types still receives every event and filters in the consumer (see `CourseDashboardSubscriber`, which subscribes with `DcbQuery.all()` and does an is-type check). Give DCB subscriptions server-side filtering the same way `Subscriptions` has it, either by accepting a `Filter` or by translating the `DcbQuery` type and tag constraints into a server-side change-stream match. Part of the DCB/stream subscription split. ADR 0019 recorded the in-process post-filter as the v1 simplification. Requested by the user on 2026-06-26.
- `DcbDomainEventQueries` already has Kotlin Sequence and List extensions (`queryForSequence`, `queryForList`, `queryForSequenceWithPosition`, `queryForListWithPosition` in `DcbDomainEventQueriesExtensions.kt`). For DCB these are leak-free without an explicit close: a DCB read materializes its matched window into a `List` (`DcbEventStream.events`), so `query(...)` returns a list-backed Stream whose close is a no-op. `CourseDetail` uses `queryForSequence` directly with no `.use`. This corrects an earlier note that wrongly recorded these extensions as missing. Open question worth checking separately: the stream-side `DomainEventQueries.queryForSequence` (query-dsl) also wraps `query(...).asSequence()`, but if the stream event store returns a live Mongo-cursor-backed Stream there, a partially consumed Sequence would not close the cursor, so the stream side may need a closing or loan-pattern variant.
