...
- Reactive STREAM catch-up model: does not exist, only `ReactorDcbCatchupSubscriptionModel` (DCB) does. `@StreamSubscription` on the reactive stack fails loud for any history-replay start; only `NOW`/`DEFAULT` (live plus durable resume) are supported. Documented as a known gap in ADR 44, not started.
