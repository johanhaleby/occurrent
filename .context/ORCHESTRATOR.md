...
- **Blocking stream catch-up is not DCB-free, plus the same latent routing bug (follow-up to the reactive decoupling above, PR2).** `subscription/util/blocking/catchup-subscription` hard-depends on `eventstore-api-dcb` and `CatchupSubscriptionModel` imports DCB types, so a STREAM-only blocking user (for example `entity-history`) drags the DCB API onto the classpath. `CatchupSubscriptionModel.routesToDcb` has the byte-identical routing bug, currently un-triggered only because the blocking annotation processor maps `@StreamSubscription(BEGINNING_OF_TIME)` to a `TimeBasedSubscriptionPosition` (not a `GlobalSubscriptionPosition`), so it dodges the position heuristic. An explicit `GlobalSubscriptionPosition` stream start on a dual-mode blocking store would hit it. PR2 splits the module into a DCB-free `stream-catchup-subscription` (shared pipeline + `StreamCatchupSubscriptionModel`) and a `dcb-catchup-subscription` (DCB model + the `CatchupSubscriptionModel` dispatcher, keeping the class name and constructors), and applies the same filter-first `routesToDcb` fix.
...
- Reactive competing-consumer remains the main open reactive gap. The blocking stream-catchup DCB decoupling (above, PR2) is the pending follow-up to the reactive decoupling work.
- **Java 21 baseline upgrade: when it happens, tighten the stream catch-up start routing (do not miss this).** `CatchupSubscriptionModel` (blocking, ADR 45 area) now classifies a resolved start into a `StreamStart` enum (`BEGINNING_OF_TIME`, `SPECIFIC_TIME`, `GLOBAL_POSITION`, `LIVE`) via `classifyStreamStart` and routes with exhaustive `switch` expressions, so no start silently falls through to live delivery (that silent fall-through is the bug that once dropped specific-time replay on a position store, caught only by a merged-in test). On Java 17 the enum switch expression already gives compile-time exhaustiveness, so correctness is covered today. When the project moves to a Java 21 baseline, revisit this to model it as a `sealed interface` with data-carrying variants (e.g. `GlobalPosition(long)`, `SpecificTime(...)`) plus a pattern-matching `switch`, so the routing carries the position/time value in the type instead of re-extracting it, and apply the same classify-then-switch modernization elsewhere. The baseline bump itself is consumer-facing (raises the required JDK, drops the `build (17)` CI matrix job) so it is its own PR, never bundled into feature work.
