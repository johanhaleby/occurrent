...
## A fleet-wide simultaneous stall is an account-level event, not N unit stalls (2026-08-13)
The stalled-worker detector fired with every chip worktree quiet for 40 minutes at once. The cause was the account spend limit, hit at ~09:0x, which stopped every session from running turns, including nudges already queued. Diagnostic rule: when ALL workers go idle in the same window, check environmental causes (spend limit, host outage, network) before reading per-unit tails, and after the cause clears, re-wake each unit with its open items restated, because messages queued during the outage may have been swallowed by errored turns.

## Four rounds of the same race family means the synchronization model is wrong (2026-08-13)
U3's marker bookkeeping went through eight review rounds, and rounds five through eight were all one family: check-then-act windows between lock-free map operations and the delegate's own state transitions. Each patch (identity checks, pre-checks, conditional reinstates) narrowed one interleaving and revealed the next. The third-rewrite lesson generalizes from sentences to synchronization designs: when adversarial review keeps finding the same CLASS of race, stop patching windows and change the model, here per-id mutual exclusion that makes the whole invariant table true by construction and deletes the reference counter entirely. The tell is that the first review round already named the alternative ("or same-ID operations need serialization") and it was declined for diff-size reasons that four extra rounds then dwarfed.

## Full local suites with testcontainers are redundant beside a green matrix on the same head (2026-08-14)
Johan stopped U13's worker mid-grind: it had CI 27/27 green by job conclusion on its exact head and was still running multi-module docker/testcontainers suites locally, twice, at 25-45 minutes each. The verification of record for suites is CI on the PR's exact head. Local runs are only for targeted evidence CI cannot produce: the base-fail/fix-pass regression demonstration, and mutant kills with narrowly scoped test selections (single class or method, offline). Every implementation brief states this split explicitly, because "run the full suites with -am" as brief language reads as a local obligation even after CI has already answered it.

## A monitor outlived its lifetime condition because the rule and the checklist lived in different phases (2026-08-14)
The work-item monitor ran an hour past U13's merge, the last PR-bearing unit, and Johan noticed before the orchestrator did. The lifetime rule existed in the skill's Phase 2 sweep-cadence prose; the merge-time procedure in Phase 4 never mentioned monitors, so the tick that flipped the condition ran branch sweep, state update and round-3 dispatch and never asked the question. Graduated to the skill: Phase 4 gains an explicit re-evaluate-monitor-lifetimes step, and arming a monitor now journals its retirement condition so the check is a mechanical list read, not a remembered rule. Cheap direction of the detection-defect family, same lesson: an invariant is checked where its transition happens, or it is not checked.
...
## A squash body inherits a [ci skip] tail commit and silently skips main's matrix (2026-08-14)
C2's squash merged with gh's default body, which concatenates the PR's commit messages. The prose tail commit carried the [ci skip] prefix, GitHub honors the marker anywhere in the message, and a +916/-166 code squash landed on main with zero workflow runs. Caught one merge later when the run list showed no rows for the SHA, and closed by the next squash's matrix running on the combined tree. Rule: before any squash merge, check whether ANY commit in the PR carries a skip marker, and when one does, pass an explicit --body with the marker stripped. The prose-only [ci skip] convention applies to PR-side heads, never to a squash landing code on the default branch.

## A cause-asserted-from-an-effect overclaim is a class, and its fix is a claim grep, not a site edit (2026-08-14)
One unit hit the same defect four times: a sentence asserting a cause (another node's position, a removed checkpoint) where the code observes only an effect (a refused write, an empty read). Each fix corrected the instance in front of the author, and the next review pass found the survivors elsewhere in the same change (message strings, then class javadoc, then field javadoc and @throws). The rule the worker distilled, adopted fleet-wide: when this shape is confirmed anywhere, grep the CLAIM across every surface of the change (messages, all javadoc sites, ADR, changelog, guide, tests) in one pass before recommitting, exactly as the invariant-first rule already demands for behavior claims.

- A claim-class sweep that verifies each absolute against source can still pass prose that is wrong: C8's pre-push sweep confirmed every never/always/only claim against the code as written, yet six residual overclaims survived, all falsified by cross-mode cases the single-mode reading never exercises (data written via forStandalone later read in Cluster-safe mode, stopped-model registration deferring the pin). Verifying an absolute means hunting a counterexample across mode boundaries and lifecycle timings, not confirming the happy path it describes; the independent adversarial reader stays mandatory even after a clean sweep.

- A test can lock a defect in as intended behavior: cdx33d's blocker survived five adversarial passes and two fixpoint rounds because the swallow-and-fall-forward path carried a rationale comment and a green test named for its fallback behavior, so every reader classified it as design. When sweeping an absolute release-contract claim, enumerate the code's ERROR and EMPTY branches against the claim, and treat a fallback-on-failure test as a claim to verify, not as evidence of intent.
- A citation is only as good as the ref actually read: the D1 planner disputed a verified fact by quoting the primary checkout's stale working tree (8 commits behind) while believing it read origin/main. Resolve such disputes with a pipeline-free read of the blob (python subprocess over git show, no rtk-rewritable shell stage), and require workers to read via git show on the fetched ref, never the working tree of a checkout they did not pin.
- Coordinator-performed blocked operations are permission laundering: when a subagent's classifier denies an operation, performing it for them and handing over the result defeats a control scoped to their session. Two workers correctly refused exactly this. The clean route is the tooling-sanctioned one (an isolation worktree assigned at launch), or routing the specific grant to the user.
- The foreground-waits-only rule failed five workers across three epics despite being a numbered brief step. A mid-brief sentence does not survive contact with a model's wait instinct; the fix belongs in the skill's worker-brief template as a structural ending condition (the only permitted stop states: DELIVERY_RESULT emitted, or BLOCKED with a question), not as advice.
- Never rationalize an exception to a lifetime invariant in the same breath as recording it: revision 9 declined to re-arm the work-item monitor for a unit holding an unmet PR deliverable because a subagent notification would cover it, exactly the reasoning the every-sweep invariant exists to forbid. The correction is cheap when caught next tick and embarrassing when the user catches it first.

- A ruling that reverses a written tracker disposition must reach the tracker in the same tick as the ruling, not at CREG time: E8's worker verified its brief against GitHub, found the 2026-08-11 deferral comments saying do-not-re-ask still standing, and correctly refused to act on a session-internal "ruled by Johan" line it could not verify. The register is only as authoritative as its most recent write, so a reversal recorded solely in epic state leaves every fresh reader with the old truth.

- The suppressed-comments discipline needs a convergence rule stated in the brief: reading every suppressed block is mandatory, fixing every suppressed item is not. Suppressed findings are low-confidence by definition, every rewrite gives the next round fresh low-confidence surface, and a worker that fixes each nit loops indefinitely (E8 reached ten heads). The brief wording that ends it: triage each suppressed item to fixed or reasoned-declined in ONE pass, and a round whose remaining items are all declined is the final round, deliver without another push.
