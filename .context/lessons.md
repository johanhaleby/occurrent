...
rel34 tested that check rather than adopting it and found the hole, which is that `Comments generated` EXCLUDES suppressed comments. PR 914 reads `Suppressed comments (2)` and `Comments generated: 0 new` two lines apart, so zero generated against zero inline aligns perfectly while two real findings sit unread, one of them the longest-unaddressed defect on that PR. **So it is two pairs, not one.** `generated` against the inline count catches a finding living in body prose. `Suppressed comments (N)` catches findings that open no thread by construction. Neither implies the other, and the second class is the one that does the damage.

Two refinements from checking seventeen Copilot reviews across five PRs. The `new` suffix appears ONLY as `0 new`, never as a positive count, so it is the phrasing for a review that generated nothing rather than a new-since-last counter, which is worth knowing because a re-review reading `0 new` is silent about findings that still stand. What does surface those is inside the suppressed block itself: `**Previously missed (N)** in code that hasn't changed since the last review`. So the suppressed block carries both the findings that never opened a thread and the ones a prior round left unfixed, which makes reading it the single highest-value part of the whole gate.

Applied backwards over sdi's own merged PRs, the method gap was real and the outcome was not. PR 923 carried one then four suppressed findings and PR 924 carried one then one, none of which this orchestrator's gate ever read. All four were addressed before merge, by the workers, who did read them. The right result reached by a route that could not establish it, which is the same shape as the zero byte grep and the effort-level skim, three times in one day from three different directions.
