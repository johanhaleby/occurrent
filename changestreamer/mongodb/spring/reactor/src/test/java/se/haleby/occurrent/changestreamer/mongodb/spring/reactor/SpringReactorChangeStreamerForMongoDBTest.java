...
@Testcontainers
public class SpringReactorChangeStreamerForMongoDBTest {
    ...
    private SpringReactorChangeStreamerForMongoDB changeStreamer;
    ...
    void create_mongo_event_store() {
        ...
        changeStreamer = new SpringReactorChangeStreamerForMongoDB(reactiveMongoTemplate, "events", TimeRepresentation.RFC_3339_STRING);
    ...
    @Test
    void using_bson_query_for_type_with_reactive_spring_change_streamer() throws InterruptedException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        CopyOnWriteArrayList<CloudEvent> state = new CopyOnWriteArrayList<>();
        changeStreamer.stream(filter().type(Filters::eq, NameDefined.class.getSimpleName()))
    ...
    @Test
    void using_bson_query_dsl_composition_with_reactive_spring_change_streamer() throws InterruptedException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        CopyOnWriteArrayList<CloudEvent> state = new CopyOnWriteArrayList<>();
        NameDefined nameDefined1 = new NameDefined(UUID.randomUUID().toString(), now, "name1");
        ...
        changeStreamer.stream(filter().id(Filters::eq, nameDefined2.getEventId()).and().type(Filters::eq, NameDefined.class.getSimpleName()))
                .flatMap(cloudEvent -> Mono.fromRunnable(() -> state.add(cloudEvent)))
                .subscribe();
    ...
    @Test
    void using_bson_query_native_mongo_filters_composition_with_reactive_spring_change_streamer() throws InterruptedException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        CopyOnWriteArrayList<CloudEvent> state = new CopyOnWriteArrayList<>();
        NameDefined nameDefined1 = new NameDefined(UUID.randomUUID().toString(), now, "name1");
        ...
        changeStreamer.stream(filter(match(and(eq("fullDocument.id", nameDefined2.getEventId()), eq("fullDocument.type", NameDefined.class.getSimpleName())))))
                .flatMap(cloudEvent -> Mono.fromRunnable(() -> state.add(cloudEvent)))
                .subscribe();
    ...
    @Test
    void using_json_query_for_type_with_reactive_spring_change_streamer() throws InterruptedException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        CopyOnWriteArrayList<CloudEvent> state = new CopyOnWriteArrayList<>();
        changeStreamer.stream(JsonMongoDBFilterSpecification.filter("{ $match : { \"" + FULL_DOCUMENT + ".type\" : \"" + NameDefined.class.getSimpleName() + "\" } }"))
                .flatMap(cloudEvent -> Mono.fromRunnable(() -> state.add(cloudEvent)))
                .subscribe();
        NameDefined nameDefined1 = new NameDefined(UUID.randomUUID().toString(), now, "name1");
        NameDefined nameDefined2 = new NameDefined(UUID.randomUUID().toString(), now.plusSeconds(2), "name2");
