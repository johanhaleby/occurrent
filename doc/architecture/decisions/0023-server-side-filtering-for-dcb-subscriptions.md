...
The subscription filter layer already has the seam needed to fix this. `org.occurrent.subscription.SubscriptionFilter` is an empty marker interface. `StreamSubscriptionFilter` is its stream implementation, wrapping an Occurrent `Filter`. The Mongo subscription models already dispatch on the concrete `SubscriptionFilter` type: `ApplyFilterToChangeStreamOptionsBuilder` (Spring) and `NativeMongoSubscriptionModel` (native driver) translate `StreamSubscriptionFilter` to a change stream `$match` and throw on filter types they do not recognize.
...
- Add `org.occurrent.subscription.DcbSubscriptionFilter`, a record wrapping a `DcbQuery`, in `subscription/core`. It is the DCB counterpart to `StreamSubscriptionFilter`: the stream filter wraps a `Filter`, the DCB filter wraps a `DcbQuery`. The stream `Filter` class is not changed or forked.
