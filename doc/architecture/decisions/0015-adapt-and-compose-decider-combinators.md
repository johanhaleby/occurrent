...
Two fixed-arity Kotlin overloads cover the common cases and are the ergonomic surface: they return a `Pair` for two deciders and a `Triple` for three, and because they are `inline` with reified sub-types they `adapt` each constituent themselves. For four or more deciders a list form `compose(deciders)` returns a `CompositeState` that holds each state positionally with a typed `slice(index)` accessor. The list form does NOT adapt, because the elements of one list cannot carry the per element types `adapt` needs, so its deciders must already share `C` and `E`. It takes a `List` rather than a `vararg` on purpose, so that a two or three decider call resolves to the typed, auto-adapting overloads rather than to the list form. This asymmetry, fixed arity adapts and the list form does not, is the price of the cleaner common-case call site.
