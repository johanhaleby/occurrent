...
The canonical form is a vararg `compose(...)` for any number of deciders, returning a `CompositeState` that holds each state positionally with a typed `slice(index)` accessor. On top of it, two fixed-arity Kotlin overloads fully type the common small cases, returning a `Pair` for two deciders and a `Triple` for three. Kotlin resolves the fixed-arity overloads ahead of the vararg, so callers get the typed tuple for two or three deciders and fall to `CompositeState` only at four or more.
