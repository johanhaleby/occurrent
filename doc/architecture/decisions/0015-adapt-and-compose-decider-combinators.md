...
That layer should also split the event type into an input event and an output event, in the style of Fmodel's `Ei` and `Eo`. The shape is `Decider<C, S, Ei, Eo>` with `Eo` a subtype of `Ei`, where `evolve` folds any `Ei` and `decide` returns only `Eo`. A DCB cross-boundary decider is the textbook case: it reads a broad set of events to build its state but emits only its own. The course-enrollment example shows it, the enrollment decider reads every `DomainEvent` in its boundary (course capacity, student existence, enrollments) but only ever emits `CourseEnrollmentEvent`. Today it is typed `Decider<EnrollmentCommand, EnrollmentState, DomainEvent>`, which is sound but does not narrow `decide`'s return type to the events it actually produces. The `Eo` subtype-of-`Ei` constraint is not optional, because a decider folds its own emitted events back into its state, so it must be able to `evolve` from anything it emits. The everyday `Decider<C, S, E>` is the `Ei == Eo` case and stays the default. This split belongs in the DCB decider layer rather than in the core `Decider`, because widening the core interface to four type parameters would ripple through every decider, both application service DSLs, the `decider-arrow` module, and `adapt` and `compose`, for a gain that only cross-boundary deciders need. A non-breaking alternative, if it is ever wanted in the core, is a general `BiDecider<C, S, Ei, Eo>` base that `Decider<C, S, E>` extends as `BiDecider<C, S, E, E>`, but that still needs split-aware `create`, `adapt`, `compose`, and `execute`.
