...
The rule is enforced through a single `retryOnlyWhenThisStoreOwnsTheTransaction` helper in each Spring store. Both DCB retries go through it, the append itself and the position counter's cold start. Routing them through one place is deliberate: the counter retry was written without an ownership check and is exactly the kind of site that gets added again later.

One retry is knowingly left outside the rule. The stream `write` path retries a `WriteConditionNotFulfilledException` for an any-version write, and that retry has the same weakness when it runs inside a caller's transaction. It is not changed here because `write` has been released far longer than DCB and has many more callers, and nobody has reported the problem. Note that the counter guard reaches the stream path anyway, since `write` reserves positions through the same method.
