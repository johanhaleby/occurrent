...
> that has since been fixed for 0.34.0. [#836](https://github.com/johanhaleby/occurrent/issues/836) was the issue
> it says fixing this needs, and it closed against that milestone. `SubscriptionAnnotations.invokeDescriptorFactory`
> unwraps a bean to its ultimate AOP target and runs the recorded factory method there directly, but only when the
> unwrapped target's own class equals exactly the class the scan found the method on, shared by all five registrar
> paths that make this call, so a JDK interface proxy no longer fails. Equal, not merely a subtype, because a
> subtype could override the method with a different implementation the scan never saw. When the target's class
> does not match exactly, the factory refuses instead of guessing which method to run, naming the annotation, the
> factory, and both causes that can produce that mismatch. A proxy backed by a prototype- or pool-scoped target
> source that cannot be unwrapped safely is one. A factory bean that built a different implementation, whether an
> unrelated class or a subclass overriding the method, on a later call than the one the scan recorded is the other,
> closed by #990 after an earlier version of this fix matched on the resolved class by name or by mere
> assignability, either of which a wrong implementation could still satisfy. Corrected here because the changelog
> entry for #836 links to this section, so a reader arriving from the release notes would otherwise be told that
> what they had just read was fixed is still broken, or would read a description of the fix that no longer matches
> what it does.
