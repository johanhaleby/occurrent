...
`@StreamSubscription`, `@DcbSubscription`, and `@Projection` and become shared top-level types. An OpenRewrite
recipe handles the rewrite for you.
...
`org.occurrent.annotation.ResumeBehavior`/`org.occurrent.annotation.StartupMode`. This covers a fully-qualified
reference, an import, and a static import. Safe to run and commit without review.
...
`StartPosition` and `DcbStartPosition` are untouched by this release. Their constants differ per annotation
(`Subscription.StartPosition.BEGINNING` versus `StreamSubscription.StartPosition.BEGINNING_OF_TIME`, and
`DcbSubscription.DcbStartPosition`'s own constants), so they stay nested and annotation-specific.
