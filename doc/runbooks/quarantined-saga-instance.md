...
On the first delivery after the budget elapses, the same logger logs an `ERROR` saying the instance is now
`QUARANTINED`, and that line is the one to alert on. The quarantine is decided on a delivery rather than on a clock,
so where nothing more is delivered to that instance the line never comes. Any failing delivery to it will do, not only
another of the event it stopped on. The `ERROR` names two durations, how long the instance had been failing and then
