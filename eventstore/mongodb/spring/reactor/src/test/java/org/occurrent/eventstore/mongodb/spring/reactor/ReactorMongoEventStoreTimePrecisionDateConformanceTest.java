...
import org.occurrent.tck.eventstore.blocking.EventStoreTimePrecisionConformance;
...
/**
 * Runs the blocking time-precision suite against the reactive store through {@link BlockingEventStoreOverReactive}, so
 * the scenarios are described once rather than a second time in terms of {@code Mono} and {@code Flux}.
 */
@Testcontainers
class ReactorMongoEventStoreTimePrecisionDateConformanceTest extends EventStoreTimePrecisionConformance {
    ...
    protected EventStoreFixture createFixture() {
        return new ReactorMongoEventStoreConformanceFixture(new ConnectionString(mongoDBContainer.getReplicaSetUrl() + ".events"), TimeRepresentation.DATE);
    }
}
