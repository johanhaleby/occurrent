...
public class CloudEventDaoLogOperations extends EventLogOperationsDefaultImpl<CloudEventDao> {
  ...
  public <Y> Expression<Y> expressFieldName(Root<CloudEventDao> root, String fieldName) {
    return FieldNames.fromStringSafe(fieldName)
        .map(x -> CloudEventDaoLogOperations.<Y>expressFieldName(root, x))
        .orElseThrow(
            () ->
                new IllegalArgumentException(
                    "Field \"%s\" not represented in FIELD_NAMES enum".formatted(fieldName)));
  }
