...
* The blocking event store write API now takes `List<CloudEvent>` instead of `Stream<CloudEvent>`, and the application service domain function is now `Function<List<E>, List<E>>` instead of `Function<Stream<E>, Stream<E>>` (with the side effect a `Consumer<List<E>>`, and the reactor domain function likewise a `List` function while its `Mono`/`Flux` I/O is unchanged). Reads and queries stay lazy: `EventStoreQueries`, `readInPositionOrder`, `EventStream.events()` and `CloudEventConverter.toDomainEvents` still return `Stream`, and the reactor `write(Flux<CloudEvent>)` signature is untouched. The original `Stream` write API was chosen for a lazy-insert that never materialized (every store collected the stream immediately, and an append is a bounded transaction that may retry), so this drops that ceremony and matches the DCB `append(List<CloudEvent>)` that already existed. As part of this the `Stream`/`Sequence` shape-bridging is removed: `CommandConversion`, `StreamCommandComposition`, the Kotlin `executeSequence`/`sideEffectOnSequence` families, and the `write(Sequence<CloudEvent>)` Kotlin extensions are gone (compose with `ListCommandComposition` and call `executeList`/`write(List)` instead). This is a breaking change to the released stream API. See [ADR 54](doc/architecture/decisions/0054-list-instead-of-stream-for-event-store-writes.md).
* The integration tests now run against MongoDB 8.0 instead of the end of life MongoDB 4.2, raising the MongoDB version Occurrent is validated against. As part of this the MongoDB event stores now treat error code 86 (IndexKeySpecsConflict) the same as error code 85 (IndexOptionsConflict) when an operator has created an incompatible stream version index out of band, since MongoDB 7.0 and later report that clash as 86. A sort that combines a field with a natural step now applies the natural order alone, matching how MongoDB 4.x behaved and how MongoDB 7.0 and later require a natural sort to be specified.
