...
* Events that Occurrent's own `updateEvent` damaged in 0.33.0 or earlier can be repaired with the new `occurrent-eventstore-mongodb-update-event-repair` module, and a store now warns at startup when it finds an event with a damaged position. See the `updateEvent` entry under Changes for what the damage was and what a repair cannot restore.
