...
* **A projection, a subscription built through the subscription DSL, a query through `DomainEventQueries`, a `@Snapshot` view, `ExecuteFilter`'s `type`/`includeTypes` and `DcbCriteriaBuilder`'s `type`/`types` now each expand a declared sealed event type into the concrete types it permits, the same fix 0.33.0 already shipped for a saga and an annotation-based subscription.** Declaring a sealed supertype used to derive a filter naming only that supertype's own CloudEvent type, so a projection, a subscription, a query, a snapshot, an application service execute filter, or a DCB criterion keyed on one silently matched fewer events than dispatch would actually accept. All six now enforce the same `EventTypeExpansion.expand` rule the saga DSL and the annotation registrars already used, the four DSLs above through the shared `deriveFilter`, `ExecuteFilter` and `DcbCriteriaBuilder` directly, since neither of those two derives a plain `Filter`. `DomainEventQueries.query(Collection)` keeps treating a null or empty collection as "match nothing" rather than "match everything", unaffected by the wider expansion. See [ADR 126](doc/architecture/decisions/0126-every-derived-event-type-filter-expands-a-declared-sealed-type.md). Resolves [#750](https://github.com/johanhaleby/occurrent/issues/750), [#758](https://github.com/johanhaleby/occurrent/issues/758) and [#912](https://github.com/johanhaleby/occurrent/issues/912).
* **`ExecuteFilter.excludeTypes` now widens a declared type it cannot fully expand to every concrete subtype a downward walk can find, instead of naming only the declared type's own CloudEvent type.** Excluding a supertype has to exclude everything under it, since a narrower exclusion would silently read events the caller asked to keep out, so `excludeTypes(SensitiveEvent.class)` on a sealed `SensitiveEvent` now keeps out every concrete subtype it permits, not just events stored under `SensitiveEvent`'s own CloudEvent type. This is the opposite treatment from `type`/`includeTypes` and `DcbCriteriaBuilder`'s `type`/`types` above, and on purpose. Those refuse a declaration they cannot fully expand, since a narrower inclusion would silently miss events dispatch would accept, while widening an exclusion can only exclude more. The walk it widens over is the sealed-permits walk, which starts at the declared type, follows a `permits` clause through `Class.getPermittedSubclasses`, and stops at the first level that is not sealed, so a type it cannot reach means an event the caller wanted out stays in, rather than the reverse. **Widening is not the same as completeness, and two shapes still need calling out plainly rather than merged into "always safe."** A concrete class declared directly that is itself neither final nor sealed contributes itself, so the exclusion still does something there, exactly as before this fix, just without reaching that class's own unsealed subclasses. An interface or an abstract class whose hierarchy reopens before the walk finds anything concrete contributes its own declared name and nothing else, and how much that excludes is decided by your `CloudEventTypeGetter` rather than by the walk. Under a getter that maps each type to its own class name, which is what `ReflectionCloudEventTypeMapper` does in both its qualified and its simple form, no stored event is written under that name, so `excludeTypes(SensitiveEvent.class)` on a `SensitiveEvent` that permits only a non-sealed abstract class excludes zero real events, silently, exactly as it did before this fix. Under a `CloudEventTypeMapper` of your own that maps the whole hierarchy onto one CloudEvent type string, the same declaration excludes the whole family instead, since that one string is what the concrete events are stored under. Seal the hierarchy or declare the concrete types directly for an exclusion that does not depend on which of those two your mapper is. An array or a primitive declared type is refused here too, the same two shapes `type`/`includeTypes` refuse, one because no event is ever an instance of a primitive type and the other for consistency with the other two methods rather than impossibility, and that refusal is filed separately below under `#### Breaking changes` rather than here, since it is one. See `EventTypeExpansion.expandWhatCanBeFound`. The widening itself is not a breaking change in the same sense as the refusal above, it makes `excludeTypes` do what its name always promised for every shape it can actually reach. Resolves [#912](https://github.com/johanhaleby/occurrent/issues/912).
...
* **A projection, a subscription built through the subscription DSL, a query through `DomainEventQueries`, or a `@Snapshot` view that declares an event type whose concrete subtypes cannot be found is now refused, the same refusal 0.33.0 already shipped for a saga and an annotation-based subscription. And a concrete class that is neither final nor sealed, which every one of those six exempted in 0.33.0, is refused too. `ExecuteFilter`'s `type`/`includeTypes` and `DcbCriteriaBuilder`'s `type`/`types` get the same refusal for the first time, since neither expanded a declared type at all before this release.** You are affected if one of these declares an interface or an abstract class that is not sealed, an array, a primitive class literal, a sealed hierarchy reopened below the declared level, or a concrete class that is neither final nor sealed. Only a saga and an annotation-based subscription already refused the first five shapes in 0.33.0; every other place named above, `ExecuteFilter` and `DcbCriteriaBuilder` included, is newly exposed to all six. The last shape also changes the saga and the annotation-based subscription, since 0.33.0 exempted it there on purpose. Up to 0.33.0 that shape derived a filter naming the declared class alone, so under every `CloudEventTypeMapper` Occurrent ships a caller declaring `class OrderPlaced` and publishing a `class SpecialOrderPlaced extends OrderPlaced` never received the subclass and got no warning, even though dispatch would have accepted it. One caller was working rather than losing events, the one whose own `CloudEventTypeMapper` maps a whole hierarchy onto a single CloudEvent type string, since the subclass was then stored under the declared class's type string and did arrive. That caller is refused too and wants an explicit filter, the same remedy the shapes above already point it at. `Projection` and `SnapshotView` throw when a runner or a query derives the filter, `DomainEventQueries` and the subscription DSL throw at the first query or subscription registration that needs one, a saga throws at `build()`, `SnapshotAnnotationRegistrar` and the subscription annotations throw at Spring Boot startup, `ExecuteFilter` throws when `ApplicationService#execute` resolves the filter, and `DcbCriteriaBuilder` throws when `type(...)`/`types(...)` builds the criterion. Three remedies for the concrete-class shape, mark the declared class `final` when nothing extends it, seal the hierarchy, or declare the concrete event types, and two for the rest, seal the hierarchy or declare the concrete event types. `Projection.filter(...)`, `SnapshotView.filter(...)`, a saga's `replacementFilter(...)` and `ExecuteFilter.from(StreamReadFilter)` are a further way out for the four that have an override; `DcbCriteriaBuilder` has none, so build the `DcbCriterion` from the raw CloudEvent type string with `DcbCriteria.type(String)`/`types(String, ...)` instead. There is no recipe entry, for the same reason 0.33.0's equivalent change had none, and for the concrete-class shape also because a recipe cannot tell whether a class should become `final` or the declaration should list concrete types. See [section 3 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#3-declaring-an-event-type-whose-concrete-subtypes-cannot-be-found-is-refused) and [ADR 126](doc/architecture/decisions/0126-every-derived-event-type-filter-expands-a-declared-sealed-type.md). Resolves [#753](https://github.com/johanhaleby/occurrent/issues/753) and [#912](https://github.com/johanhaleby/occurrent/issues/912).
* **`ExecuteFilter.excludeTypes` now refuses an array or a primitive declared type, where up to 0.33.0 it was accepted and handed straight to the configured `CloudEventTypeGetter`.** Unlike the widening `excludeTypes` gains above, this half is a genuine behavior break, since a call that returned some filter, however degenerate, now throws `IllegalArgumentException` instead. For a primitive that means declaring the concrete event types you actually meant. An array class is already concrete, so there is nothing narrower to declare, and it is refused for consistency with `type`/`includeTypes` rather than because excluding one is impossible, which makes `ExecuteFilter.from(StreamReadFilter)` the way out if you do mean to exclude an array type. Nobody sensibly excludes by array or primitive type in the first place, so in practice this reaches close to no one, but a call that used to build something now does not, which is what belongs under this heading rather than filed alongside the widening as a footnote. See [section 3 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#3-declaring-an-event-type-whose-concrete-subtypes-cannot-be-found-is-refused). Resolves [#912](https://github.com/johanhaleby/occurrent/issues/912).
...
* **`SagaEnvelope`, `SagaInstance`, `SagaStatus` and `SagaRunnerConfig` all change shape for the saga quarantine above.** `SagaEnvelope` gains `started` and `failure`, `SagaRunnerConfig` gains `quarantineAfter`, so a record pattern over either stops compiling. An ordinary `new SagaEnvelope(..)` or `new SagaRunnerConfig(..)` on the old arity still compiles, because both keep a constructor with the old argument list that fills the new components in. `SagaInstance` gains `failure()`, which breaks anyone implementing that interface outside this repository. `SagaStatus` gains `QUARANTINED`, so an exhaustive `switch` or `when` over it stops compiling, and, less visibly, `findByStatus(ACTIVE, ..)` no longer returns the instances that have stopped moving, which is what it was most often used to find. That is five breaks. There is no OpenRewrite recipe, because none of them can be rewritten mechanically. See [Upgrading to 0.34.0](https://occurrent.org/migration/upgrading-to-0.34.0), section 8.
* **A reactor catch-up subscription can now deliver an event twice across the replay-to-live handover, where before the history ids in the bounded cache suppressed the second delivery, see Changes above.** This changes observable delivery for the reactive catch-up models shipped in 0.30.0, for callers who never touch `recordAppliedAppends`. Catch-up delivery on those models has always been at-least-once, so a handler written to tolerate a repeat needs no change, and there is no recipe because nothing in a caller's source declares a requirement that an event arrives once. If yours is not safe to run twice on the same event, see [Upgrading to 0.34.0](doc/migration/upgrading-to-0.34.0.md#9-a-reactor-catch-up-subscription-can-deliver-a-concurrent-write-twice).
