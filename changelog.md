...
* **`ExecuteFilter.excludeTypes` now widens a declared type it cannot fully expand to every concrete subtype a downward walk can find, instead of naming only the declared type's own CloudEvent type.** Excluding a supertype has to exclude everything under it, since a narrower exclusion would silently read events the caller asked to keep out, so `excludeTypes(SensitiveEvent.class)` on a sealed `SensitiveEvent` now keeps out every concrete subtype it permits. That is the opposite treatment from `type` and `includeTypes` above, on purpose. Those refuse a declaration they cannot fully expand, since a narrower inclusion would silently miss events dispatch would accept, while widening an exclusion can only exclude more. See `EventTypeExpansion.expandWhatCanBeFound`. Resolves [#912](https://github.com/johanhaleby/occurrent/issues/912).
  * The walk starts at the declared type, follows a `permits` clause through `Class.getPermittedSubclasses`, and stops at the first level that is not sealed, so a type it cannot reach means an event the caller wanted out stays in.
