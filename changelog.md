...
* Restored ADR 61's layering and closed a snapshot-key collision. `SnapshotPolicies.whenTerminal(Decider)`, the only decider-aware member of `snapshot-dsl-common`, moves to `org.occurrent.dsl.snapshot.blocking`/`.reactor` in the executor modules (import from there instead of `org.occurrent.dsl.snapshot`), so a `SnapshotView`-only consumer no longer pulls `occurrent-decider` onto its classpath; `snapshot-dsl-common`'s `pom.xml` drops the dependency accordingly. `DcbSnapshotKeys.canonicalKey` now SHA-256-hashes its canonical, sorted rendering of a `DcbCriteria` instead of returning the delimited string directly, since two structurally distinct criteria could otherwise render to the identical string (for example a single type named `"A,B"` and the two types `"A"`/`"B"` both rendered as `types[A,B]`) and silently share, and corrupt, one snapshot. `DcbExecuteOptions` (blocking and reactor) drops its structural `equals`/`hashCode`, matching `ExecuteOptions`, which never had one: both carry lambda-typed fields compared by identity, so a structural equals was misleading rather than useful.
