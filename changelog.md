...
* **`occurrent.event-store.collection`, `occurrent.event-store.time-representation`, `occurrent.subscription.collection` and `occurrent.subscription.restart-on-change-stream-history-lost` are renamed to `occurrent.event-store.mongodb.collection`, `occurrent.event-store.mongodb.time-representation`, `occurrent.subscription.mongodb.collection` and `occurrent.subscription.mongodb.restart-on-change-stream-history-lost`.** Each old key still works and is deprecated, removed in the release after next, and the new `UpgradeToOccurrent_0_34` OpenRewrite recipe rewrites both `.properties` and `.yaml` for you. See [section 2 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#2-four-mongodb-only-keys-move-under-mongodb). Resolves [#439](https://github.com/johanhaleby/occurrent/issues/439).
