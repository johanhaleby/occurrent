...
* **A `@Saga(source = PUSH)` started through `ManualStartPushSources` while the application context is closing now stops its timer poller instead of letting it outlive the context.** The poller went on dispatching commands into a context that had already shut down. Fixes behaviour shipped in 0.33.0. Resolves [#988](https://github.com/johanhaleby/occurrent/issues/988).
