...
* Restored ADR 61's layering and closed a snapshot-key collision. `SnapshotPolicies.whenTerminal(Decider)`, the only decider-aware member of `snapshot-dsl-common`, moves to `org.occurrent.dsl.snapshot.blocking`/`.reactor` in the executor modules (import from there instead of `org.occurrent.dsl.snapshot`), so a `SnapshotView`-only consumer no longer pulls `occurrent-decider` onto its classpath. `snapshot-dsl-common`'s `pom.xml` drops the dependency accordingly. `DcbSnapshotKeys.canonicalKey` now length-prefixes each type and tag value before joining them, so two structurally distinct criteria can no longer render to the identical string (for example a single type named `"A,B"` and the two types `"A"`/`"B"` previously both rendered as `types[A,B]`, and would silently share, and corrupt, one snapshot). This changes the key format, so a DCB snapshot persisted before this change will not be found under its new key after upgrading. The executor treats it as absent and rebuilds from a full replay, which only costs a longer read once. `DcbExecuteOptions` (blocking and reactor) drops its structural `equals`/`hashCode`, matching `ExecuteOptions`, which never had one. Both carry lambda-typed fields compared by identity, so a structural equals was misleading rather than useful.
* Hardened snapshot resume against a stream reset. A snapshot whose version is ahead of the stream's true head (which happens when a stream is reset, or truncated, below a snapshot written against the longer stream) is now discarded instead of trusted: `resolveBase` and `isRedelivery` demote such a snapshot to the initial state and fold fresh, on both the DSL executors and the maintained `@Snapshot` path. The maintained `@Snapshot` stream registrar probes the true head only in the ambiguous case (the delivered version at or below the snapshot version), so the happy path pays no extra read, and a reset no longer freezes the maintainer, it rebuilds and self-heals. The DSL stream executor deletes the stale snapshot with `SnapshotStore.delete` when it detects the base is ahead of the just-written head, so the next command folds fresh, and the `eventsSinceSnapshot` arithmetic now runs inside the best-effort save so nothing after a committed write can fail the command. DCB paths are unaffected because a global DCB position is monotonic and never resets. The released `ApplicationService.execute` API is unchanged. Rationale in the amendment to [ADR 61](doc/architecture/decisions/0061-first-class-snapshot-support.md).
* Hardened the Projection DSL. `DomainEventFeed.register` (blocking and reactor) now rejects a second registration under an id already taken, mirroring the durable-checkpoint-key uniqueness `RegisteringSubscribable` already enforces for subscriptions, instead of silently letting two feeds share one checkpoint. The Java `Projections` class gains `project(projection, queries)`/`project(projection, queries, instanceId)` (and a `DcbProjection` equivalent), the Java counterpart to the Kotlin on-demand fold extensions, so a Java caller can read a strongly-consistent projection state without a query DSL round-trip through Kotlin. `DcbProjection`'s compact constructor now rejects a wrapped `Projection` that carries an explicit `filter()`, since a `DcbProjection` is read through its DCB criteria and the filter would otherwise be silently ignored.
* Added a Saga DSL, a first-class way to describe an event-driven process manager: react to events, and to their absence (timeouts), by issuing commands while holding per-instance state. A `Saga<E, S, C>` is the command-issuing dual of `Decider<C, S, E>`, pure data and pure functions with no I/O, so it unit-tests the same way a decider or view does. It folds a closed input alphabet (`SagaInput`, a domain event or one of the saga's own fired timers) with `evolve`, then `react` decides effects as data (`SagaEffect`: issue a command, start or cancel a timer) against the post-evolve state, which keeps replay effect-free because rehydration folds `evolve` alone. Timers carry a `Duration` or an `Instant`, never a clock-reading `Deadline`, so a reaction stays deterministic and effect-equal in tests. Correlation is per-event-type with a `correlateAll` fallback, validated at build time so a handled event can never lack a correlation at runtime. There are two authoring surfaces that compile to the same descriptor: the machine-core builder (`Saga.builder(...)` in Java, `saga(initialState) { }` in Kotlin) and a linear flow layer (`FlowSaga.builder()` in Java, `saga { }` in Kotlin) whose steps, branches, joins, and per-step timeouts read like the process itself and lower to `Saga<E, FlowState<E>, C>`. A blocking executor runs a saga against a subscription (`SagaRunner.agnostic(...)`/`stream(...)`): it folds and persists per-instance state through a `SagaStateStore` (an in-memory implementation, and a Spring Mongo `SpringMongoSagaStateStore`), dispatches the commands each reaction issues through a `CommandDispatcher` (usually a plain lambda over an `ApplicationService`, with `CommandDispatchers.decider(...)` as an adapter into the decider machinery), and fires timeouts. Timers live in the saga's own state envelope rather than an external scheduler, so timer bookkeeping is exactly-once (saved atomically with the state) and an executor-side poller reads due timers from the store; the saga DSL therefore takes no dependency on the deadline module. Command dispatch is at-least-once (dispatched before the compare-and-set state save, so a command is never lost), which is safe because an `ApplicationService`-backed receiver re-folds the authoritative stream and rejects a stale or duplicate command. `@Saga` declares one on the blocking Spring stack (a factory method returning a `Saga`), resolving the state store by convention (an explicit or unique `SagaStateStore`, otherwise a `saga-<id>` MongoDB collection) and the command dispatcher from a bean, and registering it through the same catch-up and durable infrastructure `@Projection` uses. A saga is not a substitute for a Dynamic Consistency Boundary: where two rules must hold in one append, DCB is the right tool; a saga is for genuinely cross-boundary, time-involving, eventually-consistent processes. `adapt` ships now; `compose` is deferred because saga composition does not commute with correlation, timer namespaces, or terminal semantics. A new `order-fulfillment` example module demonstrates both surfaces, in-memory and Docker-free. Resolves [#124](https://github.com/johanhaleby/occurrent/issues/124). Rationale in [ADR 63](doc/architecture/decisions/0063-saga-dsl.md).
* A flow saga's received-event log is now bounded to a configurable window instead of retained in full, so a long-running instance no longer grows without bound (and no longer risks MongoDB's 16 MB document limit) and each save re-serializes only the window rather than the whole history. `FlowState` keeps the initiating event, the current step's events, and a configurable carry-over of earlier events, set with the flow builder's `historyWindow(int)` (default 100). A guard, a join reaction, or a timeout reaction sees only the retained window through `ReceivedEvents` (the initiating event is always available), so a guard that must count history further back than the default needs a wider `historyWindow`. A join still matches over every event received since its step was entered, regardless of the window. Rationale in [ADR 63](doc/architecture/decisions/0063-saga-dsl.md).
* Hardened the Saga DSL against a few quiet footguns: a flow saga's timer poll now projects away the state document, so polling for due timers never decodes the received log; a fired timer that resolves to no registered handler (typically a timer-name typo) is now warn-logged instead of stalling the instance silently; `FlowSaga.Builder.startsOn(...)` rejects a type already registered by a prior `correlate(...)` instead of silently overwriting it; `SpringMongoSagaStateStore` rejects a `FlowState` store built without a `CloudEventConverter`; and `Saga.create(...)` unions its start event types into a non-empty `eventTypes`, so a start event can never be filtered off the subscription.
* The saga timer poller is now lease-gated in multi-instance deployments, so only one instance queries the state store for due timers instead of every instance polling on its own interval. `SagaRunner.run` gains an overload taking a `CompetingConsumerStrategy` (the same lease SPI the competing-consumer subscription model uses); the poller registers a competing consumer under a `saga-timer:<id>` key (kept distinct from the event subscription's own lease so the two never collide) and polls only while it holds the lease, releasing it on `SagaSubscription.close` so another instance takes over within roughly one lease period. The `@Saga` Spring starter wires the shared strategy in by default, mirroring the subscription model; opt out with `occurrent.saga.competing-consumer.enabled=false`. The poll interval defaults to 15 seconds (matching JobRunr's default, set with `occurrent.saga.timer-poll-interval`), since a saga timeout fires at a minutes-to-days timescale where a shorter interval mostly adds empty queries. This is additive: without a strategy the poller runs on every instance exactly as before. Timer firing was already correct across instances (compare-and-set plus idempotent dispatch, ADR 63); this removes the redundant query load. Rationale in [ADR 64](doc/architecture/decisions/0064-lease-gate-the-saga-timer-poller.md).
* Added a generic push subscription model so the Projection DSL can be fed from an external source instead of a MongoDB change stream. `PushSubscriptionModel` (blocking and reactor, in the new `occurrent-subscription-push-blocking` and `occurrent-subscription-push-reactor` modules) is a register-only `Subscribable` driven by an `accept(CloudEvent)` call rather than a change stream, so a RabbitMQ, Kafka, Spring-event, or HTTP listener can drive projections in production. It is the same shape as the synchronous subscription model, and the shared register-and-route machinery is now extracted into `RegisteringSubscribable` in the blocking and reactor subscription api modules. Occurrent stays transport-neutral: no broker dependency is added, and the pushed events must carry the Occurrent cloud-event extensions the handlers rely on (forward the stored CloudEvent as CloudEvents JSON and reconstruct it on the listener side). Because the projection DSL already accepts any `Subscribable`, `ProjectionRunner.agnostic(pushModel, converter).project(...)` and the Kotlin `project(...)` extensions work over a push model unchanged. The plain push model carries only the live tail, so a `CatchupThenPushSubscriptionModel` (blocking and reactor) adds a one-time catch-up in front of it: on first subscribe it replays a projection's history from the event store, then hands over to the live feed (buffering the feed during the replay and de-duplicating the overlap by event id), and records a one-shot catch-up-complete marker so a restart skips the replay. Live-resume is the broker's job (acknowledge after processing), so no live position watermark is persisted and delivery is at-least-once over idempotent folds. This is the "broker owns live-resume" contract, chosen because Occurrent reserves positions outside the write transaction, making a feed-derived position watermark unworkable (permanent gaps are indistinguishable from late-committing holes in the delivery stream). Declaratively, `@Projection` gains a `source` attribute: `source = Source.PUSH` (with `subscriptionModel`/`subscriptionModelName` to pick the push model bean) binds a `@Projection` to a `PushSubscriptionModel`, wrapping it in the catch-up automatically, on both the blocking and reactor stacks. The default `Source.EVENT_STORE` is unchanged. The `accept(CloudEvent)` capability is exposed as a `Pushable` interface (blocking and reactor, in the subscription api modules) that `PushSubscriptionModel` implements, so a listener can depend on the capability rather than the concrete model.
* Added domain-event feeds for projections, so a source that already hands you domain events (a RabbitMQ or Kafka listener with its own message converter) can drive a projection with no double encode/decode on the live path. `Projections.domainEventFeed(projection, repository)` returns a `Consumer<E>` (blocking) or `Function<E, Mono<Void>>` (reactor) that folds a domain event straight into the read model, with no CloudEvent conversion. `CatchupProjectionFeed` adds a one-time catch-up to that domain feed: the live path folds domain events directly, and only the catch-up reads the event store and decodes each replayed event once, de-duplicating the replay-to-live overlap by an id extracted from the domain event (so it does not depend on the CloudEvent id). `DomainEventFeed<E>` is an application-owned fan-out sink (the domain twin of `PushSubscriptionModel`) that feeds several projections from one source, and `@Projection(source = Source.PUSH, subscriptionModelName = "...")` binds a declared `@Projection` to it on both stacks, backed by a `ViewStateRepository` or a `MaterializedView` store. Rationale in [ADR 62](doc/architecture/decisions/0062-pluggable-projection-event-source.md).
* Added first-class snapshot support, an opt-in optimization that lets a command replay fold only the events written after a saved state instead of the whole history. A snapshot is a discardable, schema-versioned cache of a fold result at a known version, so a changed state shape or a missing snapshot falls back safely to a full replay and a snapshot is never a source of truth. It works from a `Decider` (the decision state) and from a plain `View` (the deciders-free read side), across stream and DCB and across the blocking and reactor stacks. The pieces: a small storage-neutral `SnapshotStore` capability (an in-memory implementation, and a Spring Mongo `SpringMongoSnapshotStore`), a higher-order `SnapshotPolicy` that unifies technical and domain triggers (`everyNEvents`, `onEvent`, `whenState`, `always`, `never`, `or`, and `whenTerminal` which is the "closing the books" trigger built on `Decider.isTerminal`), and the executors `SnapshotDeciderApplicationService` and `SnapshotDcbDeciderApplicationService` (with reactive twins) plus a `SnapshotViews` on-demand reader. The `@Snapshot` annotation declares and maintains a snapshot through the same catch-up and durable infrastructure `@Projection` uses, on both the blocking and reactor stacks and for stream and DCB (a stream snapshot is kept per stream, a DCB snapshot per boundary keyed by a canonical form of its criteria). Snapshot persistence through the DSL executors is best-effort: the snapshot is saved after the write commits, a save failure is logged rather than failing the already-committed command, and a lost snapshot only costs a fuller replay. When a snapshot must stay consistent on the write path, maintain it with `@Snapshot(mode = SYNCHRONOUS)` or a synchronous subscription, which folds it inside the write transaction (ADR 57). Closing the books is a policy plus a documented pattern rather than a separate module: model the closing balance as a real domain event that becomes the next period's opening balance, and archive prior events with the existing `deleteEventStream(String)` or `delete(Filter)` when they are no longer needed. Enabling a snapshot adds one snapshot load and one tail read per snapshotted execute, paid only where snapshots are used. Supporting core additions: `ExecuteOptions.fromStreamVersion(...)` and `DcbExecuteOptions.fromPosition(...)` (a state-agnostic read offset so the one execute path is reused, not forked) and `Decider.evolve(state, events)` (the fold-onto-a-base-state primitive, mirroring `View.evolve`). A new `closing-the-books` example module demonstrates both a technical every-N snapshot and a domain period close. Rationale in [ADR 61](doc/architecture/decisions/0061-first-class-snapshot-support.md).
* Added the `@Projection` annotation, a persistent, declarative read model built on the Projection DSL from ADR 58. A factory method annotated with `@Projection` returning a `Projection` or `DcbProjection` is registered as a managed subscription on both the blocking and reactor stacks, subscribing through the same catch-up and durable-checkpoint infrastructure `@DcbSubscription`/`@StreamSubscription` already use, so catch-up and durable resume come from the subscription model, not from the DSL. Materialization is store-agnostic through `ViewStateRepository`, `MaterializedView`, or a Spring Data `CrudRepository`, selected by the store bean's type with `store = SomeStore.class` or by bean name with `storeName` (and `storeName` disambiguates when several beans share the type), with the Mongo default on the blocking stack when both are left unset. `mode` selects `ASYNC` (catch-up then live) or `SYNCHRONOUS` (read-your-writes, reusing the synchronous subscriptions from ADR 57), and is mutually exclusive with the catch-up start knobs `startAt`, `startAtGlobalPosition`, and `resumeBehavior`. The same catch-up-then-resume behavior is reachable programmatically, without the Spring starter, through the new `ResumeStartPositions.replayThenResume(...)`/`replayThenResumeDcb(...)` helpers (blocking and reactor) passed to `ProjectionRunner`/`DcbProjectionRunner`. Rationale in [ADR 59](doc/architecture/decisions/0059-projection-annotation.md).
* Added a Projection DSL, a higher-level way to build read models. A `Projection` couples a `View` (the pure fold) with the function deriving which view instance an event updates and the event types it handles, and a `DcbProjection` adds a DCB read boundary such as a tag filter. A single-instance read model omits the per-event id with `singletonProjection`/`dcbSingletonProjection` in Kotlin or `singletonBuilder(...)` in Java, and the runtime keys its one slot by the projection's own identity. You describe the fold with a type-safe, per-event-type handler builder, in Java with `Projection.builder(initialState).on(OrderPlaced.class, (state, event) -> ...)` or in Kotlin with `projection(initialState) { on<OrderPlaced> { state, event -> ... } }`, and the registered handlers also determine the subscription filter, so the event types are declared once. Runners then create the subscription or query and keep the read model up to date. The same shape works across stream and DCB and across the blocking and reactive stacks, in three delivery modes. An asynchronous subscription updates a stored read model eventually, a synchronous subscription updates it on the write path for read-your-writes (see synchronous subscriptions above), and an on-demand variant folds a query when you ask. It builds on the existing view DSL and is the read-side counterpart to the `DcbDecider` write-side model. A new `projection-dsl` example module demonstrates it in Java and Kotlin for both stream and DCB, including the tag-scoped `isUsernameClaimed` projection. Resolves [#194](https://github.com/johanhaleby/occurrent/issues/194). Rationale in [ADR 58](doc/architecture/decisions/0058-higher-level-read-model-projection-dsl.md).
...
* The annotation start-position, resume, and startup enums move to shared top-level types in `org.occurrent.annotation`, so a 0.30.0 caller that references a nested enum has to update the reference. This is the only breaking change in this release, and the `org.occurrent.UpgradeToOccurrent_0_31` OpenRewrite recipe rewrites every affected reference for you. See the [upgrade guide](doc/migration/upgrading-to-0.31.0.md).
