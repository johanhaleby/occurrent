...
* `UpgradeToOccurrent_0_33` stubs the two new `CheckpointStorage` members for you on a class it finds missing them, delegating `any()` to the class's own existing write and marking the rest with a review comment, so the signature half of the upgrade compiles without a manual pass. Part of [#665](https://github.com/johanhaleby/occurrent/issues/665).
