...
* **The blocking `CatchupProjectionFeed.accept(..)` and `DomainEventFeed.accept(..)` now return only once the event is applied, and throw when it was not, where 0.33.0 returned as soon as the event was buffered.** A listener acknowledges the message once `accept(..)` returns, so in 0.33.0 it acknowledged an event that was only held in memory, and if the application stopped or crashed before the catch-up finished, the broker never sent that event again. A rolling deploy during a long startup replay was enough. See [section 12 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#12-a-blocking-projection-feeds-accept-waits-until-the-event-is-applied-and-throws-when-it-is-not). Resolves [#1135](https://github.com/johanhaleby/occurrent/issues/1135).
  * Before the feed goes live, `accept(..)` waits until the catch-up has applied the event. Never call it on the thread that later calls `catchUp()`, `catchUpAll()` or `goLive()`, because that thread then waits until `stopCatchUp()` is called from another thread.
  * It throws an `IllegalStateException` when the catch-up was stopped before the feed went live, the catch-up failed, the waiting thread was interrupted, or it was called on the replay's own thread from inside the view the replay is updating. Once the feed is live it also throws when another delivery of the same event is still running, where 0.33.0 returned normally. Do not acknowledge the message when it throws, and the broker delivers it again.
