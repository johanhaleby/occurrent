...
* **`occurrent-broker-kafka-blocking` publishes to and consumes from Kafka.** `KafkaCloudEventSink` and `KafkaDomainEventSink` are the publish side. A publish does not return until the broker has acknowledged the send with `acks=all`, which the sink sets for you when your producer config leaves it unset and refuses to start if you set it weaker, since an acknowledgement under a weaker setting can arrive for a send the broker never durably stored. `KafkaCloudEventBridge` and `KafkaDomainEventBridge` are the consume side, feeding a `PushSubscriptionModel` and a `DomainEventFeed` the same way their RabbitMQ counterparts do.
  ...
  * A Kafka `Consumer` is not thread-safe, so each bridge runs one dedicated poll loop that feeds the model or the feed and commits, rather than splitting that work across threads the way the RabbitMQ bridges do. `DELIVERED`, `FILTERED`, and a delivery failure resolved by a confirmed park all stage a record's offset for the next commit. Anything left unresolved seeks the consumer back to that record and stops processing the rest of that partition's poll, leaving other partitions in the same poll unaffected. `enable.auto.commit` has to be `false` or the bridge refuses to start, since Kafka would otherwise advance the offset on a timer regardless of what the bridge decided.
