...
* **`DomainEventFeed` (both stacks) gains `accept(CloudEvent)`, for a broker bridge that has a `CloudEvent` to hand over rather than an already-decoded domain event.** It matches the event against the same `Filter` `register(...)` was given, decodes it with this feed's `CloudEventConverter` only if it matches, delivers it, and returns the `RoutingOutcome`. A non-matching event is never decoded. The replay and the live path now share that one filter, so they can never disagree about which events are this projection's. `DomainEventFeed` also takes an optional `DataFieldReader` constructor argument, mirroring `PushSubscriptionModel`'s, so a payload condition on that filter can be answered on the live path too, refused at `register(...)` rather than on the first live event if none is configured. Feeding it before a projection is registered still throws `IllegalStateException`, as it always has. This feed has no write path to protect, so it keeps refusing outright rather than reporting `NOT_DELIVERABLE`. Resolves [#848](https://github.com/johanhaleby/occurrent/issues/848).
