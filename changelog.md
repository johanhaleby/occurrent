...
* Added `adapt` and `compose` decider combinators to the `dsl/decider` module.
  ...
  * `compose` combines several deciders that share a command and event type (typically reached with `adapt`) into one whose state is the product of the individual states. Each command routes to the decider that recognizes it, each state slice evolves independently, and the composed decider is terminal once every constituent is. The vararg form holds the slices in a positional `CompositeState`, and Kotlin overloads return a typed `Pair` or `Triple` for the common two and three decider cases.
