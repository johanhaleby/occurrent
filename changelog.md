...
* **A reactor catch-up subscription can now deliver an event twice across the replay-to-live handover, where before it delivered it once and a recording projection lost the append.** The reactive replay put every id it read into the cache its live delivery filters on, the events read from history included. A position is reserved before its write commits, so a write in flight when the replay read the head holds a position at or below that head and a history window reads it, and caching that id dropped the live delivery. Nothing is recorded during a history read, so that append was never recorded at all and `AppliedAppendStore.waitUntilApplied` timed out on a write that had committed. The history windows now fill no cache, which is what all three blocking paths already did, so the live subscription delivers that event again and the append is recorded. The extra delivery reaches only events a history window read whose write committed after the catch-up captured its resume position, which is empty when nothing is being written during the replay, and catch-up delivery has always been at-least-once. `handoverCacheSize` now sizes the reconciliation overlap alone, matching the blocking `cacheSize`. Resolves [#891](https://github.com/johanhaleby/occurrent/issues/891). Reasoning in [ADR 135](doc/architecture/decisions/0135-the-reactive-handover-dedup-is-fed-only-by-the-reconciliation-read.md).
