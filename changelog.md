...
* **A saga instance whose event keeps failing is now quarantined on that event instead of blocking every other instance of the same saga.** A saga has one subscription, and up to 0.33.0 an event that `evolve`, `react`, or a command dispatcher could not handle was retried on that subscription forever, so one faulty correlation id stopped every other one behind it. The executor now records when an event first failed for an instance and keeps rethrowing while the failing is younger than `SagaRunnerConfig.quarantineAfter`, five minutes by default. Past that, the instance moves to the new `SagaStatus.QUARANTINED`, stops receiving events and stops firing timers, and the subscription carries on for everyone else. Resolves [#818](https://github.com/johanhaleby/occurrent/issues/818).
  ...
  * Quarantine needs the failing event to be obtainable a second time, so before an instance is quarantined the runner asks the subscription model whether it still holds that event, through the new `HistoryRetainingSubscriptions`. The MongoDB subscription models hold everything they deliver, and a wrapper that declares nothing itself is answered by the model it wraps, so a catch-up model over one of them does too. A `CatchupThenPushSubscriptionModel` answers per event by looking in the event store it replays from, since an event this application wrote is there and one that only ever arrived over the feed is not. A push feed on its own cannot answer at all, which is what `@Saga(source = PUSH, catchup = NONE)` gives you, and quarantine is switched off for it at startup with a warning saying why. Where the answer is no, the instance keeps blocking rather than losing the event, and that refusal is logged against the event it stopped on. See [#952](https://github.com/johanhaleby/occurrent/issues/952).
