...
* **A saga can now subscribe on an explicit filter instead of one derived from its event types, with `Saga.Builder.filter(Filter)` and `FlowSaga.Builder.filter(Filter)` (`filter(...)` in both Kotlin `saga { }` blocks, and a trailing argument on `Saga.create(...)`).** It selects on subject, source, data or time the way a `Projection` already could. It also builds a saga over an event hierarchy whose concrete types cannot all be found, which is otherwise refused when the saga is built, because no filter is derived and so nothing is refused. That is the way out for a `CloudEventTypeMapper` of your own that maps a whole hierarchy onto a single CloudEvent type string, which reflection cannot know about. Three things are yours to get right once you set one. A filter that does not match the saga's start events means no instance is ever created. Every CloudEvent the filter admits is converted before the saga sees it, so one your `CloudEventConverter` cannot turn into a domain event fails that delivery instead of being ignored. And the build-time refusal is switched off for every event type the saga declares rather than only the one you could not enumerate. Resolves [#751](https://github.com/johanhaleby/occurrent/issues/751).
...
* **`SpringRedisCheckpointStorage` now evaluates `notOlderThan` and `ifAbsent` for real, instead of refusing them, on Redis Cluster too.** The write version lives in a second, prefixed key, separate from the checkpoint's own, so a node still on the previous release keeps reading the checkpoint with a plain `GET` through a rolling deploy. A Lua script compares the stored version and writes both keys atomically, and a refused write throws `CheckpointWriteConditionNotFulfilledException` immediately rather than being retried, since a refusal can never succeed on a later attempt. The version key carries a hash tag matching whatever the checkpoint key itself hashes on, so Cluster places both keys in the same slot instead of refusing the script for crossing them, and it also carries a SHA-256 digest of the subscription id after that tag, so two ids sharing a hash tag still get their own version key rather than sharing a fencing version. `save` refuses a `notOlderThan` or `ifAbsent` condition outright for a subscription id Cluster reduces to hashing whole while it is empty or still carries a closing brace, which is what keeps `evaluatesWriteConditions()` true without exception. `any()` never refuses one, and `delete` falls back to two single-key deletes instead, provably safe since a conditional write already refused that id its version key before this fix could ever have written one. `read`, `save`, `delete`, and `exists` also refuse a subscription id that starts with the version key's own reserved prefix, since a caller-chosen id equal to another subscription's version key would otherwise let a write against it corrupt that other subscription's stored version, see the breaking change below for a subscription that already has such an id. Part of [#665](https://github.com/johanhaleby/occurrent/issues/665).
