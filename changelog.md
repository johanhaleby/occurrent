...
* `PushSubscriptionModel` takes an optional `PushObserver`, told a `RoutingOutcome` for an event `accept(...)` was asked to deliver, so a broker bridge can acknowledge a message safely instead of guessing.
...
* **`PushSubscriptionModel` and its reactor counterpart take an optional `PushObserver`, told a `RoutingOutcome` for an event `accept(...)` was asked to deliver.** `accept(...)` itself stays silent by design, since it cannot refuse an event that might also have come from the write path, so a misconfigured queue binding, a missing declared event type, or a type-mapping typo used to look identical to a saga or projection that chose not to react. Pass the observer through the same constructor that already takes a `DataFieldReader`. `PushObserver.noop()` is the default, so nothing changes for existing code, including its cost, since the match check is skipped entirely when none is configured. Resolves [#802](https://github.com/johanhaleby/occurrent/issues/802) and [#848](https://github.com/johanhaleby/occurrent/issues/848).
  * The six values are `DELIVERED` (a running, unpaused subscription's filter accepted the event), `FILTERED` (that same subscription evaluated it and declined it), `UNAVAILABLE` (no running, unpaused subscription for the event to reach at all), `NOT_DELIVERABLE` (a filter threw while being evaluated, so it neither accepted nor declined, and the outcome always arrives with that exception), `DEFERRED` (a `CatchupThenPushSubscriptionModel` in front has not reached live yet, safe to redeliver), and `REFUSED` (a registered action promises the refusal is permanent, which a catch-up-then-live engine does once its replay has failed).
...
* **`PushSubscriptionModel.acceptRedeliverable(CloudEvent)` returns the event's `RoutingOutcome`, and the reactor `PushSubscriptionModel` has a new `acceptRedeliverable(CloudEvent)` that returns a `Mono<RoutingOutcome>`, so a broker listener acknowledges on the outcome.** Acknowledge the message on `DELIVERED` or `FILTERED` and let the broker deliver it again on any other outcome. `accept(CloudEvent)` is unchanged, and it returns normally for an event fed before any subscription is registered, while the model is stopped, or while the subscription is paused, so a broker listener that acknowledged once it returned lost those events. Resolves [#1138](https://github.com/johanhaleby/occurrent/issues/1138).
  ...
  * A failed catch-up, a filter failure and a handler failure throw on the blocking stack and error on the reactor stack, the same way they do for `accept(..)`, and so does a full live buffer on the reactor stack.
  * The two stacks differ for an event whose earlier delivery is still running. The reactor model waits for that delivery and returns `DELIVERED`, and the blocking model returns `DEFERRED`.
  * `RabbitMqCloudEventBridge` and `KafkaCloudEventBridge` act on the returned outcome. When `acceptRedeliverable(..)` throws there is no returned outcome, so they still read the one their `RoutingOutcomeChannel` was told, which is how they tell a permanently failed catch-up, reported `REFUSED`, from a handler failure.
