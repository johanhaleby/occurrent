...
* Forwarding stored events to a broker at least once no longer needs a shipped transport. `CloudEventForwarder` holds its checkpoint back until your `CloudEventSink` confirms delivery, so an application with its own publisher wrapper gets that guarantee today, ahead of the RabbitMQ and Kafka modules landing on the same interfaces.
