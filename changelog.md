...
* **`PushSubscriptionModel` and its reactor counterpart take an optional `PushObserver`, told a `RoutingOutcome` for an event `accept(...)` was asked to deliver.** `accept(...)` itself stays silent by design, since it cannot refuse an event that might also have come from the write path, so a misconfigured queue binding, a missing declared event type, or a type-mapping typo used to look identical to a saga or projection that chose not to react. Pass the observer through the same constructor that already takes a `DataFieldReader`. `PushObserver.noop()` is the default, so nothing changes for existing code, including its cost, since the match check is skipped entirely when none is configured. Resolves [#802](https://github.com/johanhaleby/occurrent/issues/802) and [#848](https://github.com/johanhaleby/occurrent/issues/848).
  ...
  * Which throwable the caller then catches depends on whether the model already had a failure of its own to propagate, not on the outcome your observer was told. Told alongside a failure, a filter that threw, a handler that failed or an action that refused the event, your `Error` arrives attached to that failure through `addSuppressed` and the failure itself is what propagates. Told with nothing else in flight, it propagates on its own. `DELIVERED` reaches your observer both ways, since a handler that failed is reported delivered too.
  * On the reactor stack every report runs on whichever thread subscribed, which is the caller's thread only while nothing upstream moved the work off it. An `InterruptedException` your observer throws therefore sets the interrupt flag on a scheduler worker rather than on the caller's thread whenever a handler published elsewhere, and the caller may never see it.
