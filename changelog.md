...
* `InMemorySubscriptionModel` gained `waitUntilAllEventsProcessed()`, with a `Duration` overload, so a test can write events and then assert on the read model directly instead of polling until an assertion passes. It takes a timeout and reports whether it drained rather than throwing, because a handler that keeps failing is retried until its strategy gives up. In-memory only: a change stream never reaches a point where everything written has arrived, so a test against a real MongoDB still polls. Rationale in [ADR 77](doc/architecture/decisions/0077-deterministic-in-memory-subscription-drain.md). Resolves [#451](https://github.com/johanhaleby/occurrent/issues/451).
