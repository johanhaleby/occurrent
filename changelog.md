...
* Forwarding stored events to a broker at least once no longer needs a shipped transport. `CloudEventForwarder`, `CloudEventSink` and their domain-level counterparts are transport-neutral, so an application with its own publisher wrapper can use them today, ahead of the RabbitMQ and Kafka modules landing on the same interfaces.
...
* **`PushSubscriptionModel` and its reactor counterpart take an optional `PushObserver`, told about each event `accept(...)` delivers and whether a currently registered subscription matched it.** `accept(...)` itself stays silent on both counts by design, since it cannot refuse an event that might also have come from the write path (see ADR 104), so a misconfigured queue binding, a missing declared event type, or a type-mapping typo has looked identical to a saga or projection that simply chose not to react. Pass it through the same constructor that already takes a `DataFieldReader`, and `PushObserver.noop()` is the default, so nothing changes for existing code, including its cost. The match check the observer needs is skipped entirely when none is configured, and where it does run it shares the same evaluation `route(...)` dispatches from, so the two can never disagree. The observer runs before delivery is attempted, matched or not, independent of whether the handler that follows succeeds or throws. A `RuntimeException` or `AssertionError` the observer throws is always caught and logged rather than turning a delivered event into a broker redelivery, whether it was told the real outcome or a filter's own failure. Any other `Error` it throws propagates on its own when it was told the real outcome, or is attached to the filter's own exception rather than replacing it when it was being told about that failure instead. `accept(Iterable<CloudEvent>)` stops observing at the same event where an earlier handler's exception already stops routing. Resolves [#802](https://github.com/johanhaleby/occurrent/issues/802).
* **A new transport-neutral broker foundation, `occurrent-broker-api-blocking`, lets an application forward stored events to a broker at least once with its own publisher wrapper, ahead of a shipped transport.** `CloudEventForwarder` runs a durable subscription and hands each stored event to a `CloudEventSink` an application implements, advancing its checkpoint only once the sink returns, so a sink that throws leaves the checkpoint where it was and the event publishes again on the next run. `DomainEventForwarder` does the same for an application whose own converter already produces domain events, decoding each stored `CloudEvent` once and stamping its `EventMetadata` onto the domain event rather than converting twice. `EventDestination`, `DestinationResolver` and `DeliveryFailurePolicy` are the same module's shared shape the RabbitMQ and Kafka transports build on next. See [ADR 133](doc/architecture/decisions/0133-a-broker-is-a-transport-for-the-push-feed-and-never-a-subscription-model.md). Resolves [#413](https://github.com/johanhaleby/occurrent/issues/413).
