...
* Hardened snapshot resume against a stream reset. A snapshot whose version is ahead of the stream's true head (which happens when a stream is reset, or truncated, below a snapshot written against the longer stream) is now discarded instead of trusted: `resolveBase` and `isRedelivery` demote such a snapshot to the initial state and fold fresh, on both the DSL executors and the maintained `@Snapshot` path. The maintained `@Snapshot` stream registrar probes the true head only in the ambiguous case (the delivered version at or below the snapshot version), so the happy path pays no extra read, and a reset no longer freezes the maintainer, it rebuilds and self-heals. That probe is cached per stream and snapshot version, so a catch-up replaying many events below an existing snapshot's version probes the head once per version instead of once per event. The DSL stream executor deletes the stale snapshot with `SnapshotStore.delete` when it detects the base is ahead of the just-written head, so the next command folds fresh, and the `eventsSinceSnapshot` arithmetic now runs inside the best-effort save so nothing after a committed write can fail the command. DCB paths are unaffected because a global DCB position is monotonic and never resets. The released `ApplicationService.execute` API is unchanged. Rationale in the amendment to [ADR 61](doc/architecture/decisions/0061-first-class-snapshot-support.md).
