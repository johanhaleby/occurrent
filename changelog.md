...
* `Saga.Step` can now read the timer effects a transition produced, through `timerEffects()`, the complement of `issuedCommands()`. A timer effect needs no unwrapping, so you could already compare one against `effects()`, but not in isolation: a reaction that issues a command and arms a timer forces `contains(...)` on the mixed list, which cannot show that no other timer was touched. `timerEffects()` can, and together the two partition `effects()`.
