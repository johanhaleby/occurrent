...
* **A `@Projection` or `@Saga` with `source = PUSH` started through `ManualStartPushSources` while the application context is closing now stops what it started instead of letting it outlive the context.** A saga left its timer poller dispatching commands into a context that had already shut down, and a projection could start a catch-up replay against a store that was being disposed. Fixes behaviour shipped in 0.33.0. Resolves [#988](https://github.com/johanhaleby/occurrent/issues/988).
