...
* **A saga that declares a sealed event type now receives the concrete events stored under it, which changes behaviour the saga DSL shipped in 0.32.0.** `startsOn(OrderEvent.class)`, `evolve`/`react` on a sealed supertype, `step.on(OrderEvent.class, ...)` and `event(OrderEvent.class, 2)` all used to subscribe on the CloudEvent type of the supertype itself, which no stored event has, so the saga received nothing and looked like it was still waiting. A saga now also asks for every concrete type its sealed types permit. A declared event type that is never stored under its own name and whose subtypes cannot all be found, a plain interface or an abstract class that is not sealed, is refused when the saga is built instead, and the message asks you to declare the concrete event types. A flow saga that declares a sealed supertype and correlates only some of the types it permits now fails to build too, because it will receive the rest. See [ADR 124](doc/architecture/decisions/0124-a-saga-expands-a-declared-sealed-event-type.md) and [#743](https://github.com/johanhaleby/occurrent/issues/743).
