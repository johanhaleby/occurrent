...
* **`ReactorCheckpointStorage` now retries a transient MongoDB error while reading, saving or deleting a checkpoint, instead of letting it reach the subscription.** It retries with exponential backoff by default, 100 ms up to 2 seconds over 5 attempts, before rethrowing the original failure, the same default `SpringMongoCheckpointStorage` already uses on the blocking stack. Pass your own `reactor.util.retry.Retry` to the new three-argument constructor to change it. Resolves [#656](https://github.com/johanhaleby/occurrent/issues/656).
