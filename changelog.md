...
* **A framework-built materialized view now coalesces its store calls during a catch-up replay instead of paying one read and one write per event.** A history of N events over K projection keys used to cost 2N round trips through `MaterializedView.updateFromRepository`; it now costs about 2K, and about two per batch when the repository overrides the new bulk operations below. A view opts in by implementing the new `ReplayAwareMaterializedView` capability (a reactive twin lives in the reactor projection DSL, since the blocking view DSL carries no reactor dependency), which `BlockingHandover` and `ReactiveHandover` now call at the right points in a replay: `replayStarted()` before the first replayed event, `replayCompleted()` once every replayed event has been folded and before the catch-up marker is recorded, and `replayAbandoned()` when a replay is stopped or fails partway, so nothing buffered is ever written after a partial replay. `Projections.materializedView` (blocking) and `Projections.reactiveUpdateWithMetadata` (reactor) build views that implement it, on by default, sized by a `batchSize` on the new `MaterializedViewOptions` (1000 events buffered across every key before a batch flushes); pass a batch size of `1` to write through per event, the same as before this change. `ViewStateRepository` gains defaulted `findAllById` and `saveAll` methods in the same shape [ADR 76](doc/architecture/decisions/0076-batch-command-dispatch-seam.md) settled for `CommandDispatcher.dispatchAll`, so an existing repository, including one built from two lambdas, keeps working unchanged; an implementation that overrides `saveAll` for a real bulk round trip trades away the per-key optimistic-locking retry a configured `RetryStrategy` otherwise gets. A batch write is not atomic across keys: a failure partway through leaves some keys durable and some not, which is narrower than, not new relative to, the at-least-once replay contract the fold already had. Rationale in [ADR 110](doc/architecture/decisions/0110-a-replay-tells-the-view-where-it-begins-and-ends.md). Resolves [#638](https://github.com/johanhaleby/occurrent/issues/638).
* **The three Mongo-backed `ViewStateRepository` implementations the library ships now override `findAllById` and `saveAll` for a real bulk round trip instead of the looping defaults ADR 110 added.** That covers the `MongoOperations`- and `CrudRepository`-based repositories `SpringMongoViewExtensions` builds, and the zero-config store `MongoProjectionStoreProvider` builds. `findAllById` becomes one `_id in (..)` query. `saveAll` becomes as few bulk writes as `@Version`-aware optimistic locking allows, and `OptimisticLockingFailureException` and `DuplicateKeyException` still reach you the same way a single `save` throws them. Resolves [#643](https://github.com/johanhaleby/occurrent/issues/643).
...
* A composed `AND` filter with several `data` payload leaves no longer reparses a byte-backed event's payload once per leaf. `JacksonDataFieldReader` and `FilterMatcher` now read every leaf in one pass instead of one `read()` call each, roughly 18 times faster for a 20-leaf filter over a 256 KB payload. An already-decoded, Map-backed event, the production MongoDB path, is unaffected. Resolves [#623](https://github.com/johanhaleby/occurrent/issues/623).
* **A dotted path through an array of objects on a byte-backed event no longer picks up a same-named field nested inside a later sibling.** `JacksonDataFieldReader.read()` stopped scanning an array element's fields the moment it found the one it needed, so a later sibling field, if its own value held a nested field of the same name, got misread as a further array match once the array loop resumed. `read()` now finishes scanning each element's fields before moving to the next one, the same way `readAll` already did. Found by property tests written for the batching change above.
...
* **`SpringMongoSubscriptionModel.stop()`/`start()` and `NativeMongoSubscriptionModel.stop()`/`start()` snapshot the subscriptions they move instead of walking the live map, matching the reactor fix for [#509](https://github.com/johanhaleby/occurrent/issues/509).** Both iterated `runningSubscriptions`/`pausedSubscriptions` with `forEach` while `pauseSubscription`/`resumeSubscription`, called from inside that same loop, move each entry to the other map. `ConcurrentHashMap.forEach` leaves that self-mutation undefined: same-thread removal of the visited key is benign in practice, but the two blocking models now snapshot the keys first anyway, the same as `ReactorMongoSubscriptionModel` and `ReactorDurableSubscriptionModel` already did. Resolves [#590](https://github.com/johanhaleby/occurrent/issues/590).
* **`CommandDispatcher` gains a `ForwardingCommandDispatcher` base class for decorators.** `dispatchAll` is a default method, so a decorator written the natural way, overriding `dispatch` and delegating, silently turns a delegate's atomic `dispatchAll` back into one `dispatch` call per command, reintroducing the partial-progress hazard [ADR 76](doc/architecture/decisions/0076-batch-command-dispatch-seam.md) removed. Extending `ForwardingCommandDispatcher` instead forwards both methods, so overriding one leaves the other correctly delegated. The `dispatchAll` javadoc now names the hazard. Resolves [#590](https://github.com/johanhaleby/occurrent/issues/590).
* **`ReactiveHandover`'s live buffer no longer pre-allocates its full capacity at construction.** It backed the buffer with `ArrayBlockingQueue`, which allocates its whole backing array up front, roughly 800 KB per subscription for the 100k default, held for the handover's whole lifetime whether or not the live feed ever buffers anything. It now uses `LinkedBlockingQueue`, capped at the same `maxBufferedEvents` and rejecting past it the same way, but allocating one node per buffered item instead, so memory tracks actual use, matching what the blocking twin's `ArrayDeque` already did. Resolves [#590](https://github.com/johanhaleby/occurrent/issues/590).
* Added a TCK, a set of abstract conformance suites that an implementation of an Occurrent contract runs against itself, so verifying a new event store is a few lines of test code instead of a few thousand. It ships as five published artifacts, `occurrent-tck-common`, `occurrent-tck-eventstore-blocking`, `occurrent-tck-eventstore-reactor`, `occurrent-tck-subscription-blocking` and `occurrent-tck-subscription-reactor`, so an implementation living outside this repository can depend on them. You supply one `EventStoreFixture`, which hands back the store under test and declares the `EventStoreCapability` values it was built with, then extend one suite per capability. Twelve suites ship. `StreamEventStoreConformance` covers reading and writing a stream, paging a read, stream existence, `WriteResult`, duplicate detection, reading through a `StreamReadFilter`, and every `WriteCondition` operator with its exact failure message. `EventStoreQueriesConformance` covers querying across streams, filtering on any CloudEvent attribute, sorting and paging. `EventStoreOperationsConformance` covers deleting a stream, deleting a single event, deleting by filter, and updating an event. `EventStoreTimePrecisionConformance` covers how precisely a store keeps the `time` attribute, and pins that a store refuses a value it cannot represent rather than rounding it off, since a truncated timestamp cannot be told apart afterwards from an accurate one. `StreamConcurrencyConformance` covers writing to one stream from several threads at once: that a conditional write leaves exactly one winner with the loser writing nothing, and that an unconditional write never fails on a version race. `StreamPositionConformance` covers the global position: that it is positive, unique and strictly increasing, that `currentPosition()` is 0 on an empty store, and that a position-ordered read honours its range bounds, a filter and the current high-watermark. It asserts no literal position value anywhere, because a position is deliberately not dense. `StreamPositionDisabledConformance` covers a store built with its position turned off, which writes no position and refuses the position APIs with a message the suite pins. `DcbEventStoreConformance` covers the Dynamic Consistency Boundary contract: which events a criteria selects, whether an excluded type, a tag or a type decides that, what a position window and a direction, skip and limit do to the selection, `exists` and `count`, the position block an append reports, and when an append condition conflicts with and without a consistency token. `DcbStreamInteropConformance` covers what a store owes when it has both capabilities at once, so it requires both: that a DCB read never returns a stream-written event, that a stream write refuses an event carrying DCB tags, that one global position sequence covers both modes, and that a condition with no token asks whether a matching event exists now rather than whether one was ever appended. `CapabilityGuardConformance` covers what a store does when asked for a capability it was not built with, in both directions: every stream call on a DCB-only store and every DCB call on a stream-only store must refuse with an `UnsupportedOperationException` naming the capability, rather than answering emptily, since no events is indistinguishable from an empty store and a decider would then append against a boundary it never checked. Each direction also asserts the restricted store still serves the capability it does have, so a fixture handing back a dead store cannot pass. The `EventStoreFixture` accessors that hand back these restricted stores are `eventStoreWithoutStream()`, `eventStoreWithoutDcb()` and `eventStoreWithoutPosition()`, matching the `EventStore` vocabulary used everywhere else in the fixture. `ReactiveEventStoreConformance` covers the part of a reactive store's contract that blocking on a result destroys, which is why it is the one suite written in terms of `Mono` and `Flux`: that nothing is written or deleted until something subscribes, that a failure reaches the subscriber rather than being thrown while the publisher is assembled, that a `Mono` documented to always emit never completes empty, and that cancelling a read part way through completes and leaves the store readable. `DcbConcurrencyConformance` covers two writers deciding against the same consistency boundary at once, which is what an append condition exists for: same-boundary contention, boundaries sharing no tag that must not be serialised against each other, two shapes of write skew where each writer appends an event the other's boundary matches, the create guard that carries no token, and appends with no condition at all, which have nothing to fail on. The queries suite also covers `Filter.data` and what natural order guarantees, and the time suite covers where a time-range filter's boundaries fall, all three as fixture-declared variations. `supportsDataFilter()` says whether a store can reach into a payload, which every store shipping with Occurrent now can, and an in-memory store built without a `DataFieldReader` cannot. `naturalOrderIsInsertionOrder()` says whether natural order is insertion order, which only the in-memory store promises. `appendConditionModel()` says how a store decides whether a token-qualified DCB append condition was violated, by comparing the events committed since the token against the criteria, which is what the in-memory store does, or by comparing version markers kept per consistency boundary, which is what the three MongoDB stores do so the check is one conditional write rather than a scan. That one answer settles two observable differences at once, in opposite directions: the marker model conflicts on an excluded type that shares a boundary tag, a false conflict the caller retries out of, and a whole-store lock does not notice a later tag-scoped append, which is the limitation `wholeStoreLock()` already documents. Both branches run here, so neither is a claim nobody checks. Occurrent's own four event stores run all of them, which is the first time the build proves they agree. A reactive store runs the same suites through `BlockingEventStoreOverReactive`, a test-only bridge presenting a reactive store as a blocking one. The suites never skip a test, so an implementation declines a capability by not extending that suite, which is visible in the code. Two things to know before depending on them. The suites are ordinary classes under `src/main`, so JUnit and AssertJ arrive at compile scope rather than test scope. And where the four stores genuinely disagree, the fixture declares which way a store goes and the suite asserts the documented outcome for that answer, rather than the assertion being loosened until everyone passes. Running every store against one suite also found real bugs, fixed and listed separately below. The subscription side has started arriving in the same shape, beginning with `CheckpointStorageConformance` in `occurrent-tck-subscription-blocking`. You supply a `CheckpointStorageFixture`, which hands back a storage holding no checkpoints and declares one thing the API cannot be asked: whether `read` gives back a checkpoint of the same type `save` was handed. The suite covers round-tripping every checkpoint type through `save`, `read`, `exists` and `delete`, that `exists` and `read` never disagree, that saving twice replaces rather than accumulates, that two subscriptions stay apart, and that a deleted checkpoint leaves no tombstone refusing a later save. It asserts the type declaration in both directions, so a storage claiming it rebuilds a type has to, and one claiming it does not must not, which pins the real difference between them: the MongoDB storages recognise their own resume-token and operation-time checkpoints and rebuild those, while the Redis storage keeps the string a checkpoint reports and hands back a `StringBasedCheckpoint` for everything, including the MongoDB types a change stream feeds it. All four of Occurrent's blocking checkpoint storages run it. `SubscriptionModelConformance` covers what every `SubscriptionModel` owes: that a running subscription receives every published event in publish order, that a filter decides which ones, that a duplicate subscription id and an unrecognized filter are refused, the whole of `SubscriptionModelLifeCycle` including the promise that `stop()` leaves every running subscription paused and individually resumable, and that cancelling a subscription stops delivery and frees its id. `IntrospectableSubscriptionModelConformance` covers `subscriptionIds()`, which both MongoDB models implement and neither had a test for. `InProcessDeliveryConformance` covers the stronger promise the synchronous and push models make, that the handler has already run when the publishing call returns, which is the whole reason to choose one of them. `CheckpointAwareSubscriptionModelConformance` covers the position a catch-up subscription hands over from: that a position read before a write, started from with `StartAt.checkpoint(..)`, gives you a subscription that receives that write. That is what the end of a replay depends on, and a model reporting a position you cannot start from loses every event written while history replayed, which neither MongoDB model had a test for. All five blocking subscription models run the suites. `CompetingConsumerStrategyConformance` covers `CompetingConsumerStrategy`, which had no test of any kind before this. You supply a `CompetingConsumerStrategyFixture`, which hands back the strategy under test, a factory that builds more strategies contending over the same storage, and one number the API cannot be asked, which is how long a strategy may take to work out on its own who holds a lock. The suite covers that one subscriber holds a subscription at a time while two different subscriptions never compete, that `hasLock` agrees with what registering just returned, that a listener is told when a lock is granted and when it is taken away and is told each change once rather than once per round of coordination, and that a consumer which never adds a listener finds out by asking, which is what `SagaRunner` does. It covers the difference between unregistering and releasing, which the two methods' identical javadoc never mentioned. And it covers the guarantee the whole pattern exists for, that a consumer which stops coordinating with the others without releasing or unregistering loses the lock to a rival, instead of holding a subscription that nobody consumes ever again. No test in the suite mentions a lease. Occurrent's two strategies keep one in MongoDB, but a strategy built on a leader election owes every one of those guarantees with no lease to expire, so when a MongoDB lease is up and what refreshing one does are covered separately, deterministically, against a clock the test moves itself. Both MongoDB strategies run the suite. The reactor subscription models run the same suites through `BlockingSubscriptionOverReactive`, a test-only bridge in `occurrent-tck-subscription-reactor` presenting a reactive subscription model as a blocking one, alongside a small `ReactiveSubscriptionModelConformance` covering what blocking on a result destroys: that the action's `Mono` does its work when it is subscribed to rather than when it is assembled, that a failing action fails through the model and leaves it running, and that `waitUntilStarted()` answers and answers again. `RestartConformance` covers what a subscription owes when it outlives the model that was running it, which is where at-least-once delivery lives: an event published while nothing was running is either still delivered to that subscription afterwards or it is gone, and which of the two is a promise the model makes rather than something you find out in production. A model that keeps a checkpoint owes the first, and `DurableSubscriptionModel` and `CatchupSubscriptionModel` are held to it here for the first time. A model without one owes the second, so a bare change-stream model has to start at the present rather than quietly keep a position it does not admit to. Redelivery is allowed on either answer, since resuming from the last position that was stored rather than from just after it hands one event over twice. A model that cannot be rebuilt over state it left behind, which is every model fed by being handed events, declines the suite by not extending it. Five things are declared by the fixture rather than asked, because nothing on the API reports them: whether a paused subscription's events are held or dropped, whether a throwing handler is retried or the exception reaches the publisher, whether the model accepts more than one subscription, which of the four ways of saying where a subscription starts the model accepts, and whether a subscription id it has not seen before is replayed the whole history or starts where it was told. Both answers to each are asserted. The start-position one is where a model's deliberate restrictions get written down instead of rediscovered: every variant it accepts owes a subscription that receives what is published next, and every variant it leaves out owes a refusal from `subscribe`, so `CatchupThenPushSubscriptionModel` saying it takes only the model default, and the in-memory model saying it cannot start from a checkpoint because it keeps no history, are claims each of them is held to rather than gaps. As a side effect `SynchronousSubscriptionModel` and `PushSubscriptionModel` are now `SubscriptionModel`s: they always had every method the interface requires without declaring it, so they can now be passed anywhere a `SubscriptionModel` is expected, including `ManualStartSubscriptionModel.stoppedByDefault` and `StreamSubscriptionModel.from`. `NativeMongoSubscriptionModel` also now refuses a subscription filter it cannot apply when you subscribe, instead of failing later on a background thread and leaving you holding a subscription that never delivered. **Upgrading the TCK to a new minor release can turn a build that was green red**, because a minor may add suites and tighten what the existing ones assert. Fix the implementation, or stay on the Occurrent version you were on, since each TCK artifact is compiled against the runtime API of its own version, so holding the TCK back on its own does not work. Your fixture keeps compiling either way, because a new fixture member always arrives with a `default`. There are no `@since` tags and no way to turn off one group of tests, so declining a contract stays a matter of not extending its suite. You now also say how long the subscription suites may wait for something to arrive. `SubscriptionModelFixture.deliveryTimeout()` defaults to ten seconds and `ReactiveSubscriptionModelFixture.deliveryTimeout()` to twenty, so a model that has to reach a broker before it can deliver widens the wait rather than having no way to pass. Neither is capped, and if you raise one past a few seconds you put a matching `@Timeout` on your own test class, which JUnit uses instead of the suite's. The blocking artifact also gained a `package-info` covering which suite to extend for which capability, every declaration a fixture makes with what each answer obliges, and what a version of the TCK promises. Part of [#393](https://github.com/johanhaleby/occurrent/issues/393), and it resolves the older [#75](https://github.com/johanhaleby/occurrent/issues/75). Rationale in [ADR 77](doc/architecture/decisions/0077-a-published-tck-for-occurrent-contracts.md), in [ADR 88](doc/architecture/decisions/0088-what-a-dcb-append-condition-guarantees.md) for the DCB half, in [ADR 93](doc/architecture/decisions/0093-a-missing-capability-is-refused-and-a-reactive-publisher-is-cold.md) for the capability guards and the reactive contract, and in [ADR 94](doc/architecture/decisions/0094-the-subscription-tck-declares-three-differences-and-waits-deterministically.md) for the subscription side, and in [ADR 107](doc/architecture/decisions/0107-what-a-tck-version-promises.md) for what a version of it promises.
* A subscription can now filter on a field inside an event's `data` payload, on every subscription model Occurrent ships. Before this, `Filter.data("amount", eq(42))` on a subscription threw on the first event delivered rather than filtering, which hit a reactive stream subscription on MongoDB (the store matched the event server-side, then the catch-up model re-checked it in memory and refused) and any synchronous or push subscription (those match entirely in memory). A synchronous, push, or in-memory model reads the payload through a `DataFieldReader` you supply, so on Spring Boot add `occurrent-common-inmemory-filter-matching-jackson` and the starter contributes one, or define your own `DataFieldReader` bean. Built without one, such a model refuses a payload filter at subscribe time with an `UnsupportedOperationException` naming the artifact to add, the same way anything refuses a capability it was not built with, rather than accepting the subscription and only failing once an event needing the reader arrives. A catch-up subscription model needs nothing: it trusts the store for a payload condition, since the store applied it to have delivered the event, and keeps checking attributes and extensions itself, through the new `FilterMatcher.matcherIgnoringPayloadConditions(filter)`. The one behaviour change worth knowing is what that trust means for a subscription model you wrote yourself: if it ignores a payload filter rather than applying it, a catch-up wrapper over it now over-delivers instead of throwing. `InMemorySubscriptionModel` also gains a constructor taking just a reader, rather than only the four-argument form. Resolves [#499](https://github.com/johanhaleby/occurrent/issues/499) and [#582](https://github.com/johanhaleby/occurrent/issues/582). Rationale in [ADR 92](doc/architecture/decisions/0092-a-subscription-can-filter-on-a-payload-field.md).
* `@Saga` can now be fed by a push subscription model instead of the event store, with `source = PUSH`, so a saga can react to events arriving from RabbitMQ, Kafka or an HTTP listener without you wiring a `SagaRunner` by hand. It mirrors `@Projection(source = PUSH)`: `subscriptionModel` or `subscriptionModelName` picks the feed bean, and the saga is wrapped in a replay-then-live catch-up so one that has never run works through the event store's history before it starts reacting. Two things differ from a push projection, both on purpose. Only a `PushSubscriptionModel` is accepted, not a `DomainEventFeed`, because a saga recognises a redelivered event by its `streamid`/`streamversion` or `position` extension and a domain-event feed carries neither, so binding one would quietly cost the saga its redelivery protection under the at-least-once delivery a broker gives you. And `catchup = NONE` opts out of the replay entirely, taking live events only and touching no event store, which is what a saga fed by another application's broker needs: those events are not in this application's store, so a replay would find nothing, or apply unrelated events that happen to live there. Under the default, a missing `PositionOrderedReader` or `CheckpointStorage` now names `catchup = NONE` rather than surfacing as a bare `NoSuchBeanDefinitionException`. `startAt`, `startAtGlobalPosition` and `resumeBehavior` are refused on a push saga rather than silently ignored, since a catch-up always replays from the beginning and where a live feed resumes is the broker's business. `startupMode` is supported under the default `catchup`, so `BACKGROUND` keeps that replay off the startup path, and is refused with `catchup = NONE` where there is no replay to wait for. Setting `catchup` on an event-store saga is refused too, because ignoring it would leave the saga reading the whole history it was asked to skip. `occurrent.subscription.mode = manual` withholds a push saga the same way it withholds a push projection, which matters most for a saga behind a leader election, though its instances stay observable while it waits, since reading them needs the state store and not a running subscription. `ReplayAwareSubscriptionModel` came out of this and is useful on its own. It is a capability interface on both stacks with one method, `isCatchingUp(id)`, reporting whether a subscription is still replaying history or has handed over to live delivery, which `isRunning(id)` cannot answer because it is true throughout the replay. Both `CatchupThenPushSubscriptionModel`s implement it, as do the event-store catch-up models (`CatchupSubscriptionModel`, `StreamCatchupSubscriptionModel` and the three reactor ones), so the question can be asked of any model that replays rather than only the push one. On the blocking stack `ReplayAwareSubscriptionModel.of(..)` unwraps a `DelegatingSubscriptionModel` chain to find it, the same lookup `IntrospectableSubscriptionModel` already offers, so `DurableSubscriptionModel(CatchupSubscriptionModel(..))` answers. A saga's timers now wait for that handover, so a timeout cannot decide against state that has only been replayed halfway. A replay is stopped when the application context closes, whether it runs on the startup path or in the background, so a saga that is catching up does not keep folding history, and issuing the commands that come out of it, into an event store that is closing underneath it. Resolves [#349](https://github.com/johanhaleby/occurrent/issues/349). Rationale in [ADR 96](doc/architecture/decisions/0096-a-push-fed-saga-may-have-no-history-to-replay.md).
* `occurrent.subscription.mode` decides whether Occurrent starts your subscriptions for you. `auto` is the default and behaves as before. `manual` creates every subscription bean and registers every annotated subscription, then waits: nothing is delivered, no change stream is opened, no history is replayed and no competing consumer lock is taken until you call `resumeSubscription("someId")` or `start()` on the subscription model. Use it to bring subscriptions up behind a leader election or a health check, or in a test that picks which ones run. `disabled` creates no subscription beans at all, which is what `occurrent.subscription.enabled=false` used to do. A subscription waiting to be started does not miss what was written while it waited, because its position is pinned when it is registered rather than when it is started. Calling `waitUntilStarted` on what `subscribe(..)` hands you answers `false` while the subscription is still waiting, since it has not started, and `resumeSubscription(id)` gives you the started one to wait on instead. A synchronous projection is the exception, since a stopped one drops events rather than holding them, so a write succeeds while its projection does not run ([ADR 85](doc/architecture/decisions/0085-every-subscription-model-can-be-stopped.md)). A saga's timers also wait for its subscription to be started, which additionally stops a paused saga firing timers in a test. `occurrent.subscription.enabled` still works, is deprecated, and fails at startup if it contradicts `mode`. The `org.occurrent.UpgradeToOccurrent_0_32` OpenRewrite recipe rewrites it for you, in `.properties` and `.yaml` alike, mapping `false` to `disabled` and `true` to `auto`. It leaves alone what it cannot safely rewrite, a value like `${SUBSCRIPTIONS_ON}` for example, and it cannot see an environment variable at all, which is why setting both properties is tolerated while they agree. The [upgrade guide](doc/migration/upgrading-to-0.32.0.md) lists what is left for you. A `@Projection(source = PUSH)` or `@Saga(source = PUSH)` is fed by a bean you supply rather than by Occurrent's subscription model, so `manual` withholds those through `ManualStartPushSources`, which you inject and call `start("someId")` or `startAll()` on. One registry covers both, so bringing every push source up behind a leader election is one `startAll()`. `DomainEventFeed.catchUp(id)` catches up a single projection on a feed, for when one is registered long after the others already went live. `ManualStartSubscriptionModel` in `occurrent-subscription-api-blocking` gives you the same thing without Spring. The reactive stack has all of this too, with the same property and the same reactor `ManualStartPushSources`, except that its `start(id)` and `startAll()` return a `Mono` you subscribe to and there is no `startAll()` on the subscription model itself, so a reactive application resumes subscriptions by id. Combining `@EnableOccurrent` and `@EnableOccurrentReactive` on one classpath, which [ADR 44](doc/architecture/decisions/0044-reactive-spring-boot-starter.md) says should work, now actually does: each stack's `ManualStartPushSources` bean gets its own name instead of colliding with the other stack's on context refresh. [ADR 86](doc/architecture/decisions/0086-a-manual-subscription-is-registered-not-started.md) explains why registering and starting had to become separate events, and [ADR 89](doc/architecture/decisions/0089-manual-subscription-mode-on-the-reactive-stack.md) why the reactive stack needs no wrapper to do it. Resolves [#481](https://github.com/johanhaleby/occurrent/issues/481).
* A new `occurrent-testing-junit-jupiter-blocking` artifact, and a reactive twin, `occurrent-testing-junit-jupiter-reactor`, give you a JUnit 5 extension that keeps every subscription stopped while a test runs, so a test only runs the subscriptions it names. `OccurrentSubscriptionsExtension.stoppedByDefault(subscriptionModel)` stops them before and after each test, `start("someId")` resumes one and blocks until it has actually started so you cannot forget to wait, `alwaysStart("someId")` resumes a fixed set for a whole test class, and `startAll()` starts every subscription the model has, for the one test that needs to see them working together. Naming an id that does not exist fails with the ids that do. The wait for a subscription to start, whether through `start`, `alwaysStart` or `startAll`, is bounded rather than unbounded on the blocking side or fixed at ten seconds on the reactive side, both of which have the same failure mode: a subscription that never starts hangs the whole test run instead of failing the one test that asked for it. Both stacks default to 30 seconds and widen it with `withStartTimeout(Duration)`, and a subscription that has not started by then fails with an `IllegalStateException` naming the id. Pass more than one model, `stoppedByDefault(first, second)`, when your application has more than one that needs stopping, for example a durable model alongside a `SynchronousSubscriptionModel`. Each leaf depends on nothing but JUnit and its own subscription API, so it works without Spring and without a container. The point of the default is that adding a subscription to your application can no longer change what an existing test asserts. `@EnableOccurrentTesting` wires whichever leaf, or leaves, you added as a test dependency, so an application on both stacks gets both extensions. [ADR 82](doc/architecture/decisions/0082-a-published-testing-module-for-application-authors.md) explains why this is published to Maven Central while `test-support` stays internal, and [ADR 99](doc/architecture/decisions/0099-a-reactive-testing-twin-that-stops-every-model.md) explains the reactive twin and why stopping every model is the right default. Resolves [#530](https://github.com/johanhaleby/occurrent/issues/530).
* A new `occurrent-testing-mongodb` artifact empties a MongoDB database between tests without breaking your subscriptions. `OccurrentMongoFlush.everyCollectionIn(database)` deletes the documents and leaves the collections and their indexes alone, which matters for two reasons. Dropping a collection or a database invalidates a live MongoDB change stream, so every subscription resumed afterwards receives nothing, and stopping the subscription model first does not save you, because it resumes from a position pointing into a collection that no longer exists. The quieter reason is that an Occurrent MongoDB event store creates its unique indexes in its constructor, so a drop removes them and only a new store brings them back, which a cached Spring test context never does. Your optimistic-concurrency and duplicate-detection tests then keep passing with no index behind them. It names no collections on purpose, since Occurrent writes to more of them than you are likely to remember: the events, a stream position collection, a DCB checkpoint collection, the subscription checkpoint collection and a competing consumer lock collection. Views and `system.*` are skipped, `collectionsIn(database, ..)` narrows it to the collections you name, and `except(..)` keeps one, except under `droppingTheDatabaseIn(..)`, where it throws instead of compiling and being ignored: a dropped database takes every collection with it, so there is nothing left to except. A flush that fails now throws instead of leaving the previous test's data in place, and so does one where `except(..)` excludes every collection `collectionsIn(..)` named, rather than silently flushing nothing. `droppingTheDatabaseIn(database)` is there for the one case deleting cannot serve, a test asserting that a collection or an index does not exist, and it says in as many words when not to reach for it.
* A `@Projection(source = PUSH)` can now keep its catch-up off the startup path with `startupMode = BACKGROUND`, on both the blocking and reactive stacks, whether it is fed by a `PushSubscriptionModel` or a `DomainEventFeed`. A push catch-up replays the whole event store before it hands over to the live feed, and that replay used to run on the thread that starts your application, so an application with a large history was held out of service until it finished. It now runs on a thread of its own, and the application starts while the read model is still filling. The default is unchanged: without `startupMode` the replay still completes before startup does. `startAt`, `startAtGlobalPosition` and `resumeBehavior` are still rejected for a push source, because the catch-up always replays from the beginning and where the live feed resumes is the broker's business. Because nobody waits for a background replay, neither its progress nor its failure has anywhere to be returned or thrown, so both starters contribute a `PushCatchupStatus` bean you can inject and check from a health indicator or a readiness probe. It answers per id with one of five states, `CatchingUp`, `Live`, `NotStarted`, `Failed` carrying the cause, and `Unknown`, so a probe can tell a read model that is still filling from one that is ready to serve. A failures-only view would be empty in both of those cases. Where there is a subscription model to ask, those states are derived from it rather than recorded, so a model that is stopped and started again, replaying its history a second time, reports `CatchingUp` again. `NotStarted` is what `occurrent.subscription.mode = manual` leaves a projection as until you start it, which is why a projection that replays nothing is not reported live until it is actually running. A saga fed by `@Saga(source = PUSH)` is covered too. A failure is still logged at `ERROR` either way. `PushCatchupStatus` is read-only, `of`, `isCaughtUp` and `all`, and the `@Projection` and `@Saga` registrars write to it through `PushCatchupStatusImpl`, the same split `SagaInstancesRegistry` uses against `SagaInstancesRegistryImpl`. Registering the same id twice, which only a context running both the blocking and reactive stacks together against one shared bean can reach, is refused rather than silently replacing the first registration. [ADR 91](doc/architecture/decisions/0091-a-push-catch-up-replays-off-the-startup-path.md) explains the design, including why stopping a replay is the right verb here when [ADR 86](doc/architecture/decisions/0086-a-manual-subscription-is-registered-not-started.md) says it is the wrong one for withholding.
* **`SpringMongoEventStore.readInPositionOrder` no longer loads the whole matched history into memory before delivering the first event.** It called `MongoOperations.find(...).stream()`, which decodes the entire query result into a `List` before the returned stream produces anything, and this read applies no limit by default. A from-the-beginning replay, which is what both push catch-up models issue, therefore held the whole event history as decoded documents at once and delayed the first replayed event until the whole read had completed, defeating what `startupMode = BACKGROUND` exists to avoid. It now reads through `MongoOperations.stream(...)`, a live server cursor consumed one batch at a time, matching the native, reactor and in-memory stores, which were already lazy here. The returned `Stream` now must be closed (`PositionOrderedReader.readInPositionOrder`, and its `DomainEventQueries` DSL wrapper, document this); both push catch-up callers already close it via try-with-resources. Resolves [#586](https://github.com/johanhaleby/occurrent/issues/586).
* **A conformance-suite failure now names the test class that ran it.** Every suite in the four TCK modules keeps its tests in `@Nested` classes, so an implementation runs them under the suite's own class name rather than under its own. Two implementations in one module therefore report the same test twice, and when one fails and the other passes the run report shows `Run 1` and `Run 2` for what looks like a single test, which is also what a retried flaky test looks like. An assertion failure out of a suite now starts with `Run by <YourConformanceTest>.` and keeps its description, its expected and actual values, and the line it failed on. Resolves [#575](https://github.com/johanhaleby/occurrent/issues/575).
* **`subscriptionIds()` on the blocking MongoDB subscription models can no longer omit a subscription that is mid-pause or mid-shutdown.** `SpringMongoSubscriptionModel` and `NativeMongoSubscriptionModel` move a subscription between a running map and a paused map in two steps, and `subscriptionIds()` reads both without taking the same lock as `pauseSubscription`, `resumeSubscription` and `shutdown()`, so a reader could land between the remove and the put and miss an id that exists, or see stale ids for an instant after `shutdown()` had already flagged itself shut down but not yet cleared the maps. `subscriptionIds()` is now `synchronized`, which closes both windows because the three mutators already are, matching what `ReactorMongoSubscriptionModel` and `ReactorDurableSubscriptionModel` do. `RegisteringSubscribable` (blocking and reactor, backing `PushSubscriptionModel` and `SynchronousSubscriptionModel`) had the milder version of the same gap between `subscriptionIds()` and `hasSubscriptions()`, and both now take the registration lock its own mutators already hold. Resolves [#536](https://github.com/johanhaleby/occurrent/issues/536).
...
* **A paused MongoDB subscription no longer loses what was written while it was paused.** `SpringMongoSubscriptionModel` rebuilt its change stream from the `StartAt` the subscription was created with, and for the default that resolves to the present all over again, so anything written during a pause was behind where the resumed stream started and never arrived. It now tracks the change-stream position it has read to and resumes from there, which is what `NativeMongoSubscriptionModel` and `ReactorMongoSubscriptionModel` already did. A subscription restarting after a change-stream error uses that position too, where it also used to reconnect at the present; a subscription whose change stream history is gone still restarts at the present, and still only if you asked for that with `restartSubscriptionsOnChangeStreamHistoryLost`. **The consequence to plan for is redelivery.** Resuming this way is at least once, so handlers have to cope with the same event arriving twice. An event whose handler had not finished when the subscription was paused is handed over again, and so is every event another consumer of the same subscription id handled while this one was paused. `stop()` on the model pauses every subscription it holds, so `stop()` followed by `start()` is the same case. That is the direction to err in, since repeating work you have already done is cheaper than never doing it, and it is what [ADR 57](doc/architecture/decisions/0057-synchronous-subscriptions.md) settled for the same trade. Found by the subscription TCK, where both MongoDB models now answer `SubscriptionModelFixture.deliversEventsPublishedWhilePaused()` the same way. Resolves [#522](https://github.com/johanhaleby/occurrent/issues/522). Reasoning in [ADR 94](doc/architecture/decisions/0094-the-subscription-tck-declares-three-differences-and-waits-deterministically.md).
* **`ReactorMongoSubscriptionModel.stop()` and `start()` snapshot the subscriptions they move instead of walking the live map.** Both iterated a `ConcurrentHashMap` with `forEach` while the very calls made inside it, `pauseSubscription`/`resumeSubscription`, move each entry to the other map, which is exactly the self-mutation `ConcurrentHashMap.forEach` leaves undefined: it may visit an entry that has already moved, or miss one that has not, silently leaving a subscription running after `stop()` or paused after `start()`. `ReactorDurableSubscriptionModel` already sidestepped this by snapshotting the keys before iterating; `ReactorMongoSubscriptionModel` now does the same. The subscription TCK gained `stop_pauses_and_start_resumes_every_one_of_several_running_subscriptions`, pinning that `stop()` leaves every running subscription paused and `start()` resumes every one of them, on every wired model, rather than only the one or two subscriptions the rest of the suite happens to exercise. Resolves [#509](https://github.com/johanhaleby/occurrent/issues/509).
* **`ReactorMongoSubscriptionModel` refuses an unsupported `SubscriptionFilter` when `subscribe(..)` is called.** It used to accept the filter and fail later inside the deferred change-stream pipeline, where nobody was listening and the restart logic re-threw it on every attempt. It now throws the `IllegalArgumentException` from `subscribe(..)` itself, which is what `SpringMongoSubscriptionModel` always did and what `NativeMongoSubscriptionModel` was brought in line with earlier in this release. The plain `FluxSubscriptionModel` primitive still delivers its failure through the `Flux`, as a cold publisher should.
* **A reactor MongoDB subscription now retries a failing action instead of dying of it.** A named subscription on `ReactorMongoSubscriptionModel` whose action threw, or whose `Mono` errored, terminated that subscription silently: the retry only guarded the change stream underneath, so one bad delivery ended the subscription for good. The action is now retried with the model's configured backoff, the reactor counterpart of the blocking models' `RetryStrategy` around the handler, and the subscription keeps its id and keeps running. Two consequences worth knowing: a non-fatal `Error` such as `AssertionError` is now retried rather than terminating the subscription (fatal errors like `OutOfMemoryError` still propagate), and a handler that fails deterministically now keeps its subscription alive and retrying with a `WARN` per attempt, where it previously died silently. Found by running the subscription TCK suites over the reactor stack.
* **A durable reactor subscription hands the subscription to the model it wraps, when that model manages named subscriptions itself.** `ReactorDurableSubscriptionModel` used to read from the wrapped model's cold `Flux` and run the delivery itself, so an unsupported `SubscriptionFilter` was only reported once the subscription started, and an action that failed ended the subscription rather than being retried. Wrapping `ReactorMongoSubscriptionModel` directly now inherits both of those from it, which means the retry is configured in one place, on the model doing the retrying. That covers the reactive Spring Boot starter's own wiring for a store that writes no `position`, the `mongodb-subscription-to-spring-event` example, and anyone composing the two models by hand. The three reactor catch-up models (`ReactorCatchupSubscriptionModel`, `ReactorStreamCatchupSubscriptionModel`, `ReactorDcbCatchupSubscriptionModel`) now manage named subscriptions themselves, so the starter's primary `Durable(Catchup(Mongo))` composition inherits the retry and the filter validation the same way: the catch-up model replays history and then hands the live subscription to the model it wraps. Their cold `Flux` primitive is unchanged. One composition changes behavior loudly rather than silently: a durable model wrapping a catch-up model whose own wrapped model offers only the cold primitive used to run through the durable model's unguarded delivery, and now refuses with an `IllegalStateException` naming the remediation, because the one place that delivery loop still exists is the durable model itself and it only runs when there is nothing underneath to delegate to. The migration guide covers it. Resolves [#547](https://github.com/johanhaleby/occurrent/issues/547) and [#550](https://github.com/johanhaleby/occurrent/issues/550).
* **A subscription started through `ReactorCatchupSubscriptionModel` now reports that model's own type to a `StartAt.dynamic(..)`.** It used to report whichever of the three mode-specific catch-up models it routed the subscription to, none of which a caller normally holds, so a start position written as `context.hasSubscriptionModelType(ReactorCatchupSubscriptionModel.class)` never matched and the subscription started from the other branch instead. The blocking `CatchupSubscriptionModel` has always reported its own type here. Both the cold `Flux` primitive and the named `subscribe(..)` are covered, and a `ReactorStreamCatchupSubscriptionModel` wired on its own still reports itself. Resolves [#557](https://github.com/johanhaleby/occurrent/issues/557). Reasoning in [ADR 103](doc/architecture/decisions/0103-a-catch-up-dispatcher-answers-as-the-model-the-caller-holds.md).
* `@Projection(source = PUSH)` can now set `catchup = NONE`, the same opt-out `@Saga(source = PUSH)` already has. It takes live events only and touches no event store, which is what a projection fed by another application's broker needs, since the local event store holds none of those events. With a `PushSubscriptionModel` feed this skips the catch-up wrapper entirely. With a `DomainEventFeed` it registers and calls `goLive(id)` instead of `catchUp(id)`, since `register()` alone buffers every live event until something tells the feed to deliver them. Under the default `catchup`, a missing `PositionOrderedReader` or `CheckpointStorage` bean now names `catchup = NONE` as the fix instead of a bare missing-bean error. `startAt`, `startAtGlobalPosition` and `resumeBehavior` are still rejected under `source = PUSH` regardless of `catchup`. `startupMode` is honoured only under the default `catchup`, since there is no replay for it to move off the startup path with `NONE`. Setting `catchup` on a `source = EVENT_STORE` projection is rejected too, since ignoring it would leave the projection reading history it was asked to skip. Resolves [#528](https://github.com/johanhaleby/occurrent/issues/528). Rationale in [ADR 100](doc/architecture/decisions/0100-a-push-fed-projection-may-also-have-no-history-to-replay.md).
...
* **A released competing consumer no longer reports that it still holds the lock.** `SpringMongoLeaseCompetingConsumerStrategy` and `NativeMongoLeaseCompetingConsumerStrategy` kept a subscriber recorded as the lease holder after `releaseCompetingConsumer`, until a background refresh noticed, which is up to half the lease time later. `hasLock` answered yes until then, so a consumer that asks instead of registering a listener kept working on a lease it had given up. `SagaRunner` asks on every timer tick, which is where this would have reached you. A listener also heard about the same loss twice, once from the release and once from the refresh that then found its own write rejected, and `CompetingConsumerListener` now documents that it is told what changed rather than what is currently true. `unregisterCompetingConsumer` and `releaseCompetingConsumer` also carried word-for-word identical javadoc, and now describe what separates them. Unregistering forgets the consumer, so the lock goes to somebody else and it stays out until you register it again, which is what you want for a subscription a user paused. Releasing keeps it registered, so it gives the lock up now and may take it back on its own with nobody registering it again, which is what you want for one the system paused. Found by writing `CompetingConsumerStrategyConformance`. Resolves [#516](https://github.com/johanhaleby/occurrent/issues/516).
* **A MongoDB subscription now refuses a start position it cannot apply, instead of accepting it and never delivering.** `NativeMongoSubscriptionModel` and `ReactorMongoSubscriptionModel` worked out where to start only once the subscription was already running, the first on its dispatcher thread and the second inside its deferred change stream. A `Checkpoint` whose stored string neither of them can parse therefore failed where nobody was listening, and since both retry a broken change stream indefinitely, the failure was retried forever: `subscribe(..)` returned a subscription whose `waitUntilStarted(..)` never answered, while `isRunning(subscriptionId)` kept saying yes and nothing was ever delivered. Both now refuse such a position from `subscribe(..)` itself, so the exception reaches you and no subscription is left registered under that id. `SpringMongoSubscriptionModel` always refused it there and is unchanged. A dynamic start position is still resolved when the model asks for it rather than when you subscribe, because resolving one means calling your own function and the model calls it again when it subscribes. This is the same defect the subscription filter had in [#524](https://github.com/johanhaleby/occurrent/issues/524), which was fixed for filters and left open for start positions. Found by the subscription TCK.
...
* **An in-memory checkpoint storage for the reactive stack.** `org.occurrent.subscription.inmemory.reactor.InMemoryCheckpointStorage` in `occurrent-subscription-inmemory` is the reactive twin of the blocking `InMemoryCheckpointStorage` published earlier in this release: checkpoints live in a `ConcurrentHashMap`, nothing to connect to, a good fit for tests and for small applications whose checkpoints need not survive a restart. Its publishers are cold, so nothing is read, saved or deleted until you subscribe, and arguments are validated when you call. The artifact declares the reactive subscription API as an optional dependency, so a blocking-only application carries nothing new.
* `OccurrentSubscriptionsExtension` gained things that compose with the above, and all work with any store rather than only MongoDB. `clearingCheckpoints(checkpointStorage)` deletes every known subscription's checkpoint before each test, so a subscription cannot resume from where an earlier test left it and receive events that test wrote. Clearing the events alone does not achieve that, and the checkpoint is not necessarily stored next to them, since a MongoDB event store keeping its checkpoints in Redis is an ordinary setup. On an introspectable model with no subscriptions yet, clearing is a no-op rather than an `IllegalStateException` thrown from `beforeEach` before any test body runs, since deleting nothing is the correct outcome for an empty set. `clearingCheckpointsFor(checkpointStorage, ids..)` names checkpoints to clear without starting them, for a subscription no model reports yet. `clearingStateWith(runnable)` runs whatever you give it after every subscription is stopped and before any is resumed, which is where a database flush goes. Together they mean a test class needs one `@RegisterExtension` rather than two ordered with `@Order`, which was the only way to express it before. Resolves [#483](https://github.com/johanhaleby/occurrent/issues/483). Rationale in [ADR 95](doc/architecture/decisions/0095-a-published-testing-leaf-for-mongodb.md).
* `@Saga` now honours `startupMode`, which it previously accepted and ignored. A saga that replays history from the beginning starts in the background by default, the same as `@Subscription` and `@Projection`, so it no longer holds up your application while it works through that history. **This changes behaviour even if you never set `startupMode`:** a `@Saga(startAt = BEGINNING)` used to block startup until its replay finished and now does not. Set `startupMode = WAIT_UNTIL_STARTED` to get the old behaviour back. A saga starting in the background does not fire timers until its subscription is running and has finished replaying, so it cannot issue commands against state it has not finished rebuilding. That now covers an event-store saga behind a catch-up subscription model as well as a push-fed one, since `isRunning(id)` is true throughout a replay on both and only `ReplayAwareSubscriptionModel.isCatchingUp(id)` can tell them apart. `SagaRunner.run(...)` gained a `waitUntilStarted` parameter for the same reason, since a caller driving the runner directly could not opt out of the wait at all.
* `CatchupThenPushSubscriptionModel` can be stopped, started, paused and cancelled like any other subscription model on both stacks, and stopping it now stops a catch-up replay that is already running rather than only the live feed behind it. [ADR 85](doc/architecture/decisions/0085-every-subscription-model-can-be-stopped.md) gave the four register-only models a life cycle but left the two catch-up wrappers out; this closes it for both push ones. `DomainEventFeed` and `CatchupProjectionFeed` gained `stopCatchUp()` for the same reason. Stopping is reversible, so starting the model again replays the history from the beginning for any catch-up the stop interrupted, and `start(false)` leaves those replays for `resumeSubscription(..)` to pick up one at a time. Closing a Spring context now stops these replays and waits for them to unwind, on both stacks, which it previously had no way to do at all on the reactive one.
* A reactive subscription registered while its subscription model is stopped no longer misses what was written before you start it. `ReactorDurableSubscriptionModel` reads the current position when the subscription is registered and stores it when the subscription starts, so stopping the model, registering a subscription and starting it later withholds events rather than losing them. Before this, the position was only decided when the subscription started, so a subscription running for the first time began wherever the feed had reached by then. One that has run before was never affected, since it resumes from its own stored position, and nothing is written until a subscription actually starts, so one you register and never start still leaves nothing behind. The blocking stack never had this problem, because its models register with the feed even while stopped rather than skipping registration entirely. [ADR 86](doc/architecture/decisions/0086-a-manual-subscription-is-registered-not-started.md) explains the guarantee this brings the reactive stack in line with.
...
* The in-memory event store and the in-memory subscription model can now filter on a field inside an event's `data` payload, which previously threw and worked only on MongoDB. That mattered most in tests, since the in-memory store is the one most people test against, so a `Filter.data(..)` that worked in production failed locally. Reading a payload means parsing it, so the store asks for a reader rather than picking a JSON library for you: add `occurrent-common-inmemory-filter-matching-jackson` and build the store with `new InMemoryEventStore().withDataFieldReader(new JacksonDataFieldReader())`, or pass the same reader as the last argument to `InMemorySubscriptionModel`. A store built without one refuses a data filter with an `UnsupportedOperationException`, rather than quietly matching nothing. The path is the dotted path MongoDB already resolves, so `Filter.data("person.city", ..)` reaches into a nested object, a further segment reaches into each element of an array of objects too (`Filter.data("items.sku", eq("a"))` matches `{"items":[{"sku":"a"}]}`, the way MongoDB reaches into it), a plain array field matches when any element does, and a number stays a number, meaning `eq("42")` does not match a stored `42`. Those rules were measured against MongoDB rather than assumed, and the conformance suite now holds every store to them. Not supported, on purpose: anything beyond dot notation, since MongoDB cannot answer it either. `JacksonDataFieldReader` reads the payload without ever serialising it: a Mongo-sourced event already carries a decoded `Document` and is walked directly, and anything else is streamed field by field with `skipChildren()` rather than materialised into a tree first. `DataFieldReader` also gained a `readAll(CloudEvent, Collection<String>)` default method, resolving every path in one call rather than one `read(..)` at a time, so an implementation backed by a streaming parser has a seam to resolve every path in a single traversal instead of parsing the payload once per path. The default just loops over `read(..)`, so no existing implementation breaks. Resolves [#58](https://github.com/johanhaleby/occurrent/issues/58), [#582](https://github.com/johanhaleby/occurrent/issues/582) and [#587](https://github.com/johanhaleby/occurrent/issues/587). Rationale in [ADR 87](doc/architecture/decisions/0087-a-seam-for-reading-a-payload-in-memory.md).
...
* A saga can now issue the domain function itself instead of a command object, so a domain model built from plain functions needs no command types and no `handle(events, command)` switch. `Invocation<E>` in `occurrent-command-dispatch` pairs a stream id with the function to run against that stream, and `CommandDispatchers.invocation(applicationService)` dispatches it. In Kotlin every core and flow reaction gets `issue(streamId) { events -> ship(events) }`. `DcbInvocation` and `DcbCommandDispatchers.invocation(...)` are the DCB versions, taking a `DcbCriteria` read boundary instead of a stream id. One thing to know before choosing this: a lambda has no value equality, so you cannot compare issued commands with `containsExactly`. Check what the command does instead, with `step.issuedCommands().single().decision().apply(events)`, or run the saga and check the events it wrote. [ADR 81](doc/architecture/decisions/0081-function-shaped-saga-commands.md) explains why this is a command type rather than a new kind of saga effect.
* `CommandDispatchers.decider(...)` and `DcbCommandDispatchers.decider(...)` now write a saga reaction's commands as one append per target instead of one per command. A reaction issuing three commands against the same stream, or inside the same `DcbCriteria` boundary, used to read that stream and append to it three times, and a failure on the third left the first two applied. Now the decider sees the run in one go, each command decides against what the ones before it decided, and a failure leaves nothing written. Only consecutive commands are combined into one append. Commands for order A, then order B, then order A again stay three appends, because the two order A commands are not next to each other. [ADR 76](doc/architecture/decisions/0076-batch-command-dispatch-seam.md) added `dispatchAll` so a dispatcher could write a whole batch together, and Occurrent's own dispatchers now do. There is nothing to change in your code. What you may notice, for a reaction dispatched through either `decider`, is fewer stream version bumps and no partial progress when one command in the run fails. On the DCB side a command the decider has no boundary for now fails the whole batch before anything is appended, rather than after the earlier commands were already written. `DcbCommandDispatchers.invocation(...)` is unaffected. It does not group commands into a shared append, by design, since invocations sharing a boundary may carry different tag generators, so a reaction dispatched through it keeps whatever partial-progress behaviour it already had. Each command's boundary is derived exactly once for a batch, which `DcbDeciderApplicationService` supports with a new `execute(DcbCriteria, List, decider)` overload, in both the blocking and reactor variants, for any caller that has already resolved the boundary and would otherwise make the decider derive it a second time. Resolves [#480](https://github.com/johanhaleby/occurrent/issues/480) and [#491](https://github.com/johanhaleby/occurrent/issues/491).
...
* Contributor-facing only, nothing in a published artifact changes: a MongoDB test container now owns the database it hands out, so two Maven runs on one machine no longer delete each other's data. Every test container comes from `ReplicaSetReadyMongoDBContainer.withDefaultVersion()`, which prefixes each database name with the process id and a per-container counter, and nothing binds host port 27017 any more, so a locally installed MongoDB no longer blocks the suite either. An IDE run also stops building the image `mongo:null`, since the version no longer comes from a system property only Surefire sets. Removing the fixed port also surfaced that the test configuration still named the Mongo url `spring.data.mongodb.uri`, which Spring Boot 4.1 deprecates at error level and no longer binds, so those applications had been silently reaching `mongodb://localhost:27017/test` all along; the test side now uses `spring.mongodb.uri`. This repository's own tests also move off `test-support`'s `FlushMongoDBExtension` and onto the published `OccurrentMongoFlush`, so the flush empties collections instead of dropping the database and throws instead of silently giving up; the old extension is deleted. Resolves [#505](https://github.com/johanhaleby/occurrent/issues/505). Rationale in [ADR 97](doc/architecture/decisions/0097-a-test-container-owns-its-database.md).
* **`Projections.materializedView` and `Projections.domainEventFeed` gain overloads taking a `RetryStrategy`, so the default push-projection read, fold and save can recover a concurrent update instead of silently losing one.** This became reachable once a live push handler stopped running behind one lock ([ADR 108](doc/architecture/decisions/0108-a-live-push-handler-runs-outside-the-handover-lock.md)): two threads folding two events for the same projection key can now both read the same state, and the second save overwrites the first. The two existing overloads are unchanged, still `RetryStrategy.none()`, so nothing that compiles today changes behaviour. The new overloads only help when your store detects the conflict and throws, an optimistic-locking failure or a unique-key violation, since a retry that never sees an exception is a no-op. `CatchupProjectionFeed`, `DomainEventFeed`, `ProjectionRunner`, `DcbProjectionRunner` and `@Projection(source = PUSH)` all take the resulting `MaterializedView` through their existing overloads, so no further API grew to reach them. [Section 14 of the upgrade guide](doc/migration/upgrading-to-0.32.0.md#14-a-live-push-handler-can-now-be-called-concurrently) covers the fix and what it does not cover. Resolves [#616](https://github.com/johanhaleby/occurrent/issues/616).
...
* **A live push handler no longer runs behind one global lock, so this changes behaviour that shipped.** `CatchupThenPushSubscriptionModel` and `CatchupProjectionFeed` (blocking stack) share `BlockingHandover`, whose `accept(..)` folded every live payload while holding the handover's own lock, permanently, once the catch-up finished. [ADR 90](doc/architecture/decisions/0090-a-push-sink-feeds-one-consumer.md) already allows a listener container with concurrency greater than one pointed at a single push sink; that lock accidentally serialised every one of those threads' handler calls regardless, capping throughput at roughly one payload per handler duration no matter how much concurrency you configured. A JMH benchmark confirmed the ceiling for a handler with realistic per-event cost (a small synchronous I/O call): 8 threads reached about 4 200 ops/s locked against a 5 000 ops/s theoretical serialised ceiling, and about 14 400 ops/s once the fold moved outside the lock, 3.4x higher. The dedup-key reservation that protects the replay-to-live overlap still happens under the lock; only the handler call itself moved outside it. **A handler must now tolerate concurrent invocation once the handover is live, wherever you configure more than one delivering thread.** A single-threaded caller (the common case) sees no change. Resolves [#588](https://github.com/johanhaleby/occurrent/issues/588). Reasoning, including the full benchmark, in [ADR 108](doc/architecture/decisions/0108-a-live-push-handler-runs-outside-the-handover-lock.md), and see [Upgrading to 0.32.0](doc/migration/upgrading-to-0.32.0.md).
