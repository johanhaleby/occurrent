...
* **Both `SpringRedisCheckpointStorage` constructors now refuse a `RedisOperations` whose key serializer does not pass a `String` key through as its own UTF-8 bytes, with an `IllegalArgumentException`, which changes behaviour every version of this storage shipped.** `RedisTemplate`'s own default, in effect whenever nothing calls `setKeySerializer`, is Java serialization, which wraps a key in a class descriptor and a length-prefixed envelope before the string's own bytes. The Cluster-slot alignment above assumes the bytes Cluster hashes are the subscription id's own text, so under Java serialization the two keys could still land in different slots and a fenced deployment could still pass startup and fail on its first conditional write, the exact gap the fix above exists to close. You are affected if you construct this storage with a plain `RedisTemplate` that never had a key serializer set, call `redis.setKeySerializer(RedisSerializer.string())` first, or construct it with a `StringRedisTemplate` instead. Part of [#665](https://github.com/johanhaleby/occurrent/issues/665).
