...
* `@Saga` now honours `startupMode`, which it previously accepted and ignored. A saga that replays history from the beginning starts in the background by default, the same as `@Subscription` and `@Projection`, so it no longer holds up your application while it works through that history. **This changes behaviour even if you never set `startupMode`:** a `@Saga(startAt = BEGINNING)` used to block startup until its replay finished and now does not. Set `startupMode = WAIT_UNTIL_STARTED` to get the old behaviour back. A saga starting in the background does not fire timers until its subscription is running, so it cannot issue commands against state it has not finished rebuilding. `SagaRunner.run(...)` gained a `waitUntilStarted` parameter for the same reason, since a caller driving the runner directly could not opt out of the wait at all.
