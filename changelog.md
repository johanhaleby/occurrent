...
* **A saga can now subscribe on an explicit filter instead of one derived from its event types, with `Saga.Builder.filter(Filter)` and `FlowSaga.Builder.filter(Filter)` (`filter(...)` in both Kotlin `saga { }` blocks, and a trailing argument on `Saga.create(...)`).** It selects on subject, source, data or time the way a `Projection` already could. It also builds a saga over an event hierarchy whose concrete types cannot all be found, which is otherwise refused when the saga is built, because no filter is derived and so nothing is refused. That is the way out for a `CloudEventTypeMapper` of your own that maps a whole hierarchy onto a single CloudEvent type string, which reflection cannot know about. Two things are yours to get right once you set one. A filter that does not match the saga's start events means no instance is ever created, and the build-time refusal is switched off for every event type the saga declares rather than only the one you could not enumerate. Resolves [#751](https://github.com/johanhaleby/occurrent/issues/751).
