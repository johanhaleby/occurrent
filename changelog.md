...
* A `@Projection(source = PUSH)` can now keep its catch-up off the startup path with `startupMode = BACKGROUND`. A push catch-up replays the whole event store before it hands over to the live feed, and that replay used to run on the thread that starts your application, so an application with a large history was held out of service until it finished. It now runs on its own thread, and the application starts while the read model is still filling. The default is unchanged: without `startupMode` the replay still completes before startup does. `startAt`, `startAtGlobalPosition` and `resumeBehavior` are still rejected for a push source, because the catch-up always replays from the beginning and where the live feed resumes is the broker's business. This is the blocking stack only for now, and within it only a projection fed by a `PushSubscriptionModel`: one fed by a `DomainEventFeed` still catches up on the startup path and rejects `startupMode` rather than accepting it and doing nothing. [ADR 87](doc/architecture/decisions/0087-a-push-catch-up-replays-off-the-startup-path.md) explains the design, including why stopping a replay is the right verb here when [ADR 86](doc/architecture/decisions/0086-a-manual-subscription-is-registered-not-started.md) says it is the wrong one for withholding.
* The blocking `CatchupThenPushSubscriptionModel` can be stopped, started, paused and cancelled like any other subscription model, and stopping it now stops a catch-up replay that is already running rather than only the live feed behind it. [ADR 85](doc/architecture/decisions/0085-every-subscription-model-can-be-stopped.md) gave the four register-only models a life cycle but left the two catch-up wrappers out; this closes it for the blocking push one. Closing a Spring context now stops these replays and waits for them to unwind, which it previously had no way to do.
* **Behaviour change for direct users of the blocking `CatchupThenPushSubscriptionModel`.** A catch-up replay failure now reaches you from `waitUntilStarted()` on the returned subscription instead of being thrown out of `subscribe(...)`, because the replay no longer runs on the calling thread. If you call `subscribe` inside a `try`/`catch`, move it to the `waitUntilStarted()` call or you will start with a read model that is silently empty. Nothing changes if you use `@Projection(source = PUSH)` or the projection DSL, both of which wait for you.
