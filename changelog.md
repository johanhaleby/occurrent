...
* **`ExecuteFilter.excludeTypes` now widens a declared type it cannot fully expand to every concrete subtype that can be found, instead of naming only the declared type's own CloudEvent type.** Excluding a supertype has to exclude everything under it, since a narrower exclusion would silently read events the caller asked to keep out, so `excludeTypes(SensitiveEvent.class)` on a sealed `SensitiveEvent` now keeps out every concrete subtype it permits, not just events stored under `SensitiveEvent`'s own CloudEvent type. A concrete class that is declared directly and is itself neither final nor sealed is unaffected by this widening, since reflection cannot discover a subclass stored under its own name, so `excludeTypes` still only excludes that class's own CloudEvent type there, exactly as before. This is the opposite treatment from `type`/`includeTypes` and `DcbCriteriaBuilder`'s `type`/`types` above on purpose: those refuse a declaration they cannot fully expand, since a narrower inclusion would silently miss events dispatch would accept, while a wider exclusion is always safe. An array or a primitive declared type is refused here too, the same two shapes `type`/`includeTypes` refuse, one because no event is ever an instance of a primitive type and the other for consistency with the other two methods rather than impossibility. See `EventTypeExpansion.expandWhatCanBeFound`. The widening itself is not a breaking change in the same sense as the refusal above, it makes `excludeTypes` do what its name always promised. The array/primitive refusal is a narrow exception to that: declaring either to `excludeTypes` was accepted up to 0.33.0, whatever the configured `CloudEventTypeGetter` did with it, and now throws instead, though nobody sensibly excludes by array or primitive type. Resolves [#912](https://github.com/johanhaleby/occurrent/issues/912).
