...
* **A catch-up that stops on a checked exception from a replayed handler now records that failure, which fixes behaviour shipped in 0.33.0.** Java will not let a handler throw a checked exception it has not declared, but Kotlin will. The blocking catch-up recorded a failed replay only for a `RuntimeException` or an `Error`, so after a checked exception it went on buffering live events for a replay that had already stopped, and `accept(...)` returned normally for each of them, so whatever fed it treated them as handled. The exception still reached a caller that waited for the catch-up. What was missing was the record of it, and it now refuses every live event after such a failure, the same as after any other one. This covers `CatchupThenPushSubscriptionModel`, the blocking `CatchupProjectionFeed` and `DomainEventFeed`, and `@Projection` and `@Saga` with `source = PUSH`. No change is needed on your side. Resolves [#1093](https://github.com/johanhaleby/occurrent/issues/1093).
