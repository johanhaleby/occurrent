...
* **A reactor MongoDB subscription now retries a failing action instead of dying of it.** A named subscription on `ReactorMongoSubscriptionModel` whose action threw, or whose `Mono` errored, terminated that subscription silently: the retry only guarded the change stream underneath, so one bad delivery ended the subscription for good. The action is now retried with the model's configured backoff, the reactor counterpart of the blocking models' `RetryStrategy` around the handler, and the subscription keeps its id and keeps running. Found by running the subscription TCK suites over the reactor stack.
