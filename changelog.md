...
* **A live event accepted while a reactor catch-up replay runs, and still waiting when that replay is stopped, is no longer applied by the next catch-up, which changes behaviour shipped in 0.33.0.** This covers the reactor `CatchupThenPushSubscriptionModel` and `CatchupProjectionFeed`. The stop completes the event's `accept(..)` without applying it, and `CatchupThenPushSubscriptionModel` reports it as `DEFERRED`, but the event stayed in the live buffer and the next catch-up applied it once it went live. A caller that offered the event again could have it applied twice. The event also kept its place in the buffer until then, so repeated stops could fill it and refuse every new event. Resolves [#1130](https://github.com/johanhaleby/occurrent/issues/1130).
