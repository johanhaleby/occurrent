...
* **A durable reactor subscription hands the subscription to the model it wraps, when that model manages named subscriptions itself.** `ReactorDurableSubscriptionModel` used to read from the wrapped model's cold `Flux` and run the delivery itself, so an unsupported `SubscriptionFilter` was only reported once the subscription started, and an action that failed ended the subscription rather than being retried. Wrapping `ReactorMongoSubscriptionModel` directly now inherits both of those from it, which means the retry is configured in one place, on the model doing the retrying. That covers the reactive Spring Boot starter's own wiring for a store that writes no `position`, the `mongodb-subscription-to-spring-event` example, and anyone composing the two models by hand. Wrapping one of the reactor catch-up models behaves as before, because those offer only the cold `Flux` and there is no named subscription underneath to inherit anything from. See #547, which stays open for that half.
...
* `SubscriptionModelLifeCycle.stop()` now documents that it leaves every running subscription paused, so you can resume one on its own with `resumeSubscription(id)` afterwards without starting the rest. Every subscription model already behaved this way, but nothing said so, which made it unsafe to build on.
