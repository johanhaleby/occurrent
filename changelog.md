...
* A new `occurrent-testing-junit-jupiter` artifact gives you a JUnit 5 extension that keeps every subscription stopped while a test runs, so a test only runs the subscriptions it names. `OccurrentSubscriptionsExtension.stoppedByDefault(subscriptionModel)` stops them before and after each test, `start("someId")` resumes one and blocks until it has actually started so you cannot forget to wait, and `alwaysStart("someId")` resumes a fixed set for a whole test class. It depends on nothing but JUnit and the blocking subscription API, so it works without Spring and without a container. The point of the default is that adding a subscription to your application can no longer change what an existing test asserts. [ADR 82](doc/architecture/decisions/0082-a-published-testing-module-for-application-authors.md) explains why this is published to Maven Central while `test-support` stays internal.
