...
* `InMemorySubscriptionModel` can now be waited on until it has handled everything fed to it, through `waitUntilAllEventsProcessed()` and a `Duration` overload, so a test can write events and then assert on the read model with a plain assertion instead of polling one. Delivery there is asynchronous (`accept(...)` queues on the caller's thread and a pool thread runs the handler), which is why a projection test previously had to poll until an assertion passed or a timeout expired. A subscription counts as done when it has no queued events and is not inside a handler, and a handler that keeps throwing is waited out until its retry strategy gives up, which is why the wait takes a timeout and reports whether it drained rather than throwing. Pausing does not exclude a subscription, because it only stops new events being queued and the subscription's own thread keeps draining whatever was already there. This is deliberately in-memory only: a change stream is unbounded, so there is no point at which everything written has arrived, and a test against a real MongoDB keeps polling. Rationale, including why a position-based "caught up to here" was rejected as unsound, in [ADR 77](doc/architecture/decisions/0077-deterministic-in-memory-subscription-drain.md). Resolves [#451](https://github.com/johanhaleby/occurrent/issues/451).
...
* Added a TCK, a set of abstract conformance suites that an implementation of an Occurrent contract runs against itself, so verifying a new event store is a few lines of test code instead of a few thousand. Three new published artifacts, `occurrent-tck-common`, `occurrent-tck-eventstore-blocking` and `occurrent-tck-eventstore-reactor`, which means an implementation living outside this repository can depend on them (the older `test-support` module is not published and never could serve that). You supply one `EventStoreFixture`, which hands back the store under test and declares the `EventStoreCapability` values it was built with, and then extend one suite per capability. This release ships the stream half of the event-store contract as `StreamEventStoreConformance`: reading and writing a stream, paging a read, stream existence, `WriteResult`, duplicate detection, reading through a `StreamReadFilter`, and every `WriteCondition` operator together with its exact failure message. Occurrent's own four event stores run it, so the build now proves they agree on this part of the contract, which nothing did before. `EventStoreQueries`, `EventStoreOperations`, positions, DCB, and the subscription-side contracts follow. A reactive store runs the same suite through `BlockingEventStoreOverReactive`, a bridge that presents a reactive store as a blocking one so the behavior is only described once. That bridge is for tests only, because every method on it blocks the calling thread. The suites deliberately never skip a test. An implementation that does not support a capability simply does not extend that suite, so declining is something you can see in the code rather than a skipped test nobody reads. One thing to know before depending on these: the suites are ordinary classes under `src/main`, so JUnit and AssertJ come along at compile scope rather than test scope. That is on purpose, since the suites are made of JUnit annotations and a consumer cannot run them otherwise. The suites so far are `StreamEventStoreConformance` for reading and writing a stream, `EventStoreQueriesConformance` for querying across streams, and `EventStoreOperationsConformance` for the delete and update operations. The suites assert one correct outcome for every store rather than accommodating each one. Where a difference is genuinely intended, the fixture declares which way a store goes and the suite asserts the documented outcome for that answer, so both behaviours stay pinned instead of the assertion being loosened until everyone passes. Composing a natural sort step with a field sort is the one case that qualifies, because `SortBy.natural` already documents that the in-memory store treats it as an insertion-order tiebreaker while the MongoDB stores reject it. Running every store against one suite also found real bugs that nothing had recorded, listed separately below, and those were fixed rather than declared. Part of [#393](https://github.com/johanhaleby/occurrent/issues/393), and it resolves the older [#75](https://github.com/johanhaleby/occurrent/issues/75). Rationale in [ADR 77](doc/architecture/decisions/0077-a-published-tck-for-occurrent-contracts.md).
* The MongoDB event stores now persist the CloudEvent `time` attribute in one canonical shape when `TimeRepresentation.RFC_3339_STRING` is configured, always with seconds and always with nine fractional digits, so `2026-07-28T12:00:00.000000000Z` rather than `2026-07-28T12:00Z`. Two things were wrong before, and both came from the same cause: the store wrote the value with `OffsetDateTime.toString()`, which drops trailing zero components, while a query filter rendered it with a formatter that always writes seconds. Because that representation compares strings, `Filter.time(instant)` did not match an event written at exactly that instant when the instant's seconds and nanoseconds were both zero, which is any timestamp truncated to a whole minute. MongoDB subscriptions were affected too, since they convert their filters through the same code. Separately, a variable-length string cannot sort chronologically, so range queries on `time` were unsound, which is why that representation was documented as not supporting them. One fixed-width form fixes both, and nanosecond precision is preserved throughout, since the only change is that components which used to be omitted are now written out. Range ordering is sound for a collection whose events carry a consistent UTC offset, which is the common case. Occurrent deliberately does not normalise the offset away, because keeping the timezone is the reason to choose this representation, so a collection mixing offsets still needs `TimeRepresentation.DATE` or a custom attribute. Events written by an earlier version keep their old shape, so an exact match or a range boundary landing precisely on one of them can miss it. Nothing needs migrating for queries over events written from this version onward to be correct, and there is a one-off rewrite documented for anyone who wants exactness across the upgrade. Resolves [#463](https://github.com/johanhaleby/occurrent/issues/463) and [#468](https://github.com/johanhaleby/occurrent/issues/468). Rationale in [ADR 78](doc/architecture/decisions/0078-canonical-fixed-width-time-for-rfc3339-storage.md).
