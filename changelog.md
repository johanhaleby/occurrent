...
* **`occurrent-broker-rabbitmq-blocking` publishes to and consumes from RabbitMQ.** `RabbitMqCloudEventSink` and `RabbitMqDomainEventSink` are the publish side. A publish does not return until RabbitMQ has both confirmed the message and routed it, so a routing key with no matching binding fails the publish instead of confirming it and quietly discarding it. `RabbitMqCloudEventBridge` and `RabbitMqDomainEventBridge` are the consume side, reading a queued message back through the same `RabbitMqCloudEventMapper` the sink writes with. `example/broker/rabbitmq` runs the whole loop end to end against a real RabbitMQ and a real MongoDB, both as a runnable bootstrap you can point at your own broker and as Testcontainers tests.
  ...
  * Each bridge handles its deliveries on a thread of its own, one at a time and in the order the broker sent them, so a handler that blocks holds up only its own bridge and never another bridge on the same `Connection`. `close()` waits up to `closeTimeout(Duration)`, thirty seconds by default, for a delivery already being handled, and puts back on the queue every delivery it did not finish. `occurrent.broker.rabbitmq.bridge.close-timeout` sets it on the starter.
