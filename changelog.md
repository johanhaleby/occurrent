...
* **A saga instance that keeps failing can now be quarantined on the event it stopped on rather than left active and failing indefinitely.** A saga has one subscription, and up to 0.33.0 an event that `evolve`, `react`, or a command dispatcher could not handle was rethrown to the subscription model on every delivery, so wherever that model offered the event again one faulty correlation id stopped every other one behind it. The executor now records when an instance started failing and keeps rethrowing while that has lasted less than `SagaRunnerConfig.quarantineAfter`, five minutes by default. Past that, the instance moves to the new `SagaStatus.QUARANTINED` on whichever event it is failing on then, stops receiving events and stops firing timers, and the subscription carries on for everyone else. The clock belongs to the instance rather than to one event, so an instance where two events both fail keeps the instant it started failing and `SagaFailure.firstFailedAt()` is the start of that run rather than the first time one event failed. Resolves [#818](https://github.com/johanhaleby/occurrent/issues/818) and [#997](https://github.com/johanhaleby/occurrent/issues/997).
