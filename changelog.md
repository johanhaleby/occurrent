...
* **`updateEvent` no longer corrupts a stored event's `position` and drops its `dcbTags`, on every one of the three MongoDB event stores.** Native, Spring blocking, and Spring reactive `updateEvent` rebuilt the whole document through the stream-only mapper, which routes `position` through the general CloudEvent extension writer. That writer has no `Long` overload, so an updated event's `position` came back as a BSON string instead of an int64, silently dropping it from a numeric position query or position-based catch-up, and the indexed `dcbTags` array was never written back at all. Both are now store-owned fields reapplied from the event read before the update, the same way `updateEvent` already reapplies the append id, streamId and streamVersion. `DcbCloudEvents` gains `preserveTags(original, updated)`, mirroring `OccurrentCloudEventExtension.preserveAppendId`, which `updateEvent` calls on every store so a DCB event's `dcbtags` extension and its indexed `dcbTags` array stay derived from the same original tags, and a replacement event an update function builds from scratch cannot change which DCB tags a stored event carries. `OccurrentCloudEventExtension` gains the equivalent `preservePosition(original, updated)`, since the `CloudEvent` `updateEvent` returned to the caller never had its position reapplied either, only the stored document did, so a caller trusting the returned event instead of re-reading could see a different position than what was actually stored, or none at all. `preservePosition` also stops an update function from forging a position onto an event that never had one. Fixes [#876](https://github.com/johanhaleby/occurrent/issues/876).
