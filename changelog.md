...
* The stream subscription DSL `Subscriptions` was renamed to `StreamSubscriptions`.
  * The class `Subscriptions` is now `StreamSubscriptions` and the builder `subscriptions(...)` is now `streamSubscriptions(...)`, so the stream DSL name matches `@StreamSubscription`, `StreamSubscriptionModel`, and the DCB counterpart `DcbSubscriptions`. The old names stay as a deprecated `typealias` and a deprecated builder shim, so existing Kotlin code keeps compiling. Java references to the Spring bean type were migrated, since a Kotlin typealias does not reach Java.
...
* Added initial Dynamic Consistency Boundary (DCB) support.
  ...
  * `DcbQuery` is a sealed type, either `DcbQuery.MatchAll` or `DcbQuery.Items`, built through factories such as `all()`, `type(..)`/`types(..)`, `tagsAllOf(..)`, `typeAndTagsAllOf(..)`, and `anyOf(..)` (taking varargs or a collection of items) for an OR across query items. `DcbQueryItem` gains a matching `type(String)` shorthand.
