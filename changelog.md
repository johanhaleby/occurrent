...
* **A `@Subscription`, `@StreamSubscription`, `@DcbSubscription` or `@SynchronousSubscription` handler method Spring's proxy cannot invoke now fails Spring Boot startup, where it previously ran on the raw bean with no advice applied, and a live-only subscription can no longer see an event another bean wrote during its own startup, see Changes above.** A JDK interface proxy missing the handler method, a private method a CGLIB proxy cannot override, a final method a CGLIB proxy cannot override either, and a static method whose invocation never goes through any proxy at all, all used to deliver every event with `@Transactional` or any other aspect silently skipped, which is the bug fixed above, and now fail startup instead. An unproxied bean has no proxy to lose advice through in the first place, so a private or final handler there still registers and runs exactly as before. Registration for all four annotations also moves to the phase `@Projection`, `@Snapshot` and `@Saga` already use, so the bean-creation-order-dependent chance of catching an event written during another bean's own startup is gone for every one of them. See [section 11 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#11-a-subscription-handler-spring-cannot-invoke-now-fails-startup-and-a-live-subscription-can-miss-a-startup-write).
