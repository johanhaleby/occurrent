...
* **`updateEvent` no longer lets a replacement event an update function builds from scratch silently lose or forge a value the store owns, on every one of the three MongoDB event stores.** That value is `position`, the indexed `dcbTags` array and its `dcbtags` extension, or `streamId` and `streamVersion`, and the same defect shape hit all of them. Native, Spring blocking, and Spring reactive `updateEvent` rebuilt the stored document through the stream-only mapper, which routes `position` through the general CloudEvent extension writer. That writer has no `Long` overload, so an updated event's `position` came back as a BSON string instead of an int64, silently dropping it from a numeric position query or position-based catch-up, and the indexed `dcbTags` array was never written back at all. Fixing the stored document was not enough on its own. The `CloudEvent` `updateEvent` returns to the caller was never touched for `position`, `dcbTags` or `streamId`/`streamVersion` either, so a caller trusting the returned event instead of re-reading could still see a value that disagreed with what was actually stored, or a value an update function forged outright. `DcbCloudEvents` gains `preserveTags(original, updated)`, `OccurrentCloudEventExtension` gains `preservePosition(original, updated)` and `preserveStreamIdentity(original, updated)`, all three mirroring the existing `preserveAppendId(original, updated)`, and `updateEvent` calls all four on every store before returning, so the returned event and the stored document always agree, and a replacement event cannot change which stream, version, position or DCB tags a stored event carries. Fixes [#876](https://github.com/johanhaleby/occurrent/issues/876), [#904](https://github.com/johanhaleby/occurrent/issues/904) and [#927](https://github.com/johanhaleby/occurrent/issues/927).
...
* **A saga instance whose event keeps failing is now quarantined at that event's position instead of blocking every other instance of the same saga.** A saga has one subscription, and up to 0.33.0 an event that a `evolve`, a `react` or a command dispatcher could not handle was retried on that subscription forever, so one faulty correlation id stopped every other one behind it. The executor now records when an event first failed for an instance and keeps rethrowing while the failing is younger than `SagaRunnerConfig.quarantineAfter`, five minutes by default. Past that, the instance moves to the new `SagaStatus.QUARANTINED`, stops receiving events and stops firing timers, and the subscription carries on for everyone else. Find the stopped instances with `SagaInstances.findByStatus(QUARANTINED, ..)`, read `SagaInstance.failure()` for the position and the exception, and call `SagaSubscription.release(sagaId)` once the cause is fixed to replay the subscription from that position. On the annotation path each `@Saga` now publishes its running `SagaSubscription` as `sagaSubscription-<id>`, next to the `sagaInstances-<id>` it already published, since release is the one saga operation the read-only `SagaInstances` cannot offer. Quarantine needs the failing event to be obtainable a second time, so the runner switches the budget off at startup, with a warning saying why, on any subscription model that cannot be resumed at a chosen position. Such a saga keeps the 0.33.0 behaviour rather than acknowledging an event nothing could hand back. Set `quarantineAfter` to `null` to keep it everywhere. Resolves [#818](https://github.com/johanhaleby/occurrent/issues/818).
...
* **`SagaEnvelope`, `SagaInstance`, `SagaStatus` and `SagaRunnerConfig` all change shape for the saga quarantine above.** `SagaEnvelope` gains `started` and `failure`, `SagaRunnerConfig` gains `quarantineAfter`, so both canonical constructors and any record pattern over either stop compiling. `SagaInstance` gains `failure()`, which breaks anyone implementing that interface outside this repository. `SagaStatus` gains `QUARANTINED`, so an exhaustive `switch` or `when` over it stops compiling, and, less visibly, `findByStatus(ACTIVE, ..)` no longer returns the instances that have stopped moving, which is what it was most often used to find. That is five breaks. There is no OpenRewrite recipe, because none of them can be rewritten mechanically. See [Upgrading to 0.34.0](https://occurrent.org/migration/upgrading-to-0.34.0), section 8.
