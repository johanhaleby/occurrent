...
* A reactive catch-up subscription filtering on a field inside an event's `data` payload now works, on every store. `ReactorStreamCatchupSubscriptionModel` applied the filter a second time in process, after already handing the same filter to the subscription model it wraps, and that second evaluation could not read a payload. So a `Filter.data(..)` subscription over MongoDB replayed history correctly and then failed on the first live event, even though the same store answers that filter when you query it. The in-process match is gone, which is also what the blocking `StreamCatchupSubscriptionModel` has always done, so the two stacks now behave the same. Applying the filter belongs to the subscription model that was given it, and MongoDB was already doing it server-side for both the replay and the change stream. A subscription model that ignores the filter it is handed will now over-deliver on this path instead of being silently compensated for, which the blocking stack has always assumed. Resolves [#499](https://github.com/johanhaleby/occurrent/issues/499). Rationale in [ADR 92](doc/architecture/decisions/0092-a-subscription-model-owns-its-filter.md).
