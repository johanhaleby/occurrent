...
* **A saga instance that keeps failing is now quarantined on the event it stopped on instead of blocking every other instance of the same saga.** A saga has one subscription, and up to 0.33.0 an event that `evolve`, `react`, or a command dispatcher could not handle was retried on that subscription forever, so one faulty correlation id stopped every other one behind it. The executor now records when an instance started failing and keeps rethrowing while that has lasted less than `SagaRunnerConfig.quarantineAfter`, five minutes by default. Past that, the instance moves to the new `SagaStatus.QUARANTINED` on whichever event it is failing on then, stops receiving events and stops firing timers, and the subscription carries on for everyone else. The clock belongs to the instance rather than to one event, so an instance where two events both fail keeps the instant it started failing and `SagaFailure.firstFailedAt()` is the start of that run rather than the first time one event failed. Resolves [#818](https://github.com/johanhaleby/occurrent/issues/818) and [#997](https://github.com/johanhaleby/occurrent/issues/997).
  * The budget covers everything a delivery does, not the reaction alone. Reading the CloudEvent, working out which instance it belongs to, checking for a redelivery, `evolve`, `react`, the dispatcher and the store all count, and an `Error` counts like a `RuntimeException`, so a saga whose id extractor cannot read one event no longer blocks every instance of that saga. The one exclusion is `OutOfMemoryError`, because that says the JVM ran out of heap while some instance held the thread rather than anything about the instance, and nothing in this release brings an instance back out of quarantine.
  * An event that fails before the saga can say which instance it belongs to, because the converter or the id extractor threw, has no instance to quarantine. The subscription is let past it on the same budget and the skip is logged at ERROR naming the event and what stopped it. Be aware that no row is written for it, so `findByStatus(QUARANTINED, ..)` does not list it and the log line is what you have. Letting the subscription past is not what removes the event, and the runner confirms that before it does so, so wherever your source still has the event, repairing the converter or the id extractor and feeding it to the saga again is how you recover it. That check is about what the acknowledgement costs rather than about what the source holds right now, so it does not promise the event is still there.
