...
* `ManualStartSubscriptionModel` now records a subscription's first start position with a single conditional write, so two nodes starting the same manually-started subscription at once can no longer both write and lose the events between the two positions, resolving [#669](https://github.com/johanhaleby/occurrent/issues/669).
