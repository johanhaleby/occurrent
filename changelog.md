...
* Forwarding stored events to a broker at least once no longer needs a shipped transport, or you can use the new RabbitMQ one. `CloudEventForwarder` holds its checkpoint back until your `CloudEventSink` confirms delivery, so an application with its own publisher wrapper gets that guarantee at the forwarder's default start position, and `occurrent-broker-rabbitmq-blocking` now gives you a `CloudEventSink` for RabbitMQ that keeps the same guarantee, ahead of the Kafka module landing on the same interfaces.
