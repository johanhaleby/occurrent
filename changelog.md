...
* `PushSubscriptionModel` takes an optional `PushObserver`, told a `RoutingOutcome` for each event `accept(...)` is asked to deliver, so a broker bridge can acknowledge a message safely instead of guessing.
