...
* Occurrent can forward stored events to a broker at least once, either through a `CloudEventSink` you write yourself or through the new RabbitMQ and Kafka modules, which also consume from a broker back into a push subscription or a projection.
* Declaring a sealed event supertype now matches every concrete type it permits, in projections, DSL subscriptions, queries, snapshot views, `ExecuteFilter` and `DcbCriteriaBuilder` alike, the same way sagas already did.
* A broker listener acknowledges a message on the `RoutingOutcome` that `PushSubscriptionModel.acceptRedeliverable(...)` returns, on both stacks, instead of guessing. An optional `PushObserver` is told the same outcomes.
* A saga instance that keeps failing can be quarantined on the event it stopped on rather than left active and failing indefinitely.
* Events that Occurrent's own `updateEvent` damaged in 0.33.0 or earlier can be repaired with the new `occurrent-eventstore-mongodb-update-event-repair` module, and a store now warns at startup when it finds any.
...
* **Occurrent can now forward stored events to a broker at least once, through the new `occurrent-broker-api-blocking` module.** `CloudEventForwarder` runs a durable subscription at its default start position and hands each stored event to a `CloudEventSink` you implement, advancing its checkpoint only once the sink returns. A sink that throws leaves the checkpoint where it was and the event publishes again on the next run. That makes publication at least once as long as the sink does not return before the broker has taken the event, which is the sink's contract to keep. `DomainEventForwarder` does the same for an application whose converter already produces domain events, decoding each stored `CloudEvent` once and stamping its `EventMetadata` onto the domain event rather than converting twice. Occurrent adds no broker dependency of its own here. See [ADR 133](doc/architecture/decisions/0133-a-broker-is-a-transport-for-the-push-feed-and-never-a-subscription-model.md). Resolves [#413](https://github.com/johanhaleby/occurrent/issues/413), [#414](https://github.com/johanhaleby/occurrent/issues/414), [#415](https://github.com/johanhaleby/occurrent/issues/415), [#416](https://github.com/johanhaleby/occurrent/issues/416), [#417](https://github.com/johanhaleby/occurrent/issues/417), [#419](https://github.com/johanhaleby/occurrent/issues/419), [#846](https://github.com/johanhaleby/occurrent/issues/846), [#867](https://github.com/johanhaleby/occurrent/issues/867) and [#922](https://github.com/johanhaleby/occurrent/issues/922).
  * The two `CloudEventForwarder` overloads that take an explicit start position opt out of that resumption, as their javadoc explains.
* **`occurrent-broker-rabbitmq-blocking` publishes to and consumes from RabbitMQ.** `RabbitMqCloudEventSink` and `RabbitMqDomainEventSink` are the publish side. A publish does not return until RabbitMQ has both confirmed the message and routed it, so a routing key with no matching binding fails the publish instead of confirming it and quietly discarding it. `RabbitMqCloudEventBridge` and `RabbitMqDomainEventBridge` are the consume side, reading a queued message back through the same `RabbitMqCloudEventMapper` the sink writes with. `example/broker/rabbitmq` runs the whole loop end to end against a real RabbitMQ and a real MongoDB, both as a runnable bootstrap you can point at your own broker and as Testcontainers tests.
  * `RabbitMqTopicExchangeDestinationResolver` derives the exchange and routing key from the cloud event type through the same `CloudEventTypeMapper` your `CloudEventConverter` already uses, so a publisher and a consumer agree on where an event goes by reading one mapping instead of matching two hand-written strings.
  * `build()` retries opening the channel and declaring the queue, bindings and QoS on the `Connection` it was given, so a broker still starting or mid-restart does not fail application startup, backing off from 100 ms up to 2 seconds over ten attempts. It never creates or reconnects that `Connection` itself, so it does nothing for a broker still unreachable when the `Connection` bean is created, or for a `Connection` with automatic recovery turned off. `retryStrategy(RetryStrategy)` on both builders, and `occurrent.broker.rabbitmq.bridge.retry.*` on the starter, replace that default outright.
  * Both bridges keep consuming after their connection has recovered automatically, as long as that recovery restores the consumer's topology too, which it does by default. A delivery still being handled when the connection drops gets no acknowledgement at all, since the RabbitMQ client sends nothing for a tag from the dead channel, so RabbitMQ puts it back on the queue and redelivers it once the recovery has registered the consumer on the recovered channel. Under `PARK` a delivery that fails during a recovery is both parked and put back on the source queue, one extra copy, which is the same at-least-once delivery you get everywhere else here.
  * Each bridge handles its deliveries on a thread of its own, one at a time and in the order the broker sent them, so a handler that blocks holds up only its own bridge and never another bridge on the same `Connection`. A handler that finishes while the connection is down cannot acknowledge its delivery, so RabbitMQ delivers that message again once the connection has recovered, and the bridge goes on consuming as long as the `Connection` recovers its topology too, which it does by default. On a `Connection` without automatic recovery the bridge stops instead, since that connection never comes back. A `RuntimeException` or an `AssertionError` from a handler goes through your `DeliveryFailurePolicy`. An `Error`, or a checked exception, which a handler written in Kotlin can throw without declaring it anywhere, stops the bridge and puts that delivery back on the queue instead. The background poll that starts and cancels each bridge's consumer, and releases a delivery the bridge is holding, keeps going when something it calls throws, a `readinessSource` of your own included, and tries again on its next tick. `close()` waits up to `closeTimeout(Duration)`, thirty seconds by default, for a delivery already being handled, and puts back on the queue every delivery it did not finish, also when something throws while it shuts down. `occurrent.broker.rabbitmq.bridge.close-timeout` sets it on the starter.
  * `RabbitMqDomainEventSink.publish(E)` and `publish(EventMetadata, E)` now agree on which extensions a never-stored event loses. Both strip only `streamid`, `streamversion`, `position` and `appendid`, the four properties a never-stored event cannot have, and publish every other extension the converter set, with `metadata` winning where it names the same key.
* **`occurrent-broker-kafka-blocking` publishes to and consumes from Kafka.** `KafkaCloudEventSink` and `KafkaDomainEventSink` are the publish side. A publish does not return until the broker has acknowledged the send with `acks=all`, which the sink sets for you when your producer config leaves it unset and refuses to start if you set it weaker, since an acknowledgement under a weaker setting can arrive for a send the broker never durably stored. `KafkaCloudEventBridge` and `KafkaDomainEventBridge` are the consume side, feeding a `PushSubscriptionModel` and a `DomainEventFeed` the same way their RabbitMQ counterparts do.
  * `KafkaSharedTopicDestinationResolver` is the default, one topic you name for every event, keyed by the event's stream id, so two events of the same stream stay in order against each other even when they are different event types. Choose that topic's partition count before producing to it, since Kafka hashes a key against the current partition count, so growing it later remaps existing stream ids across partitions and can break ordering for whatever streams are still in flight.
  * `KafkaTopicPerTypeDestinationResolver` derives a topic per cloud event type instead, for a deployment that wants per-type topics for retention or independent consumer scaling. A stream mixing types is then ordered only within one type, since each type lands on its own topic.
  * A Kafka `Consumer` is not thread-safe, so each bridge runs one dedicated poll loop that feeds the model or the feed and commits. `DELIVERED`, `FILTERED`, and a delivery failure resolved by a confirmed park all stage a record's offset for the next commit. Anything left unresolved seeks the consumer back to that record and stops processing the rest of that partition's poll, leaving other partitions in the same poll unaffected. A record whose outcome is a permanent stop instead seeks every partition still left in that poll back to its own earliest fetched record and stops the bridge for good, rather than draining the rest of the poll into a model or feed that has already given up. `enable.auto.commit` has to be `false` or the bridge refuses to start, since Kafka would otherwise advance the offset on a timer regardless of what the bridge decided.
  * `KafkaCloudEventMapper` reads a record back through `cloudevents-kafka`'s own binary reader and corrects two defects in it. `streamversion` and `position` come back `Long` rather than `String`, and data that is present but empty survives as that instead of collapsing to no data at all.
  * `KafkaDomainEventSink.publish(E)` and `publish(EventMetadata, E)` now agree on which extensions a never-stored event loses. Both strip only `streamid`, `streamversion`, `position` and `appendid`, the four properties a never-stored event cannot have, and publish every other extension the converter set, with `metadata` winning where it names the same key.
* **Every shipped bridge acknowledges a broker message only on the `RoutingOutcome` values that mean the event was actually handled.** It acknowledges on `DELIVERED` and `FILTERED`, holds and redelivers on `DEFERRED` and `UNAVAILABLE`, applies your `DeliveryFailurePolicy` to `NOT_DELIVERABLE`, and stops on `REFUSED`. A held outcome bypasses `DeliveryFailurePolicy` entirely, `PARK` included, since nothing about such a message is broken, only not deliverable right now. A delivery that is not acknowledged is either redelivered (`REDELIVER`, the default) or published to a parking destination and acknowledged once that publish is confirmed (`PARK`), chosen through `onDeliveryFailure(DeliveryFailurePolicy)`.
  * Both bridges declare their queue and bindings from `DestinationResolver.destinationsFor(filter)`, or the catch-all destination when no filter narrows it, unless `declareTopology(false)` is set, so an existing queue and its bindings stay a platform team's to own. `bindingFilter(...)` on all four bridge builders takes either a plain `Filter` or a `SubscriptionFilter`, so a narrowing that needs no stream or DCB semantics is written without a wrapper. An explicit empty `bindings(...)` set now fails `build()` with an `IllegalStateException` instead of consuming nothing, on Kafka always and on RabbitMQ whenever `declareTopology(true)` applies.
  * A background poll starts and stops the underlying consumer to match the subscription's running state, one second apart by default. That is an efficiency measure, not what keeps a message safe. What keeps it safe is the reported outcome on every single message, `UNAVAILABLE` for a stopped or paused model, never acknowledged.
  * `RabbitMqCloudEventBridge` and `KafkaCloudEventBridge` also read an optional `readinessSource`, another pacing hint. Wire `readinessSource(catchupThenPush::isReadyForLiveDelivery)`, the wrapper's new `isReadyForLiveDelivery(String)`, and the bridge stops pulling messages at all while that answers false, cutting down on how often the refuse-and-redeliver round trip happens rather than being what prevents it. `@Projection(source = PUSH)` and `@Saga(source = PUSH)` publish the `CatchupThenPushSubscriptionModel` they build as a `"catchupThenPushSubscriptionModel-" + id` bean for exactly this, and both starters wire it automatically. The zero-config `readinessSource` each starter pre-seeds every bridge with correlates to that bean by the exact `PushSubscriptionModel` instance the bridge was built with, never by subscription id, since two independent wrappers are allowed to share an id and an id-only lookup could answer a healthy bridge with an unrelated wrapper's own paused or failed state.
  * A `DomainEventFeed` registered with a payload filter it cannot evaluate live throws `UnreadableLiveFilterException`, and `RabbitMqDomainEventBridge` treats that as permanent. It stops itself and never acknowledges that delivery, rather than redelivering into the same failure forever.
* **`occurrent-broker-rabbitmq-spring-boot-starter` and `occurrent-broker-kafka-spring-boot-starter` auto-configure the two transports**, one starter artifact each so neither broker client ever lands on the other's classpath. `@EnableOccurrentRabbitMqBroker` and `@EnableOccurrentKafkaBroker` activate them, the same `@Import` mechanism `@EnableOccurrent` already uses for the MongoDB starter. Each builds its sink and a bridge factory from `occurrent.broker.rabbitmq.*` or `occurrent.broker.kafka.*`, every default copied from the builder default it configures.
  * The RabbitMQ starter takes your own `com.rabbitmq.client.Connection` bean rather than building one, and activates only once that bean exists. `spring-boot-starter-amqp` doesn't supply it, because it registers a Spring AMQP `ConnectionFactory` instead, so declare the `Connection` bean yourself.
  * A bridge is one per consumer, so instead of one bean trying to be every consumer's bridge, each starter gives you a `RabbitMqCloudEventBridgeFactory` or `KafkaCloudEventBridgeFactory` and a domain-level twin, pre-seeded with the shared defaults and leaving only the queue or consumer group to name per call.
* **`CatchupThenPushSubscriptionModel` now checks that it still owns the subscription id immediately before writing its catch-up marker.** An attempt whose id was cancelled before that check writes nothing, so a marker that is there always describes a history that was read in full. Every later subscription of that id trusts such a marker, in the same process or after a restart, so cancelling a subscription and subscribing the same id again goes straight to live delivery rather than reading the history a second time. Delete that id's checkpoint when you want the history read again.
  * The blocking model writes that marker under a lock for the subscription id rather than the model's own monitor, so a slow checkpoint store no longer holds up `stop`, `start`, `pauseSubscription` and `resumeSubscription`. `cancelSubscription` and `subscribe` still wait for a write already running, since those are the calls that move an id out from under one, and `cancelSubscription` holds the model monitor while it waits.
  * Both stacks now refuse to write that marker while the model is stopped. A stop cannot call off a write that has already begun, since that would mean waiting for a checkpoint store inside `stop()`, and `stop()` says so on both stacks rather than promising more.
* **`PushSubscriptionModel` and its reactor counterpart take an optional `PushObserver`, told a `RoutingOutcome` for an event `accept(...)` was asked to deliver.** `accept(...)` itself stays silent by design, since it cannot refuse an event that might also have come from the write path, so a misconfigured queue binding, a missing declared event type, or a type-mapping typo used to look identical to a saga or projection that chose not to react. Pass the observer through the same constructor that already takes a `DataFieldReader`. `PushObserver.noop()` is the default, so nothing changes for existing code, including its cost, since the match check is skipped entirely when none is configured. Resolves [#802](https://github.com/johanhaleby/occurrent/issues/802) and [#848](https://github.com/johanhaleby/occurrent/issues/848).
  * The six values are `DELIVERED` (a running, unpaused subscription's filter accepted the event), `FILTERED` (that same subscription evaluated it and declined it), `UNAVAILABLE` (no running, unpaused subscription for the event to reach at all), `NOT_DELIVERABLE` (a filter threw while being evaluated, so it neither accepted nor declined, or a catch-up-then-live engine refused the event for a reason that can pass, such as a full live buffer), `DEFERRED` (a `CatchupThenPushSubscriptionModel` in front has not reached live yet, safe to redeliver), and `REFUSED` (a registered action promises the refusal is permanent, which a catch-up-then-live engine does once its replay has failed).
  * `RoutingOutcome.disposition()` hands you a four-way answer directly, `ACKNOWLEDGE`, `HOLD`, `FAIL` or `STOP`, and `mayAcknowledge()` answers the acknowledge question on its own, so a bridge you write yourself switches on four values rather than matching six constants and deciding for itself what an unmatched one means.
  * A paused subscription reports `UNAVAILABLE`, not `FILTERED`, because its filter is never consulted, and that difference is what a plain match flag could not make. A caller that checked `isRunning(...)` after `accept(...)` returned, instead of reading the reported outcome, could see a concurrent resume answer `true` for an event that had already been dropped.
  * Where the check runs it shares the same evaluation `route(...)` dispatches from, so the two can never disagree. Anything the observer throws is caught and logged rather than turning a delivered event into a broker redelivery, an `Error` other than an `AssertionError` aside. On the blocking stack the observer is told once the matched registration's action has run, never before. A filter that fails with an undeclared checked exception, or with an `Error` other than an `AssertionError`, is not reported at all on either stack, and neither is a blocking action failing either of those two ways. A matched action that throws a `RuntimeException` other than a refusal, or an `AssertionError`, is still reported `DELIVERED` on both stacks, since the action was genuinely invoked. One that ends in an `Error` other than an `AssertionError` is not reported at all on either, since an action that ended in one delivered nothing. Resolves [#1116](https://github.com/johanhaleby/occurrent/issues/1116).
  * A broker bridge calls `PushSubscriptionModel.acceptRedeliverable(CloudEvent)` rather than `accept(...)`, which refuses an event outright rather than buffering it while a `CatchupThenPushSubscriptionModel` in front has not reached live yet, reported `DEFERRED`. Fed through plain `accept(...)` instead, that wrapper still buffers the event and reports `DELIVERED` once it applies it, exactly as it always has.
  * The reactor stack draws the same distinction. `RegisteringSubscribable.routeReportingMatch` reports from a `RoutingAction` that runs first and returns `Mono<Boolean>`, `DomainEventFeed.acceptCloudEvent` and `CatchupProjectionFeed` report `DEFERRED` for an event that arrives before the projection is live, and `ReactiveHandover.acceptIfLive(T)` refuses such an event outright. A reactor broker listener calls the reactor `PushSubscriptionModel.acceptRedeliverable(CloudEvent)`, described in its own entry below, which returns `DEFERRED` for such an event as well as telling a `PushObserver`.
  * Whatever your observer throws, it never changes what the model does with the event. Any exception, a checked one thrown by an observer written in Kotlin included, and an `AssertionError` from a test spy, is logged and dropped, so the event still counts as delivered and the events after it in the same `accept(...)` batch are still routed. Only an `Error` other than an `AssertionError` reaches the caller.
  * Which throwable the caller then catches depends on whether the model already had a failure of its own to propagate, not on the outcome your observer was told. Told alongside a failure the model is already propagating, whether it came from the filter, the handler or a refusal, your `Error` arrives attached to that failure through `addSuppressed` and the failure itself is what propagates. Told with nothing else in flight, it propagates on its own. `DELIVERED` reaches your observer both ways, since a handler that failed is reported delivered too.
  * On the reactor stack a report is not always on the thread that called `accept(...)`. `UNAVAILABLE`, `FILTERED` and a filter failure's `NOT_DELIVERABLE` run on whichever thread subscribed, while `DELIVERED`, `DEFERRED` and a refusal's own outcome run on whichever thread your handler's `Mono` signalled on, and in a batch the subscription for one event happens on the thread the event before it finished on. An `InterruptedException` your observer throws can therefore set the flag on a scheduler worker that clears it before its next task, and the caller may never see it.
* **`DomainEventFeed` (both stacks) gains `acceptCloudEvent(CloudEvent)`, for a broker bridge that has a `CloudEvent` to hand over rather than an already-decoded domain event.** It matches the event against the same `Filter` `register(...)` was given, decodes it with this feed's `CloudEventConverter` only if it matches, delivers it, and returns the `RoutingOutcome`. A non-matching event is never decoded, and the replay and the live path now share that one filter, so they can never disagree about which events are this projection's. This is a new name rather than an `accept(CloudEvent)` overload, since `DomainEventFeed<CloudEvent>` is a legal instantiation and an overload would let the compiler silently choose between two methods with different behaviour for the same argument. Resolves [#848](https://github.com/johanhaleby/occurrent/issues/848).
  * `DomainEventFeed` also takes an optional `DataFieldReader` constructor argument, mirroring `PushSubscriptionModel`'s, so a payload condition on that filter can be answered on the live path too. `register(...)` is unaffected either way, since it never evaluates the filter in memory. `acceptCloudEvent(...)` throws the new `UnreadableLiveFilterException` for a payload condition it cannot answer, on its first call rather than on `register(...)`, so a caller that never calls it keeps registering exactly the filters it always could. That refusal is permanent for the registration, and the same exception instance is thrown again on every later call. See [ADR 133's amendment](doc/architecture/decisions/0133-a-broker-is-a-transport-for-the-push-feed-and-never-a-subscription-model.md#amendment-2026-08-18-a-domain-level-payload-filter-is-refused-on-first-live-match-use-not-at-register-and-the-refusal-is-permanent) for why it refuses here.
  * It reports `DEFERRED` rather than `DELIVERED` for a matching event that arrives before the registered projection is live, whether that is before any catch-up has run, while a replay is still going, or after `stopCatchUp()` interrupted one in flight. A redelivery is always safe until it lands as `DELIVERED`. Feeding it before a projection is registered still throws `IllegalStateException`. This feed has no write path to protect, so it keeps refusing outright rather than reporting `UNAVAILABLE` for that case.
  * `refusesPermanently()` answers true once its registered projection's own catch-up has failed and never goes back to false, so a listener can tell that apart from a replay that is still running and stop consuming for good instead of redelivering into the same refusal.
* **`AppliedAppendStore` records which appends a projection has applied, and answers whether a particular one has been applied yet.** `recordApplied`, `hasApplied`, `clear` and an `inMemory()` factory live in `dsl/projection-dsl/common`, and `waitUntilApplied(projectionId, appendId, timeout)` polls until the append shows up or the timeout passes. Set `@Projection(recordAppliedAppends = true)` to wire a projection into the store automatically, on both stacks, instead of calling `recordApplied` yourself. This is the membership half of [ADR 132](doc/architecture/decisions/0132-an-append-has-an-identity-and-read-your-writes-becomes-a-membership-question.md), alongside the write-side `AppendId` stamping shipped earlier in this release. Part of [#740](https://github.com/johanhaleby/occurrent/issues/740), [#865](https://github.com/johanhaleby/occurrent/issues/865) and [#871](https://github.com/johanhaleby/occurrent/issues/871).
  * A `true` answer means the projection has applied the first event of that append it handles, and not necessarily every event of it. A projection records an append after the first of its events it applies successfully, so an append whose events arrive across several deliveries can answer `true` while the rest are still unapplied. An event whose update throws records nothing. In the ordinary case the rest follow as soon as the same node works through them. What happens instead when that node stops part way is up to your subscription model. A competing-consumer deployment lets another node take the subscription once the lease expires, 20 seconds by default, which needs a healthy node there to take it, and with none available the rest wait until one runs again. While the subscription is paused or stopped they do not follow either way until someone starts it again. [ADR 132](doc/architecture/decisions/0132-an-append-has-an-identity-and-read-your-writes-becomes-a-membership-question.md) decision 10 says why recording on the append's last event instead cannot work.
  * The in-memory factory keeps the 10,000 most recently recorded appends per projection, or as many as you pass to `inMemory(int)`, and a wait for one it has already dropped times out. Both Spring Boot starters auto-configure a Mongo-backed store, one document per projection id and append id pair, with a compound unique index so recording the same append twice writes one document, and a TTL index bounding how much storage the records take.
  * Configure the collection name, retention, how many times a read or a write calls MongoDB before it fails, and the wait's poll pacing under `occurrent.projection.applied-append`. That last one counts attempts rather than time, defaulting to 10, and an unreachable server spends the driver's own server selection timeout on each attempt, so give the MongoDB client a timeout of its own when the wall clock is what matters. A store the wait cannot reach keeps it polling rather than answering true early.
  * The reactive Mongo store's `waitUntilApplied` refuses immediately with `IllegalStateException` when called from a thread Reactor has marked non-blocking, a WebFlux request handler above all, instead of polling that thread to the timeout and answering `false` for an append that was there. Call it from `Schedulers.boundedElastic()` or another thread reserved for blocking work.
  * `@Projection(recordAppliedAppends = true)` refuses at startup with no `AppliedAppendStore` bean, and with `mode = SYNCHRONOUS`, since a synchronous projection already answers read-your-writes without it.
  * A catch-up-then-push composition records an append whose only delivery was its own replay's. An event committed while the replay is running is applied by the replay, and the broker's copy of it is de-duplicated against that, so what writes the record is a call the de-duplication makes rather than a second delivery. All six compositions on that engine are covered, both `CatchupProjectionFeed`s, both `DomainEventFeed`s and both `CatchupThenPushSubscriptionModel`s. The append is recorded only when the replay applied an event of it, so an event your `Projection.id` returns `null` for records nothing. A projection remembers up to 10,000 such appends per catch-up, and a wait for one past that times out rather than answering `true`. A feed's `goLive()` after its `catchUp()` records the same way. `CatchupThenLiveOptions.dedupCacheSize` sizes what the replay delivered and what a live delivery delivered separately, so a handover holds up to twice the ids at the same setting. See [ADR 137](doc/architecture/decisions/0137-a-live-payload-the-replay-already-delivered-still-reaches-its-source.md). Part of [#963](https://github.com/johanhaleby/occurrent/issues/963).
  * `Projections.recordingAppliedAppends(..)` no longer takes a replay check. Build the recording view from a projection id and a store, then call `ReplayAwareSubscriptions.listenForCatchup(projectionId, view)` on the model your subscription runs on before you subscribe, and schedule the view's `pollForClear()` so a clear that failed while a catch-up ran is retried. An append handled while a clear is still owed waits for that clear, up to a thousand of them, and past that the oldest are dropped with a warning and their waits time out. `@Projection(recordAppliedAppends = true)` does the same registration for you, always on the model that projection's own subscription runs on, so a context holding two subscription models never has a projection listening to the one it does not run on.
  * Nothing is recorded while the projection is reading the history its replay set out to read, for a composition whose replay phase this can resolve. A catch-up also delivers the events written while it was reading that history, and those are recorded, because for some of them it is the only delivery they get. On the reactor stack that includes a write whose position was reserved before the replay read the head and that committed after it, which the live subscription then delivers again and records there.
  * A subscription model that does not send those two signals puts its projections on a polled fallback, which records nothing for a catch-up it sees and misses one that starts and finishes between two polls entirely. Implement `ReplayAwareSubscriptions.listenForCatchup(subscriptionId, listener)` if your own model catches up. The reactor stack logs a warning once when it cannot see a catch-up capability at all, and the blocking stack stays silent by design, since it cannot tell that apart from a composition that never replays.
  * An event-store projection left on the default start position on Occurrent's own shipped Mongo composition, an explicit `NOW`, or otherwise wired so its composition never replays, gets a startup `WARN` naming it, since its recorded memberships then survive a rebuild until the TTL evicts them or an operator clears them. A composition of your own stays silent instead of guessing, including one that replaces `Subscriptions`, `StreamSubscriptions` or `DcbSubscriptions`.
* **`CatchupThenPushSubscriptionModel` de-duplicates the replay-to-live overlap by a CloudEvent's id and source together, not by its id alone.** Two producers can each send an event with id `1`, and the replay delivering one of them no longer stops the other from being delivered live. Both stacks. Part of [#1041](https://github.com/johanhaleby/occurrent/issues/1041).
* **`goLive()` after `stopCatchUp()` on a `CatchupProjectionFeed` or `DomainEventFeed` delivers the live copy of an event the stopped replay delivered, rather than skipping it as a duplicate.** A view that buffers during a replay throws that buffer away on a stop, so the skipped event was lost for good. A view that wrote the event straight through receives it twice, which the at-least-once delivery `goLive()` documents allows. On the reactor stack, `goLive()` after a finished `catchUp()` also keeps the feed accepting events, where it used to make it refuse every event from then on. A `catchUp()` on a feed that is already live holds live events back until its replay ends and delivers them whether that replay completes or is stopped, so a stop no longer throws away events that arrived while it ran. On the reactor stack that feed also goes on delivering after the stop, where it used to drop every later event. Part of [#1041](https://github.com/johanhaleby/occurrent/issues/1041) and resolves [#974](https://github.com/johanhaleby/occurrent/issues/974).
* **A reactive `@Projection(source = PUSH)` fed by a `DomainEventFeed` and materializing into a `MaterializedView` now updates its read model with each live event's real `EventMetadata`, not `EventMetadata.empty()`.** The registrar routed that one combination through `Projections.reactiveUpdate(MaterializedView)`, which discards metadata by design for a caller with none to give, so a projection keyed on the stream id or reading `appendid` never worked there. It now goes through the metadata-aware `reactiveUpdateWithMetadata` builder, the same one every other reactive registration path already used. Part of [#740](https://github.com/johanhaleby/occurrent/issues/740).
* **`updateEvent` no longer lets a replacement event an update function builds from scratch silently lose or forge a value the store owns, on every event store Occurrent ships.** Those values are `position`, the indexed `dcbTags` array and its `dcbtags` extension, and `streamId` and `streamVersion`. `DcbCloudEvents` gains `preserveTags(original, updated)`, and `OccurrentCloudEventExtension` gains `preservePosition(original, updated)` and `preserveStreamIdentity(original, updated)`, all three mirroring the existing `preserveAppendId(original, updated)`. `updateEvent` calls all four on every store before returning, so the returned event and the stored document always agree, and a replacement event cannot change which stream, version, position or DCB tags a stored event has. Fixes [#876](https://github.com/johanhaleby/occurrent/issues/876), [#904](https://github.com/johanhaleby/occurrent/issues/904), [#905](https://github.com/johanhaleby/occurrent/issues/905), [#906](https://github.com/johanhaleby/occurrent/issues/906) and [#927](https://github.com/johanhaleby/occurrent/issues/927).
  * On the three MongoDB stores, `updateEvent` rebuilt the stored document through the stream-only mapper, which routes `position` through the general CloudEvent extension writer. That writer has no `Long` overload, so an updated event's `position` came back as a BSON string instead of an int64, dropping it from a numeric position query or a position-based catch-up, and the indexed `dcbTags` array was never written back at all. The `CloudEvent` handed back to the caller was never corrected either, so a caller trusting it instead of re-reading could see a value that disagreed with what was stored.
  * `InMemoryEventStore.updateEvent` had the same defect from a different cause. It only reapplied the append id, so it dropped `position` and `streamId`/`streamVersion` outright, and a stream missing those on its last event makes `read()` throw a `NullPointerException` rather than merely serve a stale value. It also accepted a forged `streamId`, `streamVersion`, `position` or DCB tag from an update function verbatim, so `read(originalStream).version()` could answer a forged number, the exact value stream-level optimistic concurrency depends on.
  * The TCK checks this now too, the stream identity and position half in `EventStoreOperationsConformance` and the DCB tag half in `DcbStreamInteropConformance`, so any store extending those suites is checked against it.
* **The new `occurrent-eventstore-mongodb-update-event-repair` module repairs events that `updateEvent` damaged before this release, and a position-writing store warns at startup when it finds any.** The repair rebuilds `position` from the string still in the document and the `dcbTags` array from the `dcbtags` extension, reusing the store's own mappers so a repaired event is what a running store would have written. The startup check runs one query, the one an index answers for free, so a silent startup rules out a damaged position rather than every kind of damage, and it reads no index keys at all on a store that was never damaged. Repair fixes documents, not the checkpoint of a consumer that already read past a repaired position, so `UpdateEventRepairResult` also holds the lowest and highest `position` a run restored, and the repair runbook's new step 7 uses that range to say which consumers to check and how to recover them. A run checkpoints that range before it touches a batch as well as after, so a killed and resumed run still reports every position it repaired, though it can also name a position it turns out not to have. See [the repair runbook](doc/runbooks/update-event-repair.md) and section 10 of [Upgrading to 0.34.0](doc/migration/upgrading-to-0.34.0.md).
  * It is not a full recovery. An update function that returned a replacement event built from scratch could destroy the only copy of a value, and the repair reports those events by `_id` rather than inventing one, whether the position was never stored, is already held by another event, is not a number, or is a number no store ever assigned. That count is of events rather than of findings, since one document can be wrong in two ways at once, and it is exact only for a run that never had to resume, an upper bound on a run that did. Events left with no position at all are counted separately, asked of the collection when the run finishes rather than tallied as it goes, so a run that was killed part way cannot report a clean collection while a position is still gone.
  * Every writer has to be on this release before the repair starts, because it walks the collection once and an older instance can damage an event it has already passed. A position it does restore is the value the document still holds and not one it can check, since the old write-back preserved whatever position the update function returned, so a caller whose update functions set `position` themselves needs an external record rather than a clean run. Damage that removed the `dcbtags` extension outright leaves a document that no longer looks like a DCB event, which nothing can find.
* **`EventStoreConfig.Builder.requireRepairedEvents(true)` makes a MongoDB event store refuse to start while its event collection holds events that `updateEvent` damaged in Occurrent 0.33.0 or earlier.** Such an event is missing from the conflict query behind a conditional append, so an append that should have been refused is accepted for as long as the damage is there. Turn it on when you would rather have the application down than let that keep happening until the repair has run. It is off by default on the native, Spring blocking and Spring reactor stores, which warn and start. See section 4 of [ADR 136](doc/architecture/decisions/0136-a-store-reports-damage-it-caused-and-never-repairs-it-by-itself.md). Resolves [#973](https://github.com/johanhaleby/occurrent/issues/973).
* **`ReactorMongoEventStore.updateEvent` no longer logs the full found document, including its data payload, at INFO before applying the update function, fixing behaviour shipped in 0.33.0.** That is the one operation `EventStoreOperations`'s javadoc documents as mainly useful for GDPR-style redaction, so logging the whole document there worked against the reason a caller uses it. Fixes [#907](https://github.com/johanhaleby/occurrent/issues/907).
* **A saga instance that keeps failing can now be quarantined on the event it stopped on rather than left active and failing indefinitely.** A saga has one subscription, and up to 0.33.0 an event that `evolve`, `react`, or a command dispatcher could not handle was rethrown to the subscription model on every delivery, with no limit. What the failing event holds up in the meantime is decided by whatever feeds the subscription, and the javadoc on `SagaStatus.QUARANTINED` says what that can be. Where a quarantine budget is in force, the executor can now record when an instance started failing on an event and keeps rethrowing while that has lasted less than `SagaRunnerConfig.quarantineAfter`, five minutes by default. Past that, the instance can move to the new `SagaStatus.QUARANTINED` on whichever event it is failing on then, and the javadoc on that constant lists what else has to hold. A quarantined instance stops receiving events and stops firing timers. The clock belongs to the instance rather than to one event, so an instance where two events both fail keeps the instant it started failing and `SagaFailure.firstFailedAt()` is the start of that run rather than the first time one event failed. Resolves [#818](https://github.com/johanhaleby/occurrent/issues/818) and [#997](https://github.com/johanhaleby/occurrent/issues/997).
  * The budget covers everything that happens to an event once the saga knows which instance it belongs to, not the reaction alone. Checking for a redelivery, `evolve`, `react`, the dispatcher and the store all count, and an `Error` counts like a `RuntimeException`. The one exclusion is `OutOfMemoryError`, because that says the JVM ran out of heap while some instance held the thread rather than anything about the instance, and nothing in this release brings an instance back out of quarantine.
  * An event that fails before the saga can say which instance it belongs to, because the converter or the id extractor threw, is never let past, whatever the budget. It may still belong to an instance, and once the subscription moved past it the next event for that instance would mark the instance as having handled it, so the event would be lost. It is refused on every redelivery instead, as in 0.33.0. The first failure is logged at WARN and after that at ERROR once per interval, naming the event and what stopped it, the event's redelivery key when it has one and otherwise its CloudEvent id and source. That interval is the quarantine budget when the saga has one, and a fixed five-minute default when it does not, so the ERROR still repeats on a subscription model this saga cannot quarantine anything on, such as a push feed, including one a broker bridge feeds, for as long as that model keeps offering the event. A model that does not offer a refused delivery again gets only the first WARN, and `DeliveryFailurePolicy` is where a consume-side broker bridge's choice is configured. Where the event is offered again, repair the converter or the id extractor and the saga applies it in the order it was written, with nothing to feed to it again. Resolves [#1042](https://github.com/johanhaleby/occurrent/issues/1042) and [#1061](https://github.com/johanhaleby/occurrent/issues/1061).
  * Find the stopped instances with `SagaInstances.findByStatus(QUARANTINED, ..)` and read `SagaInstance.failure()` for the input it stopped on, when it started failing, and the exception. Nothing in this release brings an instance back out of quarantine, so `SagaStateStore.delete(sagaId)` is how you abandon one. [The quarantined saga runbook](doc/runbooks/quarantined-saga-instance.md) is the full sequence for finding one, reading it, and deciding what to do with it.
  * An instance whose state can no longer be decoded, after a renamed event class or a converter change, is quarantined like any other. `SagaStateStore` gains two `default` methods for it, `findWithoutState` and `compareAndSaveWithoutState`, so the executor can decide and record a quarantine without the state and the state the instance stopped on survives the quarantine. Your own store keeps compiling and keeps its current behaviour, and overriding both is what gives it this. `SpringMongoSagaStateStore` overrides both, and `SagaInstances.find(sagaId)` reads this way too, so looking one instance up by id costs what enumerating them costs and answers for an instance whose state no longer decodes.
  * On the annotation path each `@Saga` now publishes its running `SagaSubscription` as `sagaSubscription-<id>`, next to the `sagaInstances-<id>` it already published, and `occurrent.saga.quarantine-after` sets the budget, where zero keeps the 0.33.0 behaviour. Set `quarantineAfter` to `null` to keep that behaviour everywhere.
  * Quarantine needs the failing event to be obtainable a second time, so before an instance is quarantined the runner asks the subscription model whether it still holds that event, through the new `HistoryRetainingSubscriptions`. The MongoDB subscription models hold everything they deliver, and a wrapper that declares nothing itself is answered by the model it wraps, so a catch-up model over one of them does too. No push model can promise it, so a saga on a push feed keeps the 0.33.0 behaviour of never quarantining and is told so at startup. A `PushSubscriptionModel` leaves the acknowledge-or-redeliver decision to the listener that called `accept`. What a failing event then holds up is that listener's call as well. A `CatchupThenPushSubscriptionModel` can say whether it holds any given event, by looking in the event store it replays from, and that is still not enough, because a quarantined instance skips everything addressed to it afterwards and skipping acknowledges, so protecting the failing event would leave the ones behind it unprotected. The per-event answer is asked on top of the promise, so a model promising wrongly is caught on the event it is about to acknowledge. See [#952](https://github.com/johanhaleby/occurrent/issues/952).
* **A saga timer reaction that throws an `Error` no longer costs the instances after it in that poll their turn, which fixes behaviour the saga timer poller shipped in 0.32.0.** The poller caught `RuntimeException` per instance and `Throwable` around the whole poll, so a `StackOverflowError` out of a recursive `evolve`, or a `NoClassDefFoundError` out of a reaction touching a class that will not load, abandoned the rest of that poll's batch. Every instance after the failing one lost its turn, on every poll, for as long as it kept failing. An `OutOfMemoryError` still abandons the poll, because it says nothing about whichever instance held the thread when the heap ran out, which is the same rule the event path applies through `SagaExecutionSupport.isAttributableToTheInstance`.
  * Nothing isolates a failing instance from the saga's other ones. A poll fires at most `SagaRunnerConfig.timerBatchLimit` instances, a hundred by default, and nothing in `SagaStateStore.findWithDueTimers` requires a store to give a different instance a turn, so once that many instances cannot fire their timers the saga can stop firing timers altogether. That many, not more, since a full batch of them leaves no place for anything else. The javadoc on `SagaRunner`, `SagaExecution`, `SagaStateStore`, `SagaStateStoreQueries`, `SagaConcurrencyException` and the `@Saga` annotation said otherwise and has been corrected, as have ADR 128 and ADR 134. [#1003](https://github.com/johanhaleby/occurrent/issues/1003) is where that is being fixed, and it owns the invariant [#998](https://github.com/johanhaleby/occurrent/issues/998) states rather than this entry claiming it.
* **A catch-up that stops on a checked exception from a replayed handler now records that failure, which fixes behaviour shipped in 0.33.0.** Java will not let a handler throw a checked exception it has not declared, but Kotlin will. The blocking catch-up recorded a failed replay only for a `RuntimeException` or an `Error`, so after a checked exception it went on buffering live events for a replay that had already stopped, and `accept(...)` returned normally for each of them until the buffer was full, 100,000 events by default, so whatever fed it treated them as handled. The exception still reached a caller that waited for the catch-up. What was missing was the record of it, and it now refuses every live event after such a failure, the same as after any other one. This covers `CatchupThenPushSubscriptionModel`, the blocking `CatchupProjectionFeed` and `DomainEventFeed`, and `@Projection` and `@Saga` with `source = PUSH`. No change is needed on your side. Resolves [#1093](https://github.com/johanhaleby/occurrent/issues/1093).
  * `CatchupThenPushSubscriptionModel.isCatchingUp(id)` answers `false` once such a replay has failed, and the failure is logged. Before, it kept answering `true` and nothing was logged.
  * A `@Projection(source = PUSH)` fed by a `DomainEventFeed` under `startupMode = BACKGROUND` logs such a failure and reports it as failed in `PushCatchupStatus`. Before, it stayed catching up there for good.
  * A `ReplayAware` view whose `replayAbandoned()` throws a checked exception no longer stops a failed replay from being reported, on either stack. On the blocking stack the failure went unrecorded, and on the reactor stack the catch-up never completed.

* **A live event buffered while a reactor catch-up replay runs is now dropped when that replay is stopped before the handover goes live, where 0.33.0 applied it once the next catch-up went live.** This covers the reactor `CatchupThenPushSubscriptionModel` and `CatchupProjectionFeed`. The `Mono` that `CatchupProjectionFeed.accept(..)` returned for such an event now errors with an `IllegalStateException`, so a listener that acknowledges on completion does not acknowledge it and the broker delivers it again. Redeliver an event reported as `RoutingOutcome.DEFERRED`, which the reactor `PushSubscriptionModel.acceptRedeliverable(..)` returns while a `CatchupThenPushSubscriptionModel` in front is replaying, and `DomainEventFeed.acceptCloudEvent(..)` returns for a projection feed. A dropped event also frees its place in the live buffer. In 0.33.0 it kept that place, so repeated stops could fill the buffer, and every new event was then refused until a catch-up went live. Resolves [#1130](https://github.com/johanhaleby/occurrent/issues/1130).
* **`PushSubscriptionModel.acceptRedeliverable(CloudEvent)` returns the event's `RoutingOutcome`, and the reactor `PushSubscriptionModel` has a new `acceptRedeliverable(CloudEvent)` that returns a `Mono<RoutingOutcome>`, so a broker listener acknowledges on the outcome.** Acknowledge the message on `DELIVERED` or `FILTERED`, stop consuming on `REFUSED`, apply your delivery failure policy on `NOT_DELIVERABLE`, and let the broker deliver it again on `DEFERRED` or `UNAVAILABLE`. `accept(CloudEvent)` is unchanged, and it returns normally for an event fed before any subscription is registered, while the model is stopped, or while the subscription is paused, so a broker listener that acknowledged once it returned lost those events. Resolves [#1138](https://github.com/johanhaleby/occurrent/issues/1138).
  * An event that reaches no subscription returns `UNAVAILABLE`. An event that arrives while a `CatchupThenPushSubscriptionModel` in front is still replaying returns `DEFERRED`, because the catch-up refuses it instead of buffering it, and a delivery after the catch-up has gone live applies it. Leave either message unacknowledged rather than parking it.
  * A filter failure and a handler failure throw on the blocking stack and error on the reactor stack, the same way they do for `accept(..)`.
  * A refusal decided before the event reaches a handler comes back as the outcome instead, where `accept(..)` throws or errors. Once a catch-up has failed every event returns `REFUSED`, which means stop consuming, since delivering it again gets the same refusal. A full live buffer on the reactor stack returns `NOT_DELIVERABLE`, which belongs in your delivery failure policy.
  * With a `CatchupThenPushSubscriptionModel` in front, the two stacks differ for an event whose earlier delivery is still running. The reactor model waits behind that delivery and then routes the event as a new delivery, which returns `DELIVERED` straight away when the earlier one applied it. The blocking model returns `DEFERRED`.
  * `RabbitMqCloudEventBridge` and `KafkaCloudEventBridge` act on the returned outcome alone. `REFUSED` stops the bridge, `NOT_DELIVERABLE` and a filter or handler failure go to the configured delivery failure policy, and `DEFERRED` and `UNAVAILABLE` are delivered again without it.
  * Fed from the event store's write path, neither push model records which events a subscription has handled, so an event is lost when the application crashes after the write commits but before the handler runs. Use a durable subscription if that is not acceptable. The javadoc of both `PushSubscriptionModel` classes and both `CatchupThenPushSubscriptionModel` classes now says so, and points a broker listener at `acceptRedeliverable(..)` instead of `accept(..)`.

* **A reactor catch-up subscription now delivers an event a second time when a write that was still in flight during the replay was read by a history window, which changes behaviour the reactive catch-up models shipped in 0.30.0.** A position is reserved before its write commits, so a write in flight when the replay read the head holds a position at or below that head and a history window reads it even though it is not history. The replay used to put every id it read into the cache the live delivery filters on, the history reads included, so the change stream's own delivery of that event was dropped for as long as its id stayed in the cache, which for an event this close to the head was long enough. The history windows now fill no cache, which is what all three blocking paths already did. Catch-up delivery is at-least-once either way. Resolves [#891](https://github.com/johanhaleby/occurrent/issues/891). Reasoning in [ADR 135](doc/architecture/decisions/0135-the-reactive-handover-dedup-is-fed-only-by-the-reconciliation-read.md).
  * On a single-primary MongoDB, the case [ADR 135](doc/architecture/decisions/0135-the-reactive-handover-dedup-is-fed-only-by-the-reconciliation-read.md) verifies, the second delivery goes to an event a history window read whose write committed after the catch-up took its live resume checkpoint, and a store with nothing written during the replay sees none at all.
  * Where that checkpoint can lag what is already committed, under a secondary read preference, a sharded `mongos`, or a `CheckpointAwareSubscriptionModel` of your own, the live stream delivers pre-replay history as well and nothing suppresses that any more, so the repeats reach as far back as the checkpoint lags.
  * `handoverCacheSize` now sizes the reconciliation overlap alone, which is what `cacheSize` on the blocking position catch-up models already means. The catch-up-then-push handover is a separate engine with a setting of its own, `CatchupThenLiveOptions.dedupCacheSize`, described with `AppliedAppendStore` above.
* **The blocking and reactor catch-up subscription models now deliver a live event that shares only its id with a replayed one, fixing behaviour shipped in 0.33.0.** CloudEvents only promises that `id` and `source` together are unique, so an event store can hold two events with the same id from different producers. `StreamCatchupSubscriptionModel`, `DcbCatchupSubscriptionModel` and their reactor equivalents keyed the replay-to-live overlap cache by id alone, so a live event from one producer was dropped whenever a replayed event from another producer had already used the same id. The cache is now keyed by `id` and `source` together, an internal change with no public API affected. Catch-up delivery is at-least-once either way, and this only ever delivers an event that was previously dropped, so there is no migration guide. Resolves [#1048](https://github.com/johanhaleby/occurrent/issues/1048).
* **`DurableSubscriptionModel` now records a fresh subscription's start position before anything is delivered, and refuses the subscription when it cannot.** A subscription that asks for the model default with no checkpoint stored is recorded from the wrapped model's `globalCheckpoint()` inside `subscribe(..)` itself. When that answer is `null`, which `NativeMongoSubscriptionModel` and `SpringMongoSubscriptionModel` both give when the server refuses the `hostInfo` command, on a shared MongoDB Atlas cluster for example, `subscribe(..)` throws `IllegalStateException` instead of starting without a recorded position. Up to 0.33.0 such a subscription started anyway with nothing in checkpoint storage, so a crash after a failed first delivery started over from wherever the feed had reached and the failed event was never redelivered. A subscription with a checkpoint already stored, and one subscribing with a `StartAt` of its own, are unaffected. See [section 7 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#7-durablesubscriptionmodel-refuses-a-first-subscription-when-no-start-position-can-be-recorded). Resolves [#852](https://github.com/johanhaleby/occurrent/issues/852).
  * To start anyway, accepting that loss, configure `DurableSubscriptionModelConfig.startWhenNoStartPositionCanBeRecorded(true)`, or set `occurrent.subscription.start-when-no-start-position-can-be-recorded=true` in the Spring Boot starter. The same override on `ReactorDurableSubscriptionModelConfig` lets `ReactorDurableSubscriptionModel` start such a registration too, and the property reaches the reactive starter as well.
* **Two nodes racing to record a manually-started subscription's first checkpoint on MongoDB no longer risk losing the events between their two positions, and neither does a checkpoint deleted and re-registered while a registration was already under way.** `CheckpointStorage` gains an opt-in `resolveFirstCheckpointRace`, which the native and Spring MongoDB storages implement by comparing the operation time a stored and a candidate checkpoint each hold, atomically with the write that comparison calls for. `ManualStartSubscriptionModel` and `ReactorDurableSubscriptionModel` both reach for it before falling back to the narrower, write-order-based rule 0.33.0 shipped, so on MongoDB the earlier of the two positions now governs regardless of which write reached storage first. A storage with no ordering to compare by, Redis and the in-memory storage included, keeps the narrower rule exactly as it was. See [ADR 130](doc/architecture/decisions/0130-a-subscriptions-first-position-race-resolves-by-order-not-by-write-order.md). Resolves [#771](https://github.com/johanhaleby/occurrent/issues/771).
* **A flow saga's `stepWindow` now counts and evicts only the events its steps declare, plus an event of the type that starts the flow, fixing behaviour shipped in 0.33.0.** An event of a type no step names and that is not the start type, reachable through a `replacementFilter` wider than the flow's own types or through a collapsing `CloudEventTypeMapper`, used to take one of `stepWindow`'s slots and could evict one of the step's own events to make room. It is still retained when it arrives, but it does not count against the cap or evict a declared event. `stepWindow` drops it only together with a declared event that arrived after it, and `historyWindow` can also drop it on a later transition, once the step it arrived in has been left. A step fed only such events grows with no cap from `stepWindow` at all, so the store-boundary warning added in 0.33.0 is what surfaces that growth instead. A correlation id that keeps sending an event of the start type does not grow an instance without bound, since each retained one fills the cap and gets evicted like any of a step's own events. See [ADR 129](doc/architecture/decisions/0129-a-flow-sagas-stepwindow-caps-only-its-own-declared-events.md) and, if you set a wide filter or a collapsing mapper, [section 2 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#2-a-flow-sagas-stepwindow-now-caps-its-declared-events-and-the-start-type). Resolves [#773](https://github.com/johanhaleby/occurrent/issues/773), [#764](https://github.com/johanhaleby/occurrent/issues/764) and [#1047](https://github.com/johanhaleby/occurrent/issues/1047).
* **Four subscription lifecycle-leak fixes in the blocking competing-consumer, durable, and catch-up subscription models.** `CompetingConsumerSubscriptionModel` no longer strands a lease when its delegate refuses to pause, so a node whose catch-up already failed still lets another node take over. `DurableSubscriptionModel` no longer skips checkpoint-based resume for a subscription that opts out once and later resubscribes as managed. In `StreamCatchupSubscriptionModel` and the DCB catch-up model, cancelling a subscription and resubscribing it right away no longer risks the cancelled replay going live anyway, the new subscription looking cancelled instead, or a stale in-flight event recreating the checkpoint the cancellation just deleted. Resolves [#737](https://github.com/johanhaleby/occurrent/issues/737).
...
* **A projection, a subscription built through the subscription DSL, a query through `DomainEventQueries`, a `@Snapshot` view, `ExecuteFilter`'s `type` and `includeTypes`, and `DcbCriteriaBuilder`'s `type` and `types` now each expand a declared sealed event type into the concrete types it permits, the same fix 0.33.0 already shipped for a saga and an annotation-based subscription.** Declaring a sealed supertype used to derive a filter naming only that supertype's own CloudEvent type, so each of those silently matched fewer events than dispatch would actually accept. All six now enforce the same `EventTypeExpansion.expand` rule, the four DSLs through the shared `deriveFilter` and the other two directly, since neither derives a plain `Filter`. `DomainEventQueries.query(Collection)` keeps treating a null or empty collection as "match nothing" rather than "match everything". See [ADR 126](doc/architecture/decisions/0126-every-derived-event-type-filter-expands-a-declared-sealed-type.md). Resolves [#750](https://github.com/johanhaleby/occurrent/issues/750), [#758](https://github.com/johanhaleby/occurrent/issues/758), [#912](https://github.com/johanhaleby/occurrent/issues/912), [#939](https://github.com/johanhaleby/occurrent/issues/939) and [#940](https://github.com/johanhaleby/occurrent/issues/940).
  * Sealing is not the only way a hierarchy is closed. An enum's constants are every class an instance of it can have, which covers an `enum class` whose constants have bodies and a sealed event interface above one. javac seals that construct and lists each constant body in its `permits` clause (JLS 8.9), so the walk already found those. Kotlin seals nothing, so its enum is read through its constants instead, and only that one construct is, because reading an enum runs its constructors and static initializers. `ExecuteFilter.excludeTypes` finds the same constants, so excluding such an enum keeps out each of them rather than only the enum's own CloudEvent type.
  * Which CloudEvent type each constant is stored under is decided by the constant's own class. One with a body has its own class, `PaymentEvent$Reserved`, and one without is an instance of the enum class itself, so adding or removing a constant body changes the type an event is stored under. See [section 3 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#3-declaring-an-event-type-whose-concrete-subtypes-cannot-be-found-is-refused).
* **`ExecuteFilter.excludeTypes` now widens a declared type it cannot fully expand to every concrete subtype a downward walk can find, instead of naming only the declared type's own CloudEvent type.** Excluding a supertype has to exclude everything under it, since a narrower exclusion would silently read events the caller asked to keep out, so `excludeTypes(SensitiveEvent.class)` on a sealed `SensitiveEvent` now keeps out every concrete subtype it permits. That is the opposite treatment from `type` and `includeTypes` above, on purpose. Those refuse a declaration they cannot fully expand, since a narrower inclusion would silently miss events dispatch would accept, while widening an exclusion can only exclude more. See `EventTypeExpansion.expandWhatCanBeFound`. Resolves [#912](https://github.com/johanhaleby/occurrent/issues/912).
  * The walk starts at the declared type, follows a `permits` clause through `Class.getPermittedSubclasses`, reads the constants of an enum that has no such clause, and stops at the first level that is neither sealed nor an enum, so a type it cannot reach means an event the caller wanted out stays in.
  * `excludeTypes` refuses a declared type that is itself a non-sealed interface or abstract class the walk cannot descend into at all, since the exclusion would then name only that type's own CloudEvent type, which no event is ever stored under, so it would silently exclude nothing while looking like a working exclusion. A sealed declared type that reopens further down is unaffected and still widens as above.
  * Two cases are still incomplete. A concrete class declared directly that is neither final nor sealed contributes itself, so the exclusion still does something, just without reaching that class's own unsealed subclasses. An interface or an abstract class whose hierarchy reopens before the walk finds anything concrete contributes its own declared name and nothing else, and how much that excludes is your `CloudEventTypeGetter`'s decision rather than the walk's.
  * Under a getter that maps each type to its own class name, which is what `ReflectionCloudEventTypeMapper` does in both forms, no stored event is written under that name, so such a declaration excludes zero real events, silently, exactly as before this fix. Under a `CloudEventTypeMapper` of your own that maps a whole hierarchy onto one type string, the same declaration excludes the whole family, since that one string is what the concrete events are stored under. Seal the hierarchy or declare the concrete types for an exclusion that does not depend on which of those two your mapper is.
* **A `@Projection`, `@Saga` or `@Snapshot` factory method no longer fails Spring Boot startup when its bean is a JDK interface proxy, and refuses instead of running a different implementation's factory when a prototype-scoped factory bean built a different class than the one the scan recorded.** The registrar used to invoke the factory method on whatever bean the context resolved, which could be a proxy implementing only the bean's interfaces rather than the concrete class the method is declared on, once that bean was advised under `spring.aop.proxy-target-class=false`. Invoking the method there threw `IllegalArgumentException`. `SubscriptionAnnotations.invokeDescriptorFactory` now unwraps the bean to its ultimate AOP target and runs the recorded factory method there only when the target's own class equals the class the scan found that method on, shared by all five registrar paths that had this call. When it does not, the factory refuses with `IllegalStateException` naming the annotation, the factory, and both causes that can produce a mismatch. A proxy backed by a prototype- or pool-scoped target source that cannot be unwrapped safely is one. A prototype-scoped factory bean that hands back a different implementation on a later call, an unrelated class or a subclass overriding the method either one, so the target the second call resolves is not the exact class the scan recorded the method from, is the other. See [ADR 127 section 4](doc/architecture/decisions/0127-a-subscription-is-a-descriptor-and-the-annotation-stops-naming-the-concept.md#4-a-descriptor-annotation-is-read-after-the-singletons-are-instantiated). Fixes [#836](https://github.com/johanhaleby/occurrent/issues/836).
* **`@Transactional`, or any other class- or method-level Spring aspect, on a `@Subscription`, `@StreamSubscription`, `@DcbSubscription` or `@SynchronousSubscription` handler method applies to every delivery, including one delivered while Spring Boot is still starting up.** Each handler registers from the same phase `@Projection`, `@Snapshot` and `@Saga` already use, once every singleton exists, and is resolved once against the bean's fully proxied instance rather than looked up again for each delivery. A handler on a bean Spring Boot has not built by then, an uncreated `@Lazy` bean for example, registers when the bean is built instead, and the two sentences above describe the handlers that registered at startup rather than that one. It looks its bean up per delivery and never waits for its replay, whatever `startupMode` says, so its deliveries reach the published bean. When one of that bean's handlers is refused, for asking for both `startAt` and `startAtGlobalPosition` for example, none of its handlers subscribes, and asking for the bean again tries every one of them again. A handler only runs on an object that runs the method Occurrent read it from, so a prototype whose factory returns a different class on a later call, an unrelated class or a subclass overriding the method, fails with `SubscriptionHandlerNotInvocableException` instead of running that class's code under the handler's subscription id. A bean behind a proxy that looks up its target on each call, a scoped proxy for example, is the exception, since Occurrent can't see which object that proxy will call. A replay running on its own thread can still deliver while the bean is being built, and a delivery there misses advice a `BeanPostProcessor` registered after Occurrent's own would have added. A proxy that only becomes unable to reach the handler method once such a post processor has wrapped the bean fails on the first delivery rather than at startup, since no proxy like that exists yet when the handler registers. A handler method the resulting proxy cannot invoke, a JDK interface proxy missing the method or a CGLIB proxy unable to override a final method, fails Spring Boot startup instead of running with no advice. See Breaking changes below. Resolves [#837](https://github.com/johanhaleby/occurrent/issues/837) and [#965](https://github.com/johanhaleby/occurrent/issues/965).
* **A `@Projection`, `@Saga` or `@Snapshot` on a bean Spring Boot has not built by the time startup finishes now registers when the bean is built, where before it never registered at all, fixing behaviour shipped in 0.33.0.** This hits a `@Lazy` bean, and a `FactoryBean` product nothing has asked for yet, when the `@Bean` factory method declares an interface and the annotated method is only on the class behind it. Resolves [#981](https://github.com/johanhaleby/occurrent/issues/981).
* **A `@Projection` or `@Saga` with `source = PUSH` started through `ManualStartPushSources` after the application context has begun closing now starts nothing that outlives the context, either by not starting it at all or by stopping what it started.** A saga left its timer poller dispatching commands into a context that had already shut down, and a projection could start a catch-up replay against a store that was being disposed. A projection fed by a `DomainEventFeed` is refused before it reaches the feed, because registering on a feed is what puts the feed into buffering mode and a feed has no way to unregister, so a registration that survived a refused start would buffer every live event into a bounded buffer nothing will ever drain, and eventually overflow it into your own publish path. With `catchup = NONE` the projection or saga subscribes straight onto the `PushSubscriptionModel` bean you supplied, which the framework does not own and so never shuts down. The subscription that registration added is now cancelled on the way out rather than left behind, where before it went on handling pushed events, and for a `@Saga` went on issuing commands, after the context had gone. This part applies under `occurrent.subscription.mode = auto` as well, since the same check runs on the startup path. It only shows when that bean outlives the context, because Spring otherwise calls the model's own `shutdown()` as an inferred destroy method and drops every registration with it. The list `startAll()` returns names only the push sources the call actually brought up, so a refused one is left out rather than reported as started, which is what a readiness probe or a leader-election handover reads that list for. Under `startupMode = BACKGROUND` the replay runs on a thread the starter owns and the call returns without waiting for it, so an id in that list means the replay is running rather than finished. A projection the close refused before that thread started is left out like every other refusal. A refused id is gone from `pendingIds()` too. The context that refused it never reopens, and the withheld work may already have subscribed or registered before it read the shutdown flag, so keeping the id pending would offer a retry that repeats that work rather than resuming it. One case is left. A start that read the shutdown flag in the moment before the close wrote it can still register afterwards, and [#1032](https://github.com/johanhaleby/occurrent/issues/1032) is the path to closing that. Fixes behaviour shipped in 0.33.0. Resolves [#988](https://github.com/johanhaleby/occurrent/issues/988), [#1020](https://github.com/johanhaleby/occurrent/issues/1020), [#1027](https://github.com/johanhaleby/occurrent/issues/1027), [#1034](https://github.com/johanhaleby/occurrent/issues/1034) and [#1036](https://github.com/johanhaleby/occurrent/issues/1036).
* **A `@Projection(source = PUSH)` fed by a `DomainEventFeed` the starter can identify without creating it no longer fails startup under an id-sensitive `CheckpointStorage`, fixing behaviour shipped in 0.33.0.** The starter used to ask about every push projection's id once `catchup` defaulted to `FROM_EVENT_STORE`, even for a `DomainEventFeed`-fed projection, which never resolves `CheckpointStorage` at all. A storage refusing that kind of id, `SpringRedisCheckpointStorage`'s Cluster-safe mode for example, could sink startup for a projection that would never have written such a checkpoint. A feed bean the starter cannot type read-only stays checked, as before. See [section 8 of the 0.33.0 upgrade guide](doc/migration/upgrading-to-0.33.0.md#8-three-ways-the-spring-boot-starter-now-refuses-to-start). Resolves [#788](https://github.com/johanhaleby/occurrent/issues/788).
* **A flow saga instance waiting in a step that was since renamed or removed now refuses the delivery with a clear `IllegalStateException`, instead of a bare `NullPointerException`, fixing behaviour shipped in 0.33.0.** The message names the step and offers two ways out. Put the step back, or add a temporary step under the old name that transitions its instances onward until every waiting instance has moved past it. Or delete the stuck instance from the `SagaStateStore`. See [ADR 128](doc/architecture/decisions/0128-a-renamed-or-removed-step-refuses-its-parked-instances.md). Resolves [#748](https://github.com/johanhaleby/occurrent/issues/748).
...
* **A registration-time position read that fails now logs at `WARN` instead of `ERROR` in `ReactorDurableSubscriptionModel`, and the refusal message for a position source answering nothing is sharper there and in `ManualStartSubscriptionModel`.** A checkpoint already stored, or a start position of its own, can still let the subscription start despite the failed read, so `ERROR` overstated it. The message no longer claims there is no position at all when a stored checkpoint is what settles the registration, and no longer says "while registering" for a read that can also happen when a stopped subscription starts. See [#804](https://github.com/johanhaleby/occurrent/issues/804) and [#805](https://github.com/johanhaleby/occurrent/issues/805).
* **Reading a stream with a `StreamReadFilter` and a nonzero skip now resumes from the correct stream position, on every event store Occurrent ships.** The native, Spring blocking, Spring reactive and in-memory stores applied skip after the filter had already narrowed the result, so it counted matching events instead of stream positions, and `ApplicationService#execute` with both `ExecuteOptions.fromStreamVersion(n)` and a filter set could silently resume from the wrong place. A negative skip is rejected with an `IllegalArgumentException` on every store, enforced by the shared conformance suite. Fixes [#810](https://github.com/johanhaleby/occurrent/issues/810).
* **The native MongoDB store no longer leaks a `MongoCursor` when a stream read's `Stream` is closed early**, including through Kotlin's `.use { }`. The Spring store already closed correctly here. Fixes [#810](https://github.com/johanhaleby/occurrent/issues/810).
...
* **`RetryStrategy.execute(..)` takes a shutdown predicate, so a component that is closing down can stop a retry loop that is already running.** Pass it as a second argument, `retryStrategy.execute(runnable, __ -> running)` for example, and the loop stops and rethrows the last failure as soon as the predicate answers `false`. The predicate is read before every retry, and read again every 50 ms while a backoff is being slept out, so a shutdown requested one millisecond into a long backoff is caught at the next read instead of after the whole sleep. That is a polling cadence, not a guaranteed deadline, since a single sleep call can run a little longer under ordinary scheduling delay. An interrupted sleep still restores the thread's interrupt flag before rethrowing. Because the predicate can run several times over one attempt, it must be a pure read with no side effects. The `execute(..)` overloads without a predicate are unchanged and keep retrying exactly as they did. Use this rather than putting a lifecycle flag in `retryIf(..)`, which is only read between attempts and replaces whatever predicate the strategy already had. One limitation. A `RetryStrategy` you implement yourself, rather than one built from `RetryStrategy.retry()` or `RetryStrategy.none()`, runs a retry loop Occurrent cannot reach into, so the predicate is ignored there and the action runs exactly as it did before. The MongoDB competing consumer strategies and `InMemoryDeadlineConsumerRegistry` warn at startup when they are given such a strategy, since their shutdown then waits for whatever that strategy decides to do. Resolves [#866](https://github.com/johanhaleby/occurrent/issues/866).
* **`SpringMongoSagaStateStore` now retries a transient MongoDB error instead of letting it reach the saga.** Every read and write it makes is retried, and so are the two indexes it creates when it is built, with exponential backoff from 100 ms up to 2 seconds, giving up after 10 attempts and rethrowing the original failure. Before this, a MongoDB error that a second attempt would have cleared surfaced as a failed transition, which under the saga quarantine above spends the instance's time budget and can quarantine an instance that was never broken. You get this whether you build the store yourself or let the Spring Boot starter build it for a `@Saga` that declares none. Pass your own `RetryStrategy` to the new five-argument constructor to change it, or `RetryStrategy.none()` to have every failure reach you on the first attempt. Resolves [#1001](https://github.com/johanhaleby/occurrent/issues/1001).
* **`MigrateSubscriptionModeProperty_0_32`'s YAML halves no longer touch an unrelated profile's document in a multi-document `application.yml`.** They used `org.openrewrite.yaml.search.FindProperty` as a precondition, which OpenRewrite evaluates over the whole file rather than the one document that sets the property, so one profile satisfying the precondition applied the rewrite to every profile in the file. Two symptoms came out of that. A profile that never set `occurrent.subscription.mode` could lose its `occurrent.subscription.enabled` key, and with it the whole document when that key was all it had. And a profile whose own `enabled` held the other boolean got the raw boolean literal written under `occurrent.subscription.mode`, an enum (`disabled`, `manual`, `auto`) that does not bind to it. Fixes [#828](https://github.com/johanhaleby/occurrent/issues/828) and [#834](https://github.com/johanhaleby/occurrent/issues/834).
...
* **`DurableSubscriptionModel` now pins a subscription's first checkpoint with the same `ifAbsent` write, race-resolving fallback its `ManualStartSubscriptionModel` and `ReactorDurableSubscriptionModel` siblings already use, instead of reading the checkpoint and then saving over whatever it found.** Two nodes racing to record the same subscription's first position could previously have the later one silently overwrite the earlier, on a checkpoint storage able to evaluate write conditions but with no lease protecting the write. Every shipped composition already puts a lease outermost, so this is a consistency fix rather than a closed live race. See [ADR 130](doc/architecture/decisions/0130-a-subscriptions-first-position-race-resolves-by-order-not-by-write-order.md).
* **`Projection.builder()`, `Projection.singletonBuilder()`, `Saga.builder()` and `SnapshotView.builder()` gain a no-argument overload, and `View.create(BiFunction)` and `View.create(Fold)` gain one that takes no initial state**, the shape `FlowSaga.builder()` already had. Call it instead of passing `null` when a fold starts from no state, the common case since most folds build their first state in the handler for the first event. Your handlers and the type you get back see that state as `@Nullable S`, which Kotlin reads as `S?`. So in Kotlin a handler that uses the state without checking for `null`, or a variable declared as `View<String, String>`, fails to compile instead of throwing on the first event. Resolves [#956](https://github.com/johanhaleby/occurrent/issues/956).
  * Kotlin's `projection { }`, `singletonProjection { }`, `saga { }`, `snapshotView { }` and `view { }` gain the same no-argument form, matching the flow DSL's `saga { }`.

* **Closing one subscription while MongoDB is unreachable no longer hangs.** The MongoDB competing consumer strategies, `NativeMongoLeaseCompetingConsumerStrategy` and `SpringMongoLeaseCompetingConsumerStrategy`, retried the removal of their lease document without limit, so `close()` on a saga subscription or a competing consumer subscription never returned while the database was down, and every shutdown step ordered behind it never ran either. Unregistering and releasing a consumer now make at most 5 attempts, the cap a lease refresh round already used, and fewer than that when you configured fewer. A refresh round used to raise a configured limit below 5 up to 5, and no longer does. Nothing is lost by giving up, because the lease expires on its own after the configured lease time and any node can take the subscription over from then on. Registering is unchanged and still retries exactly as configured. Shutting the strategy down now also stops a call that is in the middle of a backoff rather than after it. Closing a saga subscription also survives a checked exception from the lease release, which a `RetryStrategy` configured with `mapError` can produce and the retry loop rethrows unwrapped. One of those used to escape `close()` and skip the timer poller shutdown. Resolves [#999](https://github.com/johanhaleby/occurrent/issues/999).
* **`InMemoryDeadlineConsumerRegistry` keeps the retry predicate you configure, and `shutdown()` no longer waits out a full backoff.** The registry put its own running flag in `retryIf(..)`, which replaces whatever predicate the strategy already had, so a `Config.retryStrategy(RetryStrategy.retry().retryIf(..))` was dropped and every exception a consumer threw was retried, including the ones you had excluded. The flag is now passed as the shutdown predicate instead, so your own predicate is kept, and `shutdown()` stops a consumer that is between retry attempts instead of joining the polling thread through the rest of its backoff, one second with the default strategy. A consumer failure the strategy has given up on is now logged and the registry takes the next deadline, where it used to end the one polling thread that serves every category. Resolves [#999](https://github.com/johanhaleby/occurrent/issues/999).
* **A `SnapshotDeciderApplicationService` or `SnapshotDcbDeciderApplicationService` command no longer fails after its events are written when your `SnapshotStore` or `SnapshotPolicy` throws a checked exception or an `Error`, fixing behaviour shipped in 0.33.0.** The snapshot save after the write only caught a `RuntimeException`. A checked exception, which a store written in Kotlin can throw without declaring it, or an `Error` such as a `StackOverflowError` escaped `execute`, so a caller whose events were already written got an exception and was likely to run the command again. The save now logs whatever it catches and skips the snapshot, which the next replay rebuilds from the events. Deleting a snapshot left ahead of a stream you reset works the same way. The reactor `ReactiveSnapshotDeciderApplicationService` and `ReactiveSnapshotDcbDeciderApplicationService` do the same when your `ReactiveSnapshotStore` or `SnapshotPolicy` throws before a `Mono` is returned. Before, a `ReactiveSnapshotStore.delete` that threw an exception before returning its `Mono` failed the command, and a `StackOverflowError` or a `LinkageError` that your store threw from `save` or `delete` before returning its `Mono` left the command's `Mono` never completing. One raised while the returned `Mono` runs still does. In the two blocking services, an `InterruptedException` caught this way sets the thread's interrupt flag again. Resolves [#1097](https://github.com/johanhaleby/occurrent/issues/1097).
* **`ReactiveSnapshotDeciderApplicationService.executeAndReturnState` and `ReactiveSnapshotDcbDeciderApplicationService.executeAndReturnState` no longer write a command's events before refusing its null state, fixing behaviour shipped in 0.33.0.** Both mapped the decided state through the null check after the application service had already written the command, so a decider whose state is `@Nullable` and that folds to `null` had its events written or appended and then threw a `NullPointerException`, and a caller seeing a committed write fail was likely to run the command again. The refusal now runs inside the decider callback, before the write or the DCB append, so a null state is refused and nothing is written. `executeAndReturnDecision` is unaffected and still returns a decision whose state can be `null`. Resolves [#1103](https://github.com/johanhaleby/occurrent/issues/1103).
* **A checked exception from one `SynchronousSubscriptionModel` handler outside a transaction no longer stops the other handlers from receiving the events just written, fixing behaviour shipped in 0.33.0.** Every other handler now receives every event in the write, and the checked exception reaches the caller once they all have, the way a `RuntimeException` already did. A handler written in Kotlin can throw a checked exception without declaring it, and before this fix it ended the dispatch, so the handlers after it never saw those events, since the model has no replay. An `Error` still stops the dispatch at once. Resolves [#1102](https://github.com/johanhaleby/occurrent/issues/1102).
* **`DcbDeciderApplicationService.executeAndReturnState` no longer appends a command's events before refusing its null state, fixing behaviour shipped in 0.33.0.** It mapped the decided state through Reactor's own null check after the application service had already appended the command, so a decider that decided a null state had its events committed and then failed with a `NullPointerException`, and a caller seeing a committed append fail was likely to run it again. The refusal now runs inside the decider callback, before the append, so a null state is refused and nothing is appended. `executeAndReturnDecision` is unaffected and still returns a decision whose state can be null. Resolves [#1114](https://github.com/johanhaleby/occurrent/issues/1114).

#### Breaking changes

* **The flow saga's deprecated `join`, Kotlin's `expect<T>`, and `Expectation` are removed.** `join` was deprecated in 0.33.0 in favour of `on(StepCondition, ...)` with `allOf(...)`, and that replacement is what every caller now needs. `UpgradeToOccurrent_0_34` rewrites every `join` call whose expectation list is a literal `List.of(...)` or `Arrays.asList(...)` of literal `Expectation.of(...)` calls, collapsing a duplicate-typed pair to the higher of their counts the same way `join` itself did, but only when both counts are integer literals. A list built from a variable or a method call, a duplicate-typed pair whose count is not a literal, and every Kotlin call site are left alone and stop compiling, so the compiler finds them for you. [ADR 125](doc/architecture/decisions/0125-a-lowered-joins-reaction-reads-its-own-window-not-the-whole-retained-history.md) had rejected removing `join` outright, since no recipe covered it, and this recipe is what closes that gap. See [section 1 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#1-a-flow-sagas-join-kotlins-expectt-and-expectation-are-removed), [#707](https://github.com/johanhaleby/occurrent/issues/707) and [#806](https://github.com/johanhaleby/occurrent/issues/806).
* **A projection, a subscription built through the subscription DSL, a query through `DomainEventQueries`, a `@Snapshot` view, `ExecuteFilter`'s `type` and `includeTypes`, and `DcbCriteriaBuilder`'s `type` and `types` now refuse a declared event type whose concrete subtypes cannot be found, and every one of them refuses a concrete class that is neither final nor sealed.** You are affected if one of these declares an interface or an abstract class that is not sealed, an array, a primitive class literal, a sealed hierarchy reopened below the declared level, or a concrete class that is neither final nor sealed. Only a saga and an annotation-based subscription already refused the first five in 0.33.0, and the concrete-class case is new for those two as well, since 0.33.0 exempted it on purpose. See [section 3 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#3-declaring-an-event-type-whose-concrete-subtypes-cannot-be-found-is-refused) and [ADR 126](doc/architecture/decisions/0126-every-derived-event-type-filter-expands-a-declared-sealed-type.md). Resolves [#753](https://github.com/johanhaleby/occurrent/issues/753) and [#912](https://github.com/johanhaleby/occurrent/issues/912).
  * Up to 0.33.0 the concrete-class case derived a filter naming the declared class alone, so under every `CloudEventTypeMapper` Occurrent ships a caller declaring `class OrderPlaced` and publishing a `class SpecialOrderPlaced extends OrderPlaced` never received the subclass and got no warning, even though dispatch would have accepted it.
  * One caller was working rather than losing events, the one whose own `CloudEventTypeMapper` maps a whole hierarchy onto a single type string, since the subclass was then stored under the declared class's type string and did arrive. That caller is refused too and wants an explicit filter.
  * Where each one throws. `Projection` and `SnapshotView` throw when a runner or a query derives the filter, `DomainEventQueries` and the subscription DSL at the first query or subscription registration that needs one, a saga at `build()`, `SnapshotAnnotationRegistrar` and the subscription annotations at Spring Boot startup, `ExecuteFilter` when `ApplicationService#execute` resolves the filter, and `DcbCriteriaBuilder` when `type(...)` or `types(...)` builds the criterion.
  * Mark the declared class `final` when nothing extends it, seal the hierarchy, or declare the concrete event types. `Projection.filter(...)`, `SnapshotView.filter(...)`, a saga's `replacementFilter(...)` and `ExecuteFilter.from(StreamReadFilter)` are a further way out for the four that have an override. `DcbCriteriaBuilder` has none, so build the `DcbCriterion` from the raw CloudEvent type string with `DcbCriteria.type(String)` or `types(String, ...)` instead. There is no recipe entry, for the same reason 0.33.0's equivalent change had none, and because a recipe cannot tell whether a class should become `final` or the declaration should list concrete types.
* **`ExecuteFilter.excludeTypes` now refuses an array or a primitive declared type, where up to 0.33.0 it was accepted and handed straight to the configured `CloudEventTypeGetter`.** A call that returned some filter, however degenerate, now throws `IllegalArgumentException`. For a primitive that means declaring the concrete event types you actually meant. An array class is already concrete, so there is nothing narrower to declare, and it is refused for consistency with `type` and `includeTypes` rather than because excluding one is impossible, which makes `ExecuteFilter.from(StreamReadFilter)` the way out if you do mean to exclude an array type. See [section 3 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#3-declaring-an-event-type-whose-concrete-subtypes-cannot-be-found-is-refused). Resolves [#912](https://github.com/johanhaleby/occurrent/issues/912).
* **`occurrent.event-store.collection`, `occurrent.event-store.time-representation`, `occurrent.subscription.collection` and `occurrent.subscription.restart-on-change-stream-history-lost` are renamed to `occurrent.event-store.mongodb.collection`, `occurrent.event-store.mongodb.time-representation`, `occurrent.subscription.mongodb.collection` and `occurrent.subscription.mongodb.restart-on-change-stream-history-lost`.** Each old key still works and is deprecated, removed in the release after next, and `UpgradeToOccurrent_0_34` rewrites both `.properties` and `.yaml` for you. See [section 4 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#4-four-mongodb-only-keys-move-under-mongodb). Resolves [#439](https://github.com/johanhaleby/occurrent/issues/439).
* **`WriteResult` and `DcbAppendResult` each gain a fourth component, `Optional<AppendId>`.** Every store now stamps the same `appendid` CloudEvent extension on every event a single write or DCB append call persists, and the result returns that identifier. An empty write reports no append id. This is the write-side half of [ADR 132](doc/architecture/decisions/0132-an-append-has-an-identity-and-read-your-writes-becomes-a-membership-question.md). Two things break for an external caller. A test that compares a whole `WriteResult` or `DcbAppendResult` with `isEqualTo(...)` starts failing on any result that persisted something, since a fresh id is minted per call, so compare `streamId`, `oldStreamVersion` and `newStreamVersion` (or the DCB equivalents) instead. And a record pattern that still names the original three components stops compiling, because a record pattern has to name every component of the canonical constructor. `UpgradeToOccurrent_0_34` adds the fourth binding, `var appendId`, to the record patterns for you. See [section 6 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#6-writeresult-and-dcbappendresult-gain-a-fourth-component-the-append-id). Part of [#740](https://github.com/johanhaleby/occurrent/issues/740).
* **`SagaEnvelope`, `SagaInstance`, `SagaStatus` and `SagaRunnerConfig` all change shape for the saga quarantine above.** `SagaEnvelope` gains `started` and `failure`, and `SagaRunnerConfig` gains `quarantineAfter`, so a record pattern over either stops compiling. An ordinary `new SagaEnvelope(..)` or `new SagaRunnerConfig(..)` on the old arity still compiles, because both keep a constructor with the old argument list that fills the new components in. `SagaInstance` gains `failure()`, which breaks anyone implementing that interface outside this repository. `SagaStatus` gains `QUARANTINED`, so an exhaustive `switch` or `when` over it stops compiling, and, less visibly, `findByStatus(ACTIVE, ..)` no longer returns the instances that have stopped moving, which is what it was most often used to find. There is no OpenRewrite recipe, because none of them can be rewritten mechanically. See section 8 of [Upgrading to 0.34.0](doc/migration/upgrading-to-0.34.0.md).
* **A `DurableSubscriptionModel` subscription on a shared MongoDB Atlas cluster, or any wrapped model whose `globalCheckpoint()` answers `null`, now refuses to start unless it already has a checkpoint stored, see Changes above.** Up to 0.33.0 it started anyway with no recorded position. Set `occurrent.subscription.start-when-no-start-position-can-be-recorded=true`, or `DurableSubscriptionModelConfig.startWhenNoStartPositionCanBeRecorded(true)`, to keep the previous behaviour, or subscribe with a `StartAt` of your own. See [section 7 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#7-durablesubscriptionmodel-refuses-a-first-subscription-when-no-start-position-can-be-recorded).
* **A reactor catch-up subscription can now deliver an event twice across the replay-to-live handover, where before the history ids in the bounded cache suppressed the second delivery, see Changes above.** This changes observable delivery for the reactive catch-up models shipped in 0.30.0, for callers who never touch `recordAppliedAppends`. Catch-up delivery on those models has always been at-least-once, so a handler written to tolerate a repeat needs no change, and there is no recipe because nothing in a caller's source declares a requirement that an event arrives once. If yours is not safe to run twice on the same event, see [section 9 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#9-a-reactor-catch-up-subscription-can-deliver-a-concurrent-write-twice).
* **A `@Projection`, `@Saga` or `@Snapshot` bean's class-level advice, `@Transactional` or a custom aspect for example, no longer runs once at startup as a side effect of its descriptor factory being invoked.** That was never a documented behaviour. It came from CGLIB, the default under `spring.aop.proxy-target-class=true`, subclassing the bean, so the factory happened to run through the proxy and any matching advice ran with it. The factory now always runs directly on the bean's own class. See [section 5 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#5-a-descriptor-factorys-class-level-advice-no-longer-runs-at-startup).
* **A reactor `@Projection`, `@Saga` or `@Snapshot` factory method that returns `null` now fails startup with `IllegalStateException`, where it previously threw `IllegalArgumentException`.** This only affects a factory that is itself broken, returning `null` instead of the descriptor it declares, but a catch block scoped to `IllegalArgumentException` around that failure no longer catches it. The blocking stack already threw `IllegalStateException` for the same mistake. See [section 5 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#a-null-returning-factory-now-fails-differently-on-reactor).
* **A `@Subscription`, `@StreamSubscription`, `@DcbSubscription` or `@SynchronousSubscription` handler method Spring's proxy cannot invoke now fails Spring Boot startup, where it previously ran on the raw bean with no advice applied, and a live-only subscription can no longer see an event written during singleton construction, see Changes above.** A JDK interface proxy missing the handler method, a private method a CGLIB proxy cannot override, a final method a CGLIB proxy cannot override either, and a static method whose invocation never goes through any proxy at all, all used to deliver every event with `@Transactional` or any other aspect silently skipped, which is the bug fixed above, and now fail startup instead. An unproxied bean has no proxy to lose advice through in the first place, so a private or final handler there still registers and runs exactly as before. Registration for all four annotations also moves to the phase `@Projection`, `@Snapshot` and `@Saga` already use, so the bean-creation-order-dependent chance of catching an event written during singleton construction, `@PostConstruct` included, is gone for every one of them. A write from a later startup phase, an `ApplicationRunner` for example, still reaches a live subscription normally, since registration precedes it. See [section 11 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#11-a-subscription-handler-spring-cannot-invoke-now-fails-startup-and-a-live-subscription-can-miss-a-startup-write).
* **The blocking `CatchupProjectionFeed.accept(..)` and `DomainEventFeed.accept(..)` now return only once the event is applied, and throw when it was not, where 0.33.0 returned as soon as the event was buffered.** A listener acknowledges the message once `accept(..)` returns, so in 0.33.0 it acknowledged an event that was only held in memory, and if the application stopped or crashed before the catch-up finished, the broker never sent that event again. A rolling deploy during a long startup replay was enough. See [section 12 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#12-a-projection-feeds-accept-waits-until-the-event-is-applied-and-fails-when-it-is-not). Resolves [#1135](https://github.com/johanhaleby/occurrent/issues/1135).
  * Before the feed goes live, and while a catch-up runs on a feed that already went live, `accept(..)` waits until the catch-up has applied the event. Never call it on the thread that later calls `catchUp()`, `catchUpAll()` or `goLive()`, because that thread then waits until another thread runs the catch-up, takes the feed live, calls `stopCatchUp()` or interrupts it.
  * It throws an `IllegalStateException` when the catch-up was stopped before the feed went live, the catch-up failed, the waiting thread was interrupted, or it was called while the feed was not live from inside the projection, a view or another callback of the same feed, where the thread would wait for work it holds up itself. Thrown while the catch-up replays history into the projection or a view, that refusal fails the catch-up, and the feed refuses every event until you build a new one. A caller that catches the refusal and continues drops the nested event, which 0.33.0 buffered and applied once a catch-up took the feed live. It also throws when another delivery of the same event is still running, where 0.33.0 returned normally. Do not acknowledge the message when it throws, and the broker delivers it again.
  * A long replay keeps the listener thread waiting. A Kafka consumer that waits past `max.poll.interval.ms`, five minutes by default, and a RabbitMQ consumer that holds on to a message without acknowledging it for longer than `consumer_timeout`, 30 minutes by default, both get the message delivered again, which costs a redelivery rather than the event.
  * `DomainEventFeed.acceptCloudEvent(..)`, which the Kafka and RabbitMQ bridges call, is unchanged.
* **The reactor `CatchupProjectionFeed.accept(..)` and `DomainEventFeed.accept(..)` now return a `Mono` that errors with an `IllegalStateException` for an event fed while the feed is stopped, where 0.33.0 completed it without applying the event.** A feed is stopped from the moment a catch-up is stopped before the feed went live until the next catch-up starts. A listener that acknowledges on completion acknowledged such an event in 0.33.0, and the broker never sent it again. Do not acknowledge the message when the `Mono` errors. See [section 12 of the upgrade guide](doc/migration/upgrading-to-0.34.0.md#12-a-projection-feeds-accept-waits-until-the-event-is-applied-and-fails-when-it-is-not).

### Changelog 0.33.0 (2026-08-16)

#### Highlights

* A flow saga step can wait on a condition instead of a single event type, so `on(allOf(...))` and `on(anyOf(...))` express an alternative or a combined count that `join` could not. `join` is deprecated in their favour.
* A flow saga can cap how many events it keeps for the step an instance is waiting in, with `stepWindow(int)`.
* A saga or a subscription that declares a sealed event type now receives the concrete events stored under it, and one that declares a type whose subtypes cannot be found is refused when it is built rather than silently receiving nothing.
* A competing consumer's checkpoint write is now stamped with its lease version, so a node that has lost its lease can no longer move the checkpoint backwards. The Spring Boot starter wires that up for you.
* A materialized view batches its store calls during a catch-up replay, so a history of N events over K projection keys costs about one read and one write per key instead of per event.

#### Changes

* **A saga that declares a sealed event type now receives the concrete events stored under it, which fixes behaviour the saga DSL shipped in 0.32.0.** `startsOn(OrderEvent.class)`, `evolve`, `react`, `step.on(OrderEvent.class, ...)` and `event(OrderEvent.class, 2)` all asked for the supertype's own CloudEvent type and nothing else, so a saga declaring a sealed interface or an abstract class received nothing at all. A saga now asks for every concrete type its sealed types permit. A saga that already declares concrete types is unaffected. See [ADR 124](doc/architecture/decisions/0124-a-saga-expands-a-declared-sealed-event-type.md) and [#743](https://github.com/johanhaleby/occurrent/issues/743).
* **A subscription that declares a sealed event type now also asks for that type's own CloudEvent type, which changes behaviour `@Subscription`, `@StreamSubscription`, `@SynchronousSubscription` and `@DcbSubscription` shipped.** The filter named only the concrete types the sealed type permits, so an event stored as a direct instance of the declared type never matched. Adding a type can only widen what matches, so a subscription that only hit this gap keeps working. One kind of declaration is refused instead of accepted now, a sealed hierarchy that is reopened below the declared type, which 0.32.0 accepted. See [section 10 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#10-a-saga-or-subscription-declaring-a-supertype-event-is-refused) and [ADR 124](doc/architecture/decisions/0124-a-saga-expands-a-declared-sealed-event-type.md).
* **A saga can now select on more than event type, with `Saga.Builder.narrowingFilter(Filter)` and `Saga.Builder.replacementFilter(Filter)`.** `FlowSaga.Builder` has both, both Kotlin `saga { }` blocks expose them, and `Saga.create(...)` takes a replacement as a trailing argument. Keep either one wide enough for the saga's start events and for the events that move an instance on. See [section 10 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#10-a-saga-or-subscription-declaring-a-supertype-event-is-refused), [ADR 124](doc/architecture/decisions/0124-a-saga-expands-a-declared-sealed-event-type.md) and [#751](https://github.com/johanhaleby/occurrent/issues/751).
  * A narrowing filter is combined with the filter derived from the saga's event types, so the saga keeps asking for its own types and also requires your condition on subject, source, data or time. On a flow saga it also changes what a guard reading `received.none(...)` sees.
  * A replacement filter is used instead of deriving one. That is how you run a saga over an event hierarchy whose concrete types cannot all be found, for example under a `CloudEventTypeMapper` of your own that maps a whole hierarchy onto a single CloudEvent type.
...
* **A flow saga step can now wait on a condition instead of only a single event type or a `join`.** `on(StepCondition, then)` expresses an alternative ("wait for either two approvals or one rejection") or a mixed count and alternative that `join` could not. Build one from `event(type[, count][, predicate])` checks combined with `allOf(...)` and `anyOf(...)`. A condition is a plain value, so you can reuse it across `on(...)` calls and mix it with classic `on(Class, ...)` branches in the same step. Kotlin gets matching `event<T>`, `allOf`, `anyOf` and `on(condition, then)`. `join` and Kotlin's `expect<T>` are deprecated in its favour, with no forced migration. See [ADR 120](doc/architecture/decisions/0120-a-step-condition-is-a-monotone-matcher-tree.md), [ADR 123](doc/architecture/decisions/0123-a-step-conditions-counts-are-carried-so-the-steps-events-can-be-dropped.md) and [#707](https://github.com/johanhaleby/occurrent/issues/707).
  * The reaction reads the events received since its step was entered, the same ones the condition counted, so a count it takes matches the count that fired it. `received.initiating(...)` still reaches the start event, and a guard and a `timeout`'s `onExpiry` still read the whole retained history.
  * Two `allOf` checks that match the same event are refused when the saga is built, so `allOf(A.class, A.class)` asks you for `event(A.class, 2)` instead.
  * A check's predicate must answer the same way for the same event every time. It is run again over the step's events whenever the count cannot be read from the instance's state, which is the case for a predicate with no name, for a declaration that changed, and for every count during a replay.
* **A flow saga can now cap how many events it keeps for the step an instance is waiting in, with `FlowSaga.Builder.stepWindow(int)` (`stepWindow(...)` in the Kotlin `saga { }` block).** `historyWindow` only ever limited what was kept from earlier steps, so an instance waiting in one step while a large number of correlated events arrived kept every one of them, and the 0.31.0 entry that introduced `historyWindow` was wrong to say the retained state does not grow without bound. `stepWindow` is unbounded by default. Set together with `historyWindow`, an instance holds at most `historyWindow + 2 * stepWindow + 1` events, since a transition keeps the events of the step it left for that step's reaction while the step it enters fills its own cap. See [ADR 123](doc/architecture/decisions/0123-a-step-conditions-counts-are-carried-so-the-steps-events-can-be-dropped.md), [section 9 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#9-a-flow-saga-can-cap-the-events-of-the-step-it-is-parked-in) and [#741](https://github.com/johanhaleby/occurrent/issues/741).
  * A step condition still completes on the same event whatever the cap, because its counts are kept in the instance's state rather than counted from the events. What sees less is a guard, a `timeout`'s `onExpiry` and a reaction that reads its own step's events. `received.initiating()` and the event that fired a branch stay reachable at any cap.
  * Keeping a count means matching it back to its check after a redeploy, so a check in a capped step names its predicate, `event(Payment.class, 2, "isBig", p -> p.isBig())`. `build()` refuses a capped step whose predicate has no name, or whose two checks share a name while holding different predicates. Change the name when the predicate's meaning changes, since a kept name with a changed test is the one thing this cannot detect.
  * Changing what a capped step waits on while instances are waiting in it makes those instances refuse their next delivery, with a message naming the step.
  * The Spring MongoDB store warns once the first time a waiting instance's retained events cross 1,000, naming `stepWindow` as the way to trim them, since that count is what pushes the document toward MongoDB's 16 MB limit. It warns again after a later save drops back below that count, after the saga is deleted, or after the id is evicted under churn.
...
* **`ReactorCheckpointStorage` now retries a transient MongoDB error while reading, saving or deleting a checkpoint, instead of letting it reach the subscription.** It retries with exponential backoff, 100 ms up to 2 seconds, the same interval `SpringMongoCheckpointStorage` already uses on the blocking stack, bounded to 5 attempts before rethrowing the original failure. Pass your own `reactor.util.retry.Retry` to the new three-argument constructor to change that. Resolves [#656](https://github.com/johanhaleby/occurrent/issues/656).
* **`CompetingConsumerSubscriptionModel.pauseSubscription(id)` now works on a node that has not won the lock, not only on the one delivering events.** The pause is remembered and honoured once the lock arrives, and `isPaused` and `isRunning` answer truthfully for it in the meantime. Pausing cluster-wide means calling `pauseSubscription` on every node. This changes behaviour shipped in 0.31.0. See [ADR 112](doc/architecture/decisions/0112-a-competing-consumer-can-be-paused-while-still-waiting-for-the-lock.md). Resolves [#565](https://github.com/johanhaleby/occurrent/issues/565).
* **A MongoDB lease competing consumer strategy no longer lets its scheduled lease refresh and an application thread change the same consumer at the same time.** A consumer unregistered while the refresh was acquiring its lease could be registered again by that refresh, leaving the lease held and refreshed by a node with no consumer for it, and no other node able to take that subscription over until the process restarted. The same overlap in time could report one status change to a `CompetingConsumerListener` twice, or not report a real one at all. Both classes also drop the `synchronized` on `registerCompetingConsumer` and `unregisterCompetingConsumer`, so one consumer's call no longer waits behind every other consumer's call on the same strategy instance. This affects `NativeMongoLeaseCompetingConsumerStrategy` and `SpringMongoLeaseCompetingConsumerStrategy`, and changes behaviour shipped before 0.32.0. See [ADR 113](doc/architecture/decisions/0113-a-competing-consumers-status-and-its-lease-call-are-one-step.md). Resolves [#651](https://github.com/johanhaleby/occurrent/issues/651).
* **A MongoDB lease's expiry is now judged against the database's own clock, not the asking node's.** `expiresAt` was written from the holder's clock and compared against whichever node was asking, so a node whose clock ran ahead could take a healthy lease, and one whose clock ran behind waited out the skew on top of the lease time before taking over a dead one. Both the write and the read now use MongoDB's `$$NOW`. This affects `NativeMongoLeaseCompetingConsumerStrategy` and `SpringMongoLeaseCompetingConsumerStrategy`, and changes behaviour shipped before 0.32.0. Their builders' `clock(Clock)` no longer affects lease timing and is kept only so existing code compiles. See [ADR 114](doc/architecture/decisions/0114-a-lease-expires-on-the-database-clock-not-the-asking-nodes.md). Resolves [#659](https://github.com/johanhaleby/occurrent/issues/659).
* **A released MongoDB competing-consumer lock now stays in the collection instead of being deleted, so its version keeps climbing instead of resetting to 0 the next time the lock is acquired.** `MongoListenerLockService.remove` unsets `subscriberId` and `expiresAt` in place rather than deleting the document. A missing `subscriberId` was already read as a free lock and a missing `expiresAt` as an expired one, so acquiring after a release behaves exactly as before. The climbing version is what later lets a checkpoint write be refused when the lease has moved to another node ([ADR 116](doc/architecture/decisions/0116-a-checkpoint-write-from-a-lease-that-has-moved-on-is-refused.md)), and nothing reads it that way yet. Do not drop the lock collection on its own, since that resets the version underneath a checkpoint that remembers a higher one. `CheckpointStorage.delete(subscriptionId)` clears both together. Part of [#665](https://github.com/johanhaleby/occurrent/issues/665).
...
* **The Spring Boot starter now stamps every checkpoint write with the lease version, on every model it builds, so an application with a competing-consumer strategy bean gets that protection automatically.** It refuses to start when that would pair the stamping with a `CheckpointStorage` that only writes unconditionally, because the first checkpoint write a node makes while holding its lease would throw. Set `occurrent.subscription.competing-consumer.fence-checkpoints=false` to keep such a storage and write every checkpoint unconditionally. Part of [ADR 116](doc/architecture/decisions/0116-a-checkpoint-write-from-a-lease-that-has-moved-on-is-refused.md) and [#665](https://github.com/johanhaleby/occurrent/issues/665).
* **A competing consumer that regains its lease now resumes from the stored checkpoint, not from the position it had read before losing the lease.** It no longer redelivers everything the other node already handled while it was gone, and no longer moves the checkpoint backwards. At-least-once delivery is unchanged. With `EveryN(n)` checkpointing the resume can still go back up to `n - 1` events, and zero with the default `everyEvent()`. See [ADR 117](doc/architecture/decisions/0117-a-resumed-competing-consumer-continues-from-the-checkpoint.md). Resolves [#668](https://github.com/johanhaleby/occurrent/issues/668).
...
* `ManualStartSubscriptionModel` now records a subscription's first start position with a single conditional write, so two nodes starting the same manually-started subscription at once can no longer both write. The write only happens for a registration that resolves to the subscription model default, which is the one a wrapped model reads a stored checkpoint for. Resolves [#669](https://github.com/johanhaleby/occurrent/issues/669).
* **`ManualStartSubscriptionModel` now records where a subscription starts when you register it, not when you start it, which changes behaviour shipped in 0.32.0.** Two nodes registering minutes apart during a rolling deploy used to let whichever one started first decide, silently skipping the events between the two registrations. The first write to reach storage now wins, and the node whose position was not the one stored has its registration refused with the new `StartPositionAlreadyPinnedException` instead of starting from a position it never read. Register again to clear that, since the node then finds the other position stored and starts from it. See [ADR 86](doc/architecture/decisions/0086-a-manual-subscription-is-registered-not-started.md)'s amendment, [ADR 116](doc/architecture/decisions/0116-a-checkpoint-write-from-a-lease-that-has-moved-on-is-refused.md)'s, [section 8 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#8-three-ways-the-spring-boot-starter-now-refuses-to-start) and [#771](https://github.com/johanhaleby/occurrent/issues/771).
  * A registration that names a start position of its own writes nothing and is never refused, so a replay you asked for stays a replay. `stoppedByDefault(SubscriptionModel)`, and a `StartAt` of your own, record no position at all.
  * A position source that answers `null` also refuses the registration, with an `IllegalStateException`, since `null` is how a source reports a problem it cannot resolve and there is then no position to record. Such a registration used to be let through in silence and start from wherever the feed had reached.
  * One node can hit `StartPositionAlreadyPinnedException` with nobody else registering, when its checkpoint storage answers from a replica that has not caught up. That one needs a reader that has seen the write, not another start.
  * A wrapped model whose start position decides something other than where the subscription starts says so by answering `false` from the new `SubscriptionModelWrapper.decidesWhereTheSubscriptionStarts()`, and is passed over. `CompetingConsumerSubscriptionModel` is the one Occurrent ships that does, since its position decides whether to compete. A wrapper of your own that does not decide where the subscription starts and does not answer `false` can be read differently here than by the model that starts the subscription, and [ADR 86](doc/architecture/decisions/0086-a-manual-subscription-is-registered-not-started.md) has what that costs.
* **`ReactorDurableSubscriptionModel` now records a subscription's first start position with a conditional write and refuses a registration that loses the race, which changes behaviour shipped in 0.32.0.** It used to read the stored position, find none, read the current position and write that with no condition attached, so two nodes registering a subscription for the very first time at the same moment both wrote, the second one won without anybody being told, and the events between the two positions reached neither. A node that cannot show that the stored position is the one it read is now refused with `StartPositionAlreadyPinnedException`. A position that was already stored is still taken without a word, so a node joining a subscription another has been running is unaffected. See [ADR 89](doc/architecture/decisions/0089-manual-subscription-mode-on-the-reactive-stack.md)'s last amendment and [#738](https://github.com/johanhaleby/occurrent/issues/738).
  * Unlike the blocking twin this is not limited to `occurrent.subscription.mode=manual`. The reactive stack has no manual-start wrapper and this is its only durable model, so any reactive subscription whose start position resolves to the subscription model default can be refused on its very first run. A registration that names a position of its own records nothing and is never refused.
  * Starting the node again takes whatever position storage holds by then, except when the refusal came from a reader that has not seen the write, which answers the same way every time.
  * The refusal is thrown from `subscribe(..)` when the wrapped model manages named subscriptions of its own, and signalled on `Subscription.waitUntilStarted()` with an `ERROR` logged when this model opens the feed itself, where it cannot throw.
  * A read of the position that fails, and one that answers nothing, both refuse the registration too, in the same two places. Answering nothing used to fall back to `StartAt.now()`, which is not as harmless as it looks, because the wrapped model applies a start position when it opens its feed rather than when it receives one, so a wrapped model that is itself stopped would start from wherever the feed had reached by the time it was started.
  * A `CheckpointStorage` that answers nothing when asked to record the position is refused for the same reason. One of your own that answers `false` from `evaluatesWriteConditionsFor(String)` keeps the unconditional write and gets a `WARN` saying so. Both reactive storages Occurrent ships answer `true`.
* **A framework-built materialized view now batches its store calls during a catch-up replay, on the path reached through `CatchupProjectionFeed` and its reactor twin.** A history of N events over K projection keys used to cost one read and one write per event there. It now costs about one of each per key, and about two calls per batch when the repository does real bulk operations, which the Mongo ones below do. `Projections.materializedView` (blocking) and `Projections.reactiveUpdateWithMetadata` (reactor) build such views with batching on by default, sized by `batchSize` on the new `MaterializedViewOptions`, and a batch size of `1` restores the old write-through behaviour. Resolves [#638](https://github.com/johanhaleby/occurrent/issues/638).
  * Batching only applies with the default `RetryStrategy.none()`. A flush built with any other retry strategy reads and writes one key at a time.
  * A view of your own opts in by implementing the new `ReplayAware` capability (`ReactiveReplayAware` in the reactor projection DSL). `ViewStateRepository` gains defaulted `findAllById` and `saveAll`, so an existing repository keeps working unchanged.
  * `ProjectionRunner` and its reactor twin, the subscription-fed projection runners, stay on the old per-event cost for now. What a partly failed batch write leaves behind, and why that stays within the at-least-once replay contract, is in [ADR 110](doc/architecture/decisions/0110-a-replay-tells-the-view-where-it-begins-and-ends.md).
...
* **`MongoProjectionStoreProvider`'s `save` and `saveAll` now reject a `@Projection` state whose `@Id` doesn't match the projection key.** Before this, a mismatch silently wrote to the wrong document and the read model never accumulated. Both methods now throw `IllegalStateException` instead, the same check `SpringMongoViewExtensions`'s Mongo-backed repositories already apply. This changes behaviour shipped in 0.31.0 (`saveAll` in 0.32.0). Resolves [#629](https://github.com/johanhaleby/occurrent/issues/629).
...
* **A composed `AND` filter with several `data` payload paths no longer reparses a byte-backed event's payload once per path.** Once left-to-right evaluation reaches a run of two or more consecutive payload paths, that run is read in one pass instead of once per path. The filter is still evaluated strictly left to right, stopping at the first mismatch exactly as before, so a metadata condition ahead of a payload one still decides the result without touching the payload, and a store with no payload reader still gets a `false` rather than an exception for a filter that never reaches its `data` check. A Map-backed event, the production MongoDB path, already skipped that reparse. Resolves [#623](https://github.com/johanhaleby/occurrent/issues/623).
...
* **`CompetingConsumerStrategy` gains a `fencingToken(subscriptionId)` default method, and `NativeMongoLeaseCompetingConsumerStrategy` and `SpringMongoLeaseCompetingConsumerStrategy` now answer it.** The token increases on a genuine change of lease owner and stays unchanged when the same holder refreshes, which gives a `CheckpointStorage` something to compare a stored value against so it can refuse a write from a lease that has moved on ([ADR 116](doc/architecture/decisions/0116-a-checkpoint-write-from-a-lease-that-has-moved-on-is-refused.md)). It answers empty unless exactly one consumer is registered for the subscription and that consumer holds the lock, whatever that consumer's status otherwise is. The default method answers empty too, so an existing `CompetingConsumerStrategy` keeps compiling and has no such protection. Part of [#665](https://github.com/johanhaleby/occurrent/issues/665).
...
* **A `CompetingConsumerStrategy` bean of your own now replaces the Spring Boot starter's default one whatever type it is, and several of them with no `@Primary` fail startup.** A strategy of any type other than `SpringMongoLeaseCompetingConsumerStrategy` used to leave the starter's default in place beside it, so the subscription model kept delivering under the starter's own lease, and the ambiguity that created was read as no strategy at all, which wrote checkpoints unconditionally and ran a `@Saga`'s timer poller on every instance. `AmbiguousCompetingConsumerStrategyException` now names the beans found and the remedy, which is to mark one `@Primary` or leave only that one in the application context. See the amendment to [ADR 116](doc/architecture/decisions/0116-a-checkpoint-write-from-a-lease-that-has-moved-on-is-refused.md) and [#684](https://github.com/johanhaleby/occurrent/issues/684).
* **`SpringRedisCheckpointStorage` now evaluates `notOlderThan` and `ifAbsent` for real, instead of refusing them, on Redis Cluster too.** The write version lives in a second, prefixed key, separate from the checkpoint's own, so a node still on the previous release keeps reading the checkpoint with a plain `GET` through a rolling deploy. A Lua script compares the stored version and writes both keys atomically, and a refused write throws `CheckpointWriteConditionNotFulfilledException` immediately rather than being retried, since a refusal can never succeed on a later attempt. See [section 4 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#4-redis-cluster). Part of [#665](https://github.com/johanhaleby/occurrent/issues/665).
  * The version key has a hash tag matching whatever the checkpoint key itself hashes on, so Cluster puts both keys in the same slot instead of refusing the script for crossing them. It also has a SHA-256 digest of the subscription id after that tag, so two ids sharing a hash tag still get their own version key.
  * Built with either of the original two constructors, `save` refuses a `notOlderThan` or `ifAbsent` condition outright for a subscription id that Cluster hashes whole, meaning one that is empty or still contains a closing brace, whether or not the deployment behind it is actually Cluster. That is what keeps `evaluatesWriteConditions()` true without exception in that mode. `any()` never refuses one, and `delete` falls back to two single-key deletes instead, which removes a version key left behind by a move from standalone to Cluster and is otherwise a no-op.
  * `read`, `save`, `delete` and `exists` also refuse a subscription id that starts with the version key's own reserved prefix, since such an id could be, or alias, another subscription's version key. See the breaking change below.
* **`CheckpointStorage` gains `evaluatesWriteConditionsFor(subscriptionId)`, a per-id refinement of `evaluatesWriteConditions()` that defaults to it, and `SpringRedisCheckpointStorage.forStandalone(RedisOperations)` (with a `RetryStrategy` overload) builds a new mode for it.** Override the new method on a storage whose answer to a conditional write depends on the subscription id rather than being the same for every one. Part of [#665](https://github.com/johanhaleby/occurrent/issues/665).
  * `SpringRedisCheckpointStorage`'s original two constructors keep today's Cluster-safe behaviour, refusing the one kind of subscription id above. `forStandalone` is the other mode, for a deployment that is standalone or replicated rather than Cluster, where slot alignment is not a concept the server has, so a conditional write there accepts such an id too. Do not build a Cluster deployment's storage with `forStandalone`, since a conditional write for an id that standalone accepts and Cluster cannot align a slot for then fails with Redis's own `CROSSSLOT` error.
  * The Spring Boot starter now also asks `evaluatesWriteConditionsFor` for the subscription ids that `CheckpointStorageCannotFenceSubscriptionException`'s javadoc names precisely, and throws that exception, naming the storage and every refused id, when a storage answers `true` to `evaluatesWriteConditions()` overall but `false` for one of them. See [section 8 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#8-three-ways-the-spring-boot-starter-now-refuses-to-start).
...
* **`ManualStartSubscriptionModel.stoppedByDefault(SubscriptionModel, GlobalCheckpointSource, CheckpointStorage)` now refuses a `CheckpointStorage` that answers `false` to `evaluatesWriteConditions()`, which changes behaviour that factory shipped in 0.32.0.** It records a subscription's first start position with `ifAbsent()`, so it needs a storage that evaluates that condition, and it throws `IllegalArgumentException` naming the storage class rather than accepting one that says it does not. `evaluatesWriteConditions()` is new in this release and defaults to `false`, so a storage of your own written against 0.32.0 answers `false` even when it evaluates `ifAbsent` correctly. A Spring Boot application running `occurrent.subscription.mode=manual` with a `CheckpointStorage` bean of its own fails to start until that is fixed, whatever `occurrent.subscription.competing-consumer.fence-checkpoints` is set to. Every storage Occurrent ships answers `true`. See [ADR 86](doc/architecture/decisions/0086-a-manual-subscription-is-registered-not-started.md)'s amendment, [section 8 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#8-three-ways-the-spring-boot-starter-now-refuses-to-start) and [#669](https://github.com/johanhaleby/occurrent/issues/669).
  * There are two ways out. Override `evaluatesWriteConditions()` to return `true` on a storage that evaluates both `notOlderThan` and `ifAbsent`, or use the one-argument `stoppedByDefault(SubscriptionModel)`, which records no position at all and lets a first run start from the moment it is started.
* **A saga or an annotation-based subscription that declares an event type whose concrete subtypes cannot be found is refused, which changes behaviour the saga DSL shipped in 0.32.0, and for one of those declarations, behaviour `@Subscription`, `@StreamSubscription`, `@SynchronousSubscription` and `@DcbSubscription` shipped too.** You are affected if the declared type is an interface or an abstract class that is not sealed, an array, or a sealed hierarchy with a level below the root that is neither sealed nor final (`non-sealed` in Java, `open` or `abstract` in Kotlin). `build()` throws `IllegalArgumentException` naming that type for a saga, where 0.32.0 built the saga and started it, and a subscription's registrar throws the same for a subscription, at startup for a Spring Boot application. A saga or a subscription that declares concrete types, or a sealed hierarchy that is sealed or final all the way down, is unaffected. See [section 10 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#10-a-saga-or-subscription-declaring-a-supertype-event-is-refused), [ADR 124](doc/architecture/decisions/0124-a-saga-expands-a-declared-sealed-event-type.md), [#743](https://github.com/johanhaleby/occurrent/issues/743) and [#755](https://github.com/johanhaleby/occurrent/issues/755).
  * Under the type mappers Occurrent ships, a saga in that position was receiving nothing, or missing part of the hierarchy, so the exception reports a saga that never worked rather than breaking one that did.
  * For a subscription the story is only partly the same. A non-sealed interface, a non-sealed abstract class or an array already refused a subscription with an `IllegalArgumentException` in 0.32.0, under a message this release rewords. The only newly breaking case is a sealed hierarchy reopened below the declared type, which 0.32.0 accepted and matched only at the reopened level.
  * The one case that genuinely worked before is a `CloudEventTypeMapper` of your own that maps a whole hierarchy onto one type string, and there declaring the concrete types keeps working because they all map to the same string.
  * Seal the hierarchy, or declare the concrete event types. A saga has a third way out, `replacementFilter(...)`, which is the direct one under such a mapper. A subscription has its `eventTypes` attribute. There is no recipe entry, because telling a refused declaration from a sealed one that now works needs the sealed modifier and the type model behind a class literal does not record it, so the upgrade guide is the migration path.
* **A checkpoint write now states its condition, so this changes behaviour that every `CheckpointStorage` implementation ships.** The existing two-argument `save` is unchanged and still writes unconditionally. `save(subscriptionId, checkpoint, CheckpointWriteCondition)` also accepts `notOlderThan(long)`, refused once a newer version is already stored and throwing `IllegalArgumentException` for a negative version, and `ifAbsent()`, accepted only when nothing is stored yet. If you implement `CheckpointStorage` yourself, see [Upgrading to 0.33.0](doc/migration/upgrading-to-0.33.0.md). See [ADR 116](doc/architecture/decisions/0116-a-checkpoint-write-from-a-lease-that-has-moved-on-is-refused.md) and [#665](https://github.com/johanhaleby/occurrent/issues/665).
  * `writeVersion(subscriptionId)` reads back the version a condition is judged against, and `evaluatesWriteConditions()` answers whether this storage evaluates conditions at all, defaulted to `false` so a storage that only writes unconditionally needs to say nothing.
  * A refused write throws `CheckpointWriteConditionNotFulfilledException` on the blocking stack and signals it as a `Mono.error` on the reactive one.
  * Every storage Occurrent ships evaluates all three for real, meaning both Mongo storages, the reactor Mongo storage, the Redis storage and both in-memory storages, on Redis Cluster too for the subscription ids the Redis entry above doesn't already refuse outright.
* **`SpringRedisCheckpointStorage`'s `read`, `save`, `delete`, and `exists` now refuse a subscription id that starts with `occurrent:checkpoint-version:`, which changes behaviour a subscription id starting with that exact string had under 0.32.0.** That prefix is reserved for the version key the conditional-write support above introduced, and a subscription id starting with it could be, or alias, another subscription's version key, so a write against it could corrupt that other subscription's stored version. 0.32.0 stored every checkpoint at a single unprefixed key with no version concept at all, so an id like that worked there like any other. You are affected only if an existing subscription id already starts with that exact string. [section 4 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#4-redis-cluster) has the migration path, which has to run before upgrading, since `read` refuses the id afterward too and cannot hand back the checkpoint to move. Part of [#665](https://github.com/johanhaleby/occurrent/issues/665).
* **`ReplayAwareSubscriptionModel` and `IntrospectableSubscriptionModel` are renamed to `ReplayAwareSubscriptions` and `IntrospectableSubscriptions` on both stacks, and `DelegatingSubscriptionModel` is renamed to `SubscriptionModelWrapper`.** None of the three ever extended `SubscriptionModel`, so the old "SubscriptionModel" suffix claimed a relationship they do not have. `SubscriptionModelWrapper`'s two methods are renamed with it, `getDelegatedSubscriptionModel()` to `getWrappedSubscriptionModel()` and `getDelegatedSubscriptionModelRecursively()` to `getWrappedSubscriptionModelRecursively()`, so the type and its methods share one vocabulary. The published TCK base class `IntrospectableSubscriptionModelConformance` moves with the interface it is named after, to `IntrospectableSubscriptionsConformance`. `UpgradeToOccurrent_0_33` renames all five interfaces, both methods and the TCK base class for you, in Java and Kotlin alike, and [section 5 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#5-five-subscription-capability-interfaces-are-renamed) covers doing it by hand.
* **`RepositionableSubscriptions.of`, `ReplayAwareSubscriptions.of` and `IntrospectableSubscriptions.of` are renamed to `findIn` and narrowed from `Object` to the new `SubscriptionModelCapability`.** `of` is the convention Java uses for constructing a value, and `Optional.of` in particular never returns empty, but this method searches a wrapper chain and can come back empty, so `findIn` says what it actually does. This changes behaviour `ReplayAwareSubscriptions.of` and `IntrospectableSubscriptions.of` shipped under their old type names in 0.32.0. See [ADR 118](doc/architecture/decisions/0118-a-subscription-model-capability-marker-replaces-object-in-the-of-lookups.md) and [ADR 119](doc/architecture/decisions/0119-subscription-model-capability-gains-instance-side-lookups.md).
  * Every `Subscribable`, `SubscriptionModelLifeCycle`, `SubscriptionModel` and `SubscriptionModelWrapper`, on both stacks, now extends the new marker interface `SubscriptionModelCapability` and satisfies the narrowed parameter without change, so every call this library makes keeps compiling under the new name.
  * `UpgradeToOccurrent_0_33` renames `ReplayAwareSubscriptions.of` and `IntrospectableSubscriptions.of` to `findIn` for you, and marks a call whose argument is itself typed `Object` with a review comment, since the narrowed parameter no longer accepts that and the recipe cannot narrow the argument's declared type for you. `RepositionableSubscriptions.findIn` never shipped under the old name, so it needs no recipe entry.
  * `SubscriptionModelCapability` also gains two default methods, `capability(Class<T>)` and `hasCapability(Class<?>)`, so a caller holding a capability's `Class` can ask for it directly instead of switching on the class to reach the matching static `findIn`. The three static `findIn` methods now delegate to `capability(..)` rather than each walking the wrapper chain itself. `dsl/subscription-dsl` (both stacks) adds reified `capability()` and `hasCapability()` extensions returning a nullable type instead of `Optional`.
* **A saga timer's name is a `TimerName` rather than a `String`, so this changes behaviour the saga DSL shipped in 0.32.0.** `TimerName.parse("payment")` reads a plain name out of a string and `TimerName.of("step", "awaiting-players")` builds one inside a namespace, which encodes back to `step:awaiting-players`. `SagaTimeout` becomes `record SagaTimeout(String sagaId, TimerName timerName)` with no two-string constructor, and `SagaEffect`'s `StartTimeout`, `StartTimeoutAt` and `CancelTimeout` hold a `TimerName` too. See [ADR 121](doc/architecture/decisions/0121-a-saga-timers-name-carries-its-namespace.md) and [#716](https://github.com/johanhaleby/occurrent/issues/716).
  * `startTimeout`, `startTimeoutAt`, `cancelTimeout`, `evolveOnTimeout` and `reactOnTimeout` all keep their string forms, which read the string through `TimerName.parse`, so a saga that only calls those compiles unchanged and its timers keep firing under the same names. Nothing on disk changes either, since a timer is still stored under the string it always was, and an instance with a pending timer keeps working across the upgrade.
  * `SagaInput.timeout(sagaId, timerName)` fires a timer without building a `SagaTimeout` first, and `FlowSaga.stepTimer("awaiting-players")` gives you the name of the timer a flow step arms, so a test fires that timer and asserts on its effects without writing the namespace itself. Kotlin gets every overload the Java API gained, plus a top-level `stepTimer` next to `saga { }`.
  * If you read `timerName()` into a `String`, build a `SagaTimeout` from two strings, construct `StartTimeout`, `StartTimeoutAt` or `CancelTimeout` directly with a string name, or match a timer effect against a `String` component, `UpgradeToOccurrent_0_33` wraps a string handed to one of those constructors in `TimerName.parse` and appends `encode()` to a read it can type, and marks the rest for you. See [section 7 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#7-a-saga-timers-name-is-a-timername).
* **A deprecated `join`'s reaction now reads only the events of its own step, instead of the whole retained history, so this changes behaviour that shipped in 0.31.0.** [ADR 120](doc/architecture/decisions/0120-a-step-condition-is-a-monotone-matcher-tree.md)'s 2026-08-11 amendment had exempted `join` from the narrowing that `on(StepCondition, ...)` got, so the two callbacks read different things by design. That exemption is reversed, in the first step as much as any later one. See [section 11 of the upgrade guide](doc/migration/upgrading-to-0.33.0.md#11-a-lowered-joins-reaction-now-reads-its-own-window-not-the-whole-retained-history), [ADR 125](doc/architecture/decisions/0125-a-lowered-joins-reaction-reads-its-own-window-not-the-whole-retained-history.md) and [#707](https://github.com/johanhaleby/occurrent/issues/707).
  * A `join` past the first step no longer sees an earlier step's events, matching type or not, so even a repeat of one of its own expected types drops out.
  * A `join` in the first step keeps whatever a `stepWindow` cap has left of its own events, but loses the initiating event from `count`, `all`, `first`, `any`, `none` and `asList`, since a reaction's events always start after it. `received.initiating(...)` still reaches the start event regardless.
  * A `stepWindow` cap narrows the reaction's own events the same way it already did for `on(StepCondition, ...)`, while the condition still completes on the same event either way.
  * `join`'s own deprecation javadoc said none of this could happen, that replacing it with `on(allOf(...))` changed nothing about what the callback sees, which was false and is corrected too.
...
* **A reactive subscription registered while its subscription model is stopped, and asking for the subscription model default start position, no longer misses what was written before you start it.** `ReactorDurableSubscriptionModel` reads the current position when the subscription is registered and stores it when the subscription starts, so stopping the model, registering a subscription and starting it later withholds events rather than losing them. Before this, the position was only decided when the subscription started, so a subscription running for the first time began wherever the feed had reached by then. One that has run before was never affected, since it resumes from its own stored position, and nothing is written until a subscription actually starts. The blocking stack never had this problem, because its models register with the feed even while stopped rather than skipping registration entirely. [ADR 86](doc/architecture/decisions/0086-a-manual-subscription-is-registered-not-started.md) explains the guarantee this brings the reactive stack in line with.
  * A read that fails at registration, and one that answers nothing, both refuse the subscription instead of being read again when it starts, since a read taken then answers with wherever the feed has reached and starting from that is the loss this entry is about.
  * Where the refusal comes out depends on the wrapped model, as it does for the conditional write above. It is thrown from `subscribe(..)` when that model manages named subscriptions of its own. When this model opens the feed itself it cannot throw there, so it is logged at `ERROR` and signalled on `Subscription.waitUntilStarted()`, on the handle `resumeSubscription(..)` hands back, and on the handle you got when you registered, once that registration asked for the subscription model default and storage has confirmed it holds nothing.
  * A storage that cannot be read leaves that handle waiting rather than reporting a refusal the start may not make. A dynamic start position is read for too, since it may answer the model default when the subscription starts, but its registration handle keeps waiting rather than reporting a refusal that its own answer may avoid.
  ...
  * A registration naming its own `StartAt` is not read for at all, `StartAt.now()` included, since this model records no position for it and you have said where to begin. One that already has a checkpoint stored begins from that checkpoint and starts even when the read could not answer.
...
* A saga refuses an event that has neither a stream id with a stream version nor a position, rather than reacting to it. Without one of those it cannot tell a redelivered event from a new one, so every redelivery would run the reaction again and issue its commands again. The refusal reaches the feed, so the event is not acknowledged and the listener that dropped the metadata is what you find rather than a duplicate command. Set `redeliveryDetection = BEST_EFFORT` on the `@Saga`, or `SagaRunnerConfig.withRedeliveryDetection(..)` if you drive `SagaRunner` yourself, to take those events anyway with one warning, which is what a feed carrying another application's events needs when every command the saga issues is safe to receive more than once. Occurrent's own stored events always have the metadata, so an event-store saga and the catch-up in front of a push feed never reach this. Rationale in [ADR 109](doc/architecture/decisions/0109-a-saga-refuses-an-event-it-cannot-recognise-a-redelivery-of.md).
* **The in-memory event store and the in-memory subscription model can now filter on a field inside an event's `data` payload, which previously threw and worked only on MongoDB.** That mattered most in tests, where a `Filter.data(..)` that worked in production failed locally. Add `occurrent-common-inmemory-filter-matching-jackson` and build the store with `new InMemoryEventStore().withDataFieldReader(new JacksonDataFieldReader())`, or pass the same reader to `InMemorySubscriptionModel`. A store built without one refuses a data filter with an `UnsupportedOperationException`, rather than quietly matching nothing. Resolves [#58](https://github.com/johanhaleby/occurrent/issues/58), [#582](https://github.com/johanhaleby/occurrent/issues/582) and [#587](https://github.com/johanhaleby/occurrent/issues/587). Rationale in [ADR 87](doc/architecture/decisions/0087-a-seam-for-reading-a-payload-in-memory.md).
  * The path rules are MongoDB's, measured rather than assumed, and the conformance suite now holds every store to them. A dotted path reaches into nested objects and into each element of an array of objects, a plain array field matches when any element does, and a number stays a number, so `eq("42")` does not match a stored `42`. Anything beyond dot notation is unsupported on purpose, since MongoDB cannot answer it either.
  * `DataFieldReader` also gained a `readAll(CloudEvent, Collection<String>)` default method, resolving every path in one traversal, and the default just loops over `read(..)` so no existing implementation breaks.
...
* **`Projections.materializedView` and `Projections.domainEventFeed` gain overloads taking a `RetryStrategy`, so the default push-projection read, update and save can recover a concurrent update instead of silently losing one.** This became reachable once a live push handler stopped running behind one lock ([ADR 108](doc/architecture/decisions/0108-a-live-push-handler-runs-outside-the-handover-lock.md)), since two threads handling two events for the same projection key can now both read the same state and the second save overwrites the first. The two existing overloads are unchanged, still `RetryStrategy.none()`, so nothing that compiles today changes behaviour. Resolves [#616](https://github.com/johanhaleby/occurrent/issues/616).
  * The new overloads only help when your store detects the conflict and throws, an optimistic-locking failure or a unique-key violation, since a retry that never sees an exception is a no-op.
  * `CatchupProjectionFeed`, `DomainEventFeed`, `ProjectionRunner`, `DcbProjectionRunner` and `@Projection(source = PUSH)` all take the resulting `MaterializedView` through their existing overloads, so no further API grew to reach them. [Section 14 of the upgrade guide](doc/migration/upgrading-to-0.32.0.md#14-a-live-push-handler-can-now-be-called-concurrently) covers the fix and what it does not cover.

#### Breaking changes

* **A saga now refuses an event it cannot recognise a redelivery of, so this changes behaviour that shipped.** `SagaRunner` takes any `Subscribable`, so a saga wired over a `PushSubscriptionModel` by hand has always been able to receive events with neither a stream id and a stream version nor a position. It reacted to every one of them, issuing the same commands again on every redelivery, and said so in a single warning. It now throws `SagaRedeliveryDetectionException` before the reaction runs, and the event goes unacknowledged, so the listener that dropped the metadata is what you find. Forward the Occurrent CloudEvent extensions from that listener, or keep the old behaviour with `SagaRunnerConfig.withRedeliveryDetection(BEST_EFFORT)` and `@Saga(redeliveryDetection = BEST_EFFORT)`, which is right when the feed genuinely has none of them and every command the saga issues is safe to receive more than once. [Section 15 of the upgrade guide](doc/migration/upgrading-to-0.32.0.md#15-a-saga-refuses-an-event-it-cannot-recognise-a-redelivery-of) covers both. Resolves [#583](https://github.com/johanhaleby/occurrent/issues/583). Rationale in [ADR 109](doc/architecture/decisions/0109-a-saga-refuses-an-event-it-cannot-recognise-a-redelivery-of.md).
...
* **The reactor `SubscriptionModel` now means what the blocking `SubscriptionModel` means.** It is `Subscribable` plus `SubscriptionModelLifeCycle` and declares nothing of its own, so a named, lifecycle-managed subscription model has one type and one name on both stacks. `ReactorMongoSubscriptionModel`, `ReactorDurableSubscriptionModel`, `CatchupThenPushSubscriptionModel` and the reactive push and synchronous models implement it. The interface that used to have this name, the one whose `subscribe` returns a `Flux<CloudEvent>` you subscribe to yourself, is now `FluxSubscriptionModel`. `UpgradeToOccurrent_0_32` renames it in your code, and [section 5 of the upgrade guide](doc/migration/upgrading-to-0.32.0.md#5-the-reactor-subscriptionmodel-is-now-fluxsubscriptionmodel) covers doing it by hand. Reasoning in [ADR 98](doc/architecture/decisions/0098-reactor-subscriptionmodel-means-what-blocking-subscriptionmodel-means.md).
  * The rename left the blocking `CheckpointAwareSubscriptionModel` extending the wide `SubscriptionModel` while its reactor twin extends `FluxSubscriptionModel`, which over-constrained `ManualStartSubscriptionModel.stoppedByDefault`, whose position-source parameter took the whole blocking model just to call `globalCheckpoint()`. `GlobalCheckpointSource<T>` now declares just that one method, both `CheckpointAwareSubscriptionModel` interfaces extend it, and `stoppedByDefault`'s parameter widens from `CheckpointAwareSubscriptionModel` to `GlobalCheckpointSource<@Nullable Checkpoint>`.
* **`NativeMongoLeaseCompetingConsumerStrategy` moves from `org.occurrent.subscription.mongodb.spring.blocking` to `org.occurrent.subscription.mongodb.nativedriver.blocking`.** It is built on the native Java driver and never touched Spring, but shipped under the package the three Spring competing-consumer artifacts own for real, so an application depending on all four got four identical copies of the same `package-info.class`, and the import line of a Spring-free application read as though it depended on Spring. Every other native-driver subscription type already lives under `nativedriver.blocking`, and this was the one outlier. `UpgradeToOccurrent_0_32` renames it for you, including the qualified `Builder` construction and the `withDefaults(..)` factory call, and [section 9 of the upgrade guide](doc/migration/upgrading-to-0.32.0.md#9-nativemongoleasecompetingconsumerstrategy-moves-to-the-native-driver-package) covers doing it by hand. Resolves [#534](https://github.com/johanhaleby/occurrent/issues/534).
* **The MongoDB event stores now persist the CloudEvent `time` attribute in one canonical form when `TimeRepresentation.RFC_3339_STRING` is configured**, always with seconds and always with nine fractional digits, so `2026-07-28T12:00:00.000000000Z` rather than `2026-07-28T12:00Z`. That fixes two bugs. `Filter.time(instant)` did not match an event written at exactly that instant when the timestamp was truncated to a whole minute, on the event stores and on MongoDB subscriptions alike, and range queries on `time` were unsound because a variable-length string does not sort chronologically. Nanosecond precision is unchanged. Resolves [#463](https://github.com/johanhaleby/occurrent/issues/463) and [#468](https://github.com/johanhaleby/occurrent/issues/468). Rationale in [ADR 79](doc/architecture/decisions/0079-canonical-fixed-width-time-for-rfc3339-storage.md).
  * Range ordering now holds for a collection whose events all carry the same UTC offset, so one mixing offsets still needs `TimeRepresentation.DATE` or a custom attribute.
  * Events written by an earlier version keep their old form. An `eq`, `in` or `ne` filter on `time` still matches them, since it now compares against both the canonical and the legacy value, but a range boundary landing on one of them can still miss it. Nothing has to be migrated for queries over events written from this version onward to be correct, and [upgrading to 0.32.0](doc/migration/upgrading-to-0.32.0.md) has an optional one-off rewrite if you want exactness across the upgrade.
* **Each way a subscription model can refuse a call now has its own exception type, so this changes behaviour that shipped.** `subscribe(..)` throws `DuplicateSubscriptionIdException` for an id this model instance already has, `UnsupportedSubscriptionFilterException` for a filter it cannot apply, and `UnsupportedStartAtException` for a start position it does not accept. `pauseSubscription(..)` throws `SubscriptionNotRunningException` and `resumeSubscription(..)` throws `SubscriptionAlreadyRunningException`. Each one carries what the call named, so you can read `subscriptionId()`, `filterType()` or `startAt()` off it instead of parsing a message that was never part of the contract. They all extend `IllegalArgumentException`, which is what every one of them threw before, so an existing `catch` still catches them and no call site changes. Resolves [#580](https://github.com/johanhaleby/occurrent/issues/580). Reasoning in [ADR 106](doc/architecture/decisions/0106-a-refused-subscription-call-says-which-condition-it-hit.md), and see [Upgrading to 0.32.0](doc/migration/upgrading-to-0.32.0.md).
  * **The sixth is a new answer rather than a new name for an old one.** `UnknownSubscriptionException` says this model has no subscription with that id, where pausing or resuming an id no model had ever seen used to claim it was not running or not paused. If you hold several subscription models and want the one that owns an id, that is now the difference between "try the next model" and "this is the owner and the answer is no", which is what `OccurrentSubscriptionsExtension` needed and had to approximate before.
  * The six types are sealed under `SubscriptionRefusedException`, so a `switch` over them is exhaustive, and each builds one standard message, replacing the six different spellings Occurrent's own models used between them.
  * A model built without a `DataFieldReader` and asked to filter on a payload field now throws `UnsupportedOperationException` rather than an argument exception, matching what a store already does for a capability it was not built with, because supplying a different filter is not what fixes it.
...
* **A subscription that has not started now says so from `waitUntilStarted`, and three released cases change with it.** The method promised only that it returns once the subscription has started, and seven different answers had grown underneath that. It now answers for the one start the handle was created for, and reports started once nothing further is required of you before the subscription can deliver. Resolves [#579](https://github.com/johanhaleby/occurrent/issues/579). Reasoning in [ADR 105](doc/architecture/decisions/0105-starting-a-model-twice-is-allowed-and-a-subscription-that-has-not-started-says-so.md), and see [Upgrading to 0.32.0](doc/migration/upgrading-to-0.32.0.md).
  * A subscription registered while a `PushSubscriptionModel` or a `SynchronousSubscriptionModel` was stopped answers `false` instead of `true`, because a stopped model of that kind drops what it is handed rather than holding it.
  * A catch-up replay that failed reaches you as an exception from the blocking stream and DCB catch-up models, instead of being logged and reported as `false`, so a read model that was never filled cannot pass for a working one. That handle also returns its delegate's answer rather than a fixed `true`.
  * A catch-up cancelled before it went live answers `false`, and fails rather than completing on the reactive stack.
  * A subscription that has started keeps answering `true` afterwards, even once it is paused or is waiting for another node to release a competing consumer lock, so ask `isRunning(id)` and `isPaused(id)` about the present.
* **A synchronous subscription now still handles an event when another one throws**, on both the blocking and reactive stacks. Before this the first exception ended the dispatch, so the subscriptions registered after it never received that event and never would, since a synchronous subscription has no replay. It applies when the write is not inside a transaction, which is the default for a `GenericApplicationService` you build yourself but not for the Spring starter, which wires a transaction for you. The first failure still reaches you unchanged and any others arrive in its `getSuppressed()`. Inside a transaction dispatch still stops at the first failure, because the write rolls back, so no subscription is left having acted on an event that no longer exists. The amendment at the end of [ADR 57](doc/architecture/decisions/0057-synchronous-subscriptions.md) has the reasoning, and [upgrading to 0.32.0](doc/migration/upgrading-to-0.32.0.md) has the migration.
  * **One source break, only for a dispatcher you implement yourself.** `SynchronousEventDispatcher.dispatch(List)` and `ReactiveSynchronousEventDispatcher.dispatch(List)` are replaced by `dispatch(List, boolean transactional)`. Your implementation owns the handler loop, so a flag you could ignore by accident would keep stranding handlers with nothing to tell you. `SynchronousSubscriptionModel` keeps a one-argument `dispatch` of its own, so driving the model directly is unaffected.
  * `TransactionExecutor` and `ReactiveTransactionExecutor` also gained `isTransactional()`, answered for the moment of the call so an executor whose transaction depends on its configuration can tell the truth.
...
* A Saga DSL describes an event-driven process manager as pure data and pure functions, reacting to events and to timeouts by issuing commands while holding per-instance state. Two DSLs describe the same saga, a core one over your own state and a flow one over named steps.
* A Projection DSL, and the `@Projection` annotation on top of it, build a read model from a `View` plus the function deriving which view instance an event updates.
* Snapshots let a command's replay apply only the events written after a saved state instead of the whole history, from a `Decider` or a plain `View`, across stream and DCB.
* Synchronous subscriptions run a handler on the writer thread before `execute` returns, so a projection can be updated in the write path and read back immediately.
* A generic push subscription model, and domain-event feeds beside it, let a RabbitMQ, Kafka, Spring-event or HTTP listener drive a projection instead of a MongoDB change stream.

#### Changes

* **A DCB append no longer retries a write conflict when it is running inside a transaction it did not open.** Only the code that began a transaction can begin a fresh one, so retrying inside a caller's transaction could never commit. MongoDB aborts the transaction on the first conflict and every later attempt fails on its first read with `NoSuchTransaction`, spending all 15 attempts and about 5 seconds before failing anyway. The two Spring MongoDB stores now check for an active transaction and run the append once when they find one, which is what the native driver store has always done with an ambient `ClientSession`. Both stacks change together so their retry behaviour stays identical ([ADR 53](doc/architecture/decisions/0053-dcb-api-freeze-consistency.md)). Rationale in [ADR 74](doc/architecture/decisions/0074-retry-only-where-the-transaction-is-owned.md).
  * The retry moves to the layer that owns the transaction instead. `SpringTransactionExecutor` and `SpringReactiveTransactionExecutor` retry a conflict around the transaction they open, so a setup with synchronous subscriptions, where Occurrent itself opens the transaction so the write and the handlers commit together, keeps retrying as before. If you wrap a command in your own `@Transactional`, retry at that boundary, because a participating transaction is marked rollback-only once the append throws and carrying on inside it fails at commit with `UnexpectedRollbackException`.
  * The same rule now covers every retry on the write path, so a stream `write` inside a transaction it did not open no longer retries its any-version write condition or the global position counter's cold start either. That matters most for the write condition, since joined to a caller's transaction a failed condition marks it rollback-only, so a retry could previously report a success the caller could not keep.
* **Added a reusable command-dispatch capability, extracted from the Saga DSL so a command producer other than a saga can use it.** `CommandDispatcher<C>`, the interface a command producer calls to issue a command and previously part of the saga module, now lives in the new `occurrent-command-dispatch` module together with `StreamIdResolver<C>`, a named command-to-stream-id function that is the stream-side counterpart of the DCB `TagGenerator`. The convenience factory `CommandDispatchers`, the adapter turning a `Decider`-backed application service into a `CommandDispatcher`, lives there too (package `org.occurrent.command`), taking a `StreamIdResolver` in place of a bare `Function`, so it is usable outside sagas without depending on the saga module.
  * The new `@TargetStreamId` annotation, which marks the command member whose value is the target stream id, joins the other Occurrent markers in `occurrent-annotations`. `occurrent-command-dispatch-annotation` adds `AnnotationStreamIdResolver`, a reflection-based `StreamIdResolver` that reads `@TargetStreamId` or a custom marker annotation off a command, mirroring the DCB `AnnotationTagGenerator` and `@DcbTag`. The blocking Spring Boot starter contributes a default `AnnotationStreamIdResolver` bean, overridable with your own `StreamIdResolver`.
  * This is stream-side only, since DCB routes a command to its append boundary through tags (`@DcbTag`, `TagGenerator`, `DcbDecider.criteriaFor`) rather than a stream id. The DCB counterpart `DcbCommandDispatchers.decider(...)` lives in a separate `occurrent-command-dispatch-dcb` module, kept apart so the heavier DCB dependency stack never reaches stream-only users, and it needs no `StreamIdResolver` because the `DcbDecider` it wraps already has its own `DcbCriteria` boundary and tags.
...
* **The MongoDB change stream backing a subscription can now be tuned for throughput and latency.** `NativeMongoSubscriptionModelConfig` gains `batchSize(int)` and `maxAwaitTime(Duration)`, and `SpringMongoSubscriptionModelConfig` gains `maxAwaitTime(Duration)`. A larger `batchSize` reduces server round-trips, and `maxAwaitTime` bounds how long the server holds an idle `getMore` before returning a possibly empty batch. Both are opt-in and default to unset, so the driver and server defaults still apply and no existing subscription changes behaviour on upgrade. Resolves [#173](https://github.com/johanhaleby/occurrent/issues/173). Rationale in [ADR 67](doc/architecture/decisions/0067-configurable-change-stream-batchsize-and-maxawaittime.md).
  * The options differ per model because Spring Data's abstractions do not expose the full set. The native driver's `ChangeStreamIterable` supports both, while the Spring blocking model reads the change stream through Spring's `MessageListenerContainer`, whose `ChangeStreamRequest` API has a `maxAwaitTime` but no batch size, so `batchSize` is only available on the native model. `ReactorMongoSubscriptionModel` exposes neither yet, because Spring Data's `ReactiveMongoTemplate.changeStream` and its `ChangeStreamOptions` have neither.
...
* **Added a Saga DSL, a first-class way to describe an event-driven process manager.** A saga reacts to events, and to their absence (timeouts), by issuing commands while holding per-instance state. Where a decider turns commands into events, a `Saga<E, S, C>` turns events into commands. It is pure data and pure functions with no I/O, so it unit-tests the same way a decider or a view does. Two DSLs describe the same saga and run on the same executor. The core DSL (`Saga.builder(...)` in Java, `saga(initialState) { }` in Kotlin) registers a fold and a reaction per event type against your own state. The flow DSL (`FlowSaga.builder()` in Java, `saga { }` in Kotlin) describes the process as named steps with branches, joins and per-step timeouts, keeps the state for you, and builds a `Saga<E, FlowState<E>, C>` underneath. A new `order-fulfillment` example module demonstrates both, in-memory and Docker-free. Resolves [#124](https://github.com/johanhaleby/occurrent/issues/124) and [#377](https://github.com/johanhaleby/occurrent/issues/377). Rationale in [ADR 63](doc/architecture/decisions/0063-saga-dsl.md), with the instance observation in [ADR 70](doc/architecture/decisions/0070-saga-instance-observation.md).
  * A saga reacts to two kinds of input, a domain event or one of its own timers firing, and both arrive as a `SagaInput`. It applies that input to its state with `evolve`, then `react` returns the effects to run against the state `evolve` produced, where a `SagaEffect` issues a command or starts or cancels a timer. Replay is therefore effect-free, since rehydrating an instance runs only `evolve` and never calls `react`.
  * A reaction can read the delivering event's metadata (its stream id and version, global position, and any CloudEvent extension) through optional metadata-aware `evolve`, `react` and `onStart` overloads, so a saga can correlate or decide on position without putting it in the event payload. The flow layer's `on` step and its `startsOn` expose the triggering event's metadata the same way.
  * In Kotlin a reaction returns the effects receiver rather than nothing, which is what makes a produced but discarded command a compile error instead of a saga that silently does nothing. Correct reactions are unaffected, since `issue` and the timer calls return that receiver, so only a reaction whose last statement is neither, typically one ending on an `if` without an `else`, needs the new `nothing` to close it. A branch, join, timeout or start that issues nothing takes no reaction lambda at all, in Kotlin by omitting it and in Java through new no-commands overloads on `StepBuilder`. Rationale in [ADR 75](doc/architecture/decisions/0075-make-a-dropped-saga-command-a-compile-error.md).
  * Timers take a `Duration` or an `Instant`, never a clock-reading `Deadline`, so a reaction stays deterministic in tests, and they live in the saga's own state envelope rather than an external scheduler, saved atomically with the state. An executor-side poller reads due timers from the store, so the saga DSL takes no dependency on the deadline module. In a multi-instance deployment that poller is lease-gated through a `CompetingConsumerStrategy`, wired by the `@Saga` starter by default and opted out with `occurrent.saga.competing-consumer.enabled=false`, so only one instance queries the store. It polls every 15 seconds by default, `occurrent.saga.timer-poll-interval`, to suit the minutes-to-days timescale saga timeouts run on ([ADR 64](doc/architecture/decisions/0064-lease-gate-the-saga-timer-poller.md)).
  * A blocking executor runs a saga against a subscription (`SagaRunner.agnostic(...)` and `stream(...)`), persisting per-instance state through a `SagaStateStore` (an in-memory implementation, and a Spring Mongo `SpringMongoSagaStateStore`) and dispatching each reaction's commands through a `CommandDispatcher`, usually a plain lambda over an `ApplicationService`, with `CommandDispatchers.decider(...)` as an adapter into the decider machinery. Correlation is per-event-type with a `correlateAll` fallback, validated at build time so a handled event can never lack a correlation at runtime.
  * Command dispatch is at-least-once, dispatched before the compare-and-set state save so a command is never lost, which is safe because an `ApplicationService`-backed receiver replays the authoritative stream and rejects a stale or duplicate command. One reaction's commands reach the dispatcher as a unit through `CommandDispatcher.dispatchAll(List)`, whose default dispatches them one at a time. A dispatcher whose commands all target one stream or one decider can override it and write them in a single transaction, which removes the window where a failure partway through the list leaves the earlier commands dispatched and the state unsaved. That is something a dispatcher may do rather than a guarantee Occurrent makes ([ADR 76](doc/architecture/decisions/0076-batch-command-dispatch-seam.md)).
  * A flow saga keeps at most a configurable number of received events (`FlowSaga.Builder.historyWindow(int)` in Java, `historyWindow(...)` in the Kotlin `saga { }` block, default 100), so each save re-serializes only those.
  * `@Saga` declares one on the blocking Spring stack, a factory method returning a `Saga`, resolving the state store by convention (an explicit or unique `SagaStateStore`, otherwise a `saga-<id>` MongoDB collection) and the command dispatcher from a bean, and registering it through the same catch-up and durable infrastructure `@Projection` uses.
  * Instance lifecycles are observable through `SagaInstances`, obtained from `SagaSubscription.instances()` or, on the Spring stack, by injecting the `SagaInstancesRegistry` (keyed by saga id, and able to enumerate the registered ids so a dashboard need not hardcode them) or from the per-saga `sagaInstances-<id>` bean the `@Saga` registrar publishes. Both Spring paths only hold sagas once the `@Saga` scan has run, which is after the context refreshes. It hands back `SagaInstance`, with the instance id, its `SagaStatus`, the created, updated and completed timestamps, when its earliest pending timer is due, and the step a flow saga is waiting in.
  * Enumerating instances is an optional store capability. A store that also implements `SagaStateStoreQueries` supports `findByStatus(status, updatedBefore, limit)`, which returns instances least-recently-updated first, so a progress view lists everything in a status and a stuck-instance check asks for the active instances that have been quiet for longer than a threshold. `SagaInstances.find(sagaId)` works against any store, and only enumeration needs the capability. A store answers it without reading saga state, since the Mongo store keeps the flow step in its own field, so enumerating costs the same whether an instance holds one event of history or a hundred.
  * A saga is not a substitute for a Dynamic Consistency Boundary. Where two rules must hold in one append, DCB is the right tool, and a saga is for genuinely cross-boundary, time-involving, eventually-consistent processes. `adapt` ships now, and `compose` is deferred, because composing two sagas does not combine their correlation rules, their timer names, or their notions of being finished in any obvious way.
* **Added a generic push subscription model, so the Projection DSL can be fed from an external source instead of a MongoDB change stream.** `PushSubscriptionModel` (blocking and reactor, in the new `occurrent-subscription-push-blocking` and `occurrent-subscription-push-reactor` modules) is a register-only `Subscribable` driven by an `accept(CloudEvent)` call, so a RabbitMQ, Kafka, Spring-event or HTTP listener can drive projections in production. Occurrent adds no broker dependency of its own, and the pushed events must have the Occurrent cloud-event extensions the handlers rely on, so forward the stored CloudEvent as CloudEvents JSON and reconstruct it on the listener side. Because the projection DSL already accepts any `Subscribable`, `ProjectionRunner.agnostic(pushModel, converter).project(...)` and the Kotlin `project(...)` extensions work over a push model unchanged.
  * The plain push model delivers only the live tail, so `CatchupThenPushSubscriptionModel` (blocking and reactor) adds a one-time catch-up in front of it. On first subscribe it replays a projection's history from the event store, then hands over to the live feed, buffering the feed during the replay and removing the overlap by event id, and records a one-shot catch-up-complete marker so a restart skips the replay. Because it always replays from the beginning and then hands over, it rejects a caller-supplied `startAt` rather than quietly ignoring one.
  * Resuming the live feed is the broker's job, meaning acknowledge after processing, so no live position is persisted and delivery is at-least-once over folds that are safe to run twice. Occurrent reserves positions outside the write transaction, which makes a feed-derived position unworkable, since permanent gaps are indistinguishable from late-committing holes in the delivery stream.
  * `@Projection` gains a `source` attribute. `source = Source.PUSH`, with `subscriptionModel` or `subscriptionModelName` to pick the push model bean, binds a `@Projection` to a `PushSubscriptionModel` and wraps it in the catch-up automatically, on both stacks. The default `Source.EVENT_STORE` is unchanged.
  * The `accept(CloudEvent)` capability is exposed as a `Pushable` interface that `PushSubscriptionModel` implements, so a listener can depend on the capability rather than the concrete model. Cancelling one subscription by id is now its own capability too, `CancellableSubscriptions`, which the released `SubscriptionModelLifeCycle` extends and which the register-only push and synchronous models implement. Those models have nothing to start, stop or pause, since their events arrive from the caller, but cancelling one still means something, and the catch-up model uses it to release a subscription whose replay failed instead of leaving a handler that rejects every later event and blocks the subscriptions registered behind it. Existing implementations and callers of `SubscriptionModelLifeCycle` are unaffected.
* **Added domain-event feeds for projections, so a source that already hands you domain events, a RabbitMQ or Kafka listener with its own message converter, can drive a projection with no double encode and decode on the live path.** `Projections.domainEventFeed(projection, repository)` returns the sink that applies a domain event straight into the read model with no CloudEvent conversion, a `MaterializedView<E>` on the blocking stack or a `BiFunction<EventMetadata, E, Mono<Void>>` on the reactor stack. Rationale in [ADR 62](doc/architecture/decisions/0062-pluggable-projection-event-source.md).
  * Metadata reaches the live path as well as the replay, so a projection keyed on the stream id, version or position works either way. A listener whose broker message has those values passes them alongside the event using the two-argument form of whichever sink it feeds, `update(metadata, event)` on the blocking `MaterializedView`, `apply(metadata, event)` on the reactor `BiFunction`, or `accept(metadata, event)` on a `CatchupProjectionFeed` or `DomainEventFeed`, and the one-argument form applies with none. Occurrent cannot derive them for a live domain event, because there is no CloudEvent behind it, so the source supplies them or nothing does. A projection keyed on metadata that is fed without any fails loudly instead of silently skipping every event.
  * `CatchupProjectionFeed` adds a one-time catch-up to that domain feed. The live path applies domain events directly, and only the catch-up reads the event store and decodes each replayed event once, removing the replay-to-live overlap by an id extracted from the domain event rather than from the CloudEvent id.
  * `DomainEventFeed<E>` is an application-owned fan-out sink, the domain twin of `PushSubscriptionModel`, that feeds several projections from one source, and `@Projection(source = Source.PUSH, subscriptionModelName = "...")` binds a declared `@Projection` to it on both stacks, backed by a `ViewStateRepository` or a `MaterializedView` store.
  * The duplicate-detection cache size and the live-buffer cap are configured with `CatchupThenLiveOptions`, which `CatchupThenPushSubscriptionModel` takes too, or in Spring Boot with `occurrent.subscription.catchup-then-live.dedup-cache-size` and `.max-buffered-events` for a projection fed by a `PushSubscriptionModel`. Those properties do not reach a `DomainEventFeed`, since your application declares that bean, so pass the options to its constructor instead.
* **Added first-class snapshot support, an opt-in optimization that lets a command's replay apply only the events written after a saved state instead of the whole history.** A snapshot is a discardable, schema-versioned cache of the state at a known version, so a changed state shape or a missing snapshot falls back safely to a full replay and a snapshot is never a source of truth. One found ahead of the stream's head, after a stream reset or truncation, is discarded and rebuilt rather than trusted. It works from a `Decider` (the decision state) and from a plain `View` (the read side), across stream and DCB and across both stacks. Enabling one adds a snapshot load and a tail read per snapshotted execute, paid only where snapshots are used. A new `closing-the-books` example module demonstrates both a technical every-N snapshot and a domain period close. Rationale in [ADR 61](doc/architecture/decisions/0061-first-class-snapshot-support.md) and, for the API design, [ADR 69](doc/architecture/decisions/0069-programmatic-snapshot-facade-and-per-aggregate-spec.md).
  * The pieces are a `SnapshotStore` capability that binds to no particular store (an in-memory implementation, and a Spring Mongo `SpringMongoSnapshotStore`), a `SnapshotPolicy` that unifies technical and domain triggers (`everyNEvents`, `onEvent`, `whenState`, `always`, `never`, `or`, and `whenTerminal`, the closing-the-books trigger built on `Decider.isTerminal`), and the executors `SnapshotDeciderApplicationService` and `SnapshotDcbDeciderApplicationService` with reactive twins, plus a `SnapshotViews` on-demand facade whose `readState` is a pure read and whose `refresh` explicitly persists a snapshot.
  * Each of those is a global facade, constructed once around the shared infrastructure and reused for every aggregate. What is specific to one aggregate lives in a per-aggregate spec created with `from(...)`: `SnapshotDecider.from(decider, store, options)`, `SnapshotDcbDecider.from(dcbDecider, store, options)` and `SnapshotViewSource.from(view, store)`, with the reactive twins `ReactiveSnapshotDecider`, `ReactiveSnapshotDcbDecider` and `ReactiveSnapshotViewSource`, each bundling the decider or view with its `SnapshotStore` and options. One facade therefore never binds one aggregate's store or policy, and the same policy cannot vary per command.
  * The `@Snapshot` annotation declares and maintains a snapshot through the same catch-up and durable infrastructure `@Projection` uses, on both stacks and for stream and DCB. A stream snapshot is kept per stream, a DCB snapshot per boundary keyed by a canonical form of its criteria.
  * A snapshot fold can read the delivering event's metadata by building the view with the metadata-aware `View.create(initialState, fold)` form, and it sees the real metadata whether the update applies a single new event, a range of several at once (which is what an `everyNEvents` policy does on each save), or a DCB boundary. The Kotlin `snapshotView { }` and `dcbSnapshotView { }` blocks take a metadata-aware `on` too.
  * Snapshot persistence through the DSL executors is best-effort. The snapshot is saved after the write commits, a save failure is logged rather than failing the already-committed command, and a lost snapshot only costs a fuller replay. When a snapshot must stay consistent on the write path, maintain it with `@Snapshot(mode = SYNCHRONOUS)` or a synchronous subscription, which applies it inside the write transaction (ADR 57).
  * Closing the books is a policy plus a documented pattern rather than a separate module. Model the closing balance as a real domain event that becomes the next period's opening balance, and archive prior events with the existing `deleteEventStream(String)` or `delete(Filter)` when they are no longer needed.
  * Two core additions support it, `ExecuteOptions.fromStreamVersion(...)` with `DcbExecuteOptions.fromPosition(...)`, a state-agnostic read offset so the one execute path is reused rather than forked, and `Decider.evolve(state, events)`, the apply-onto-a-base-state primitive mirroring `View.evolve`.
* **Added the `@Projection` annotation, a persistent, declarative read model built on the Projection DSL from ADR 58.** A factory method annotated with `@Projection` returning a `Projection` or `DcbProjection` is registered as a managed subscription on both stacks, subscribing through the same catch-up and durable-checkpoint infrastructure `@DcbSubscription` and `@StreamSubscription` already use, so catch-up and durable resume come from the subscription model rather than from the DSL. Rationale in [ADR 59](doc/architecture/decisions/0059-projection-annotation.md).
  * Store the result through a `ViewStateRepository`, a `MaterializedView`, or a Spring Data `CrudRepository`, selected by the store bean's type with `store = SomeStore.class` or by bean name with `storeName`, which also disambiguates when several beans share the type. The Mongo default applies on the blocking stack when both are left unset.
  * `mode` selects `ASYNC` (catch-up then live) or `SYNCHRONOUS` (read-your-writes, reusing the synchronous subscriptions from ADR 57), and is mutually exclusive with `startAt`, `startAtGlobalPosition` and `resumeBehavior`. The same catch-up-then-resume behaviour is reachable programmatically, without the Spring starter, through the new `ResumeStartPositions.replayThenResume(...)` and `replayThenResumeDcb(...)` helpers, blocking and reactor, passed to `ProjectionRunner` or `DcbProjectionRunner`.
* **Added a Projection DSL, a higher-level way to build read models.** A `Projection` couples a `View` (the pure fold) with the function deriving which view instance an event updates and the event types it handles, and a `DcbProjection` adds a DCB read boundary such as a tag filter. You describe the fold with a type-safe, per-event-type handler builder, in Java with `Projection.builder(initialState).on(OrderPlaced.class, (state, event) -> ...)` or in Kotlin with `projection(initialState) { on<OrderPlaced> { state, event -> ... } }`, and the registered handlers also determine the subscription filter, so the event types are declared once. Runners then create the subscription or query and keep the read model up to date. A new `projection-dsl` example module demonstrates it in Java and Kotlin for both stream and DCB. Resolves [#194](https://github.com/johanhaleby/occurrent/issues/194). Rationale in [ADR 58](doc/architecture/decisions/0058-higher-level-read-model-projection-dsl.md).
  * A single-instance read model omits the per-event id with `singletonProjection` or `dcbSingletonProjection` in Kotlin, or `singletonBuilder(...)` in Java, and the runtime keys its one slot by the projection's own identity.
  * A handler and the id function can also read the event's metadata (stream id and version, global position, CloudEvent extensions) through optional metadata-aware `on(...)` and `id(...)` overloads, so a projection can key a view instance by stream id or global position. The event-only forms are unchanged. The reactive runner's caller-supplied update function can take the metadata as well, on both the plain and the DCB stack, and the Spring MongoDB `project(...)` helpers resolve a metadata-keyed id through the same path every other runner uses.
  * The same model works across stream and DCB and across both stacks, in three delivery modes. An asynchronous subscription updates a stored read model eventually, a synchronous subscription updates it on the write path for read-your-writes, and an on-demand variant runs a query when you ask. It builds on the existing view DSL and is the read-side counterpart to the `DcbDecider` write-side model.
* The view DSL can now handle event metadata. `View` gained a metadata-aware `evolve(state, metadata, event)` and a `View.create(initialState, fold)` taking a `(state, EventMetadata, event)` fold, and `MaterializedView` gained an `update(metadata, event)` and a `create(...)` overload that derives the view-instance id from metadata (for example keying by stream id), and the Spring MongoDB `materialized(...)` helper gained the same metadata-aware id function so a Mongo-backed view can be keyed that way too. That helper now also fails with a clear message when the document's `@Id` differs from the id derived for the event, which previously produced a view that read one document, wrote another, and so never accumulated. The event-only methods are unchanged and delegate with empty metadata, and the on-demand query and replay path, which has no originating CloudEvent, applies events with empty metadata (its typed stream accessors then have nothing to read). This is the primitive the metadata-aware projection and saga DSLs build on, reusing the existing `EventMetadata` so the fold reads the same stream id, version, position, and CloudEvent extensions a subscriber already sees. Rationale in [ADR 68](doc/architecture/decisions/0068-first-grade-event-metadata-in-the-dsls.md).
* **Added synchronous subscriptions, so a projection can be updated in the write path.** A subscription can be declared synchronous with the new `@SynchronousSubscription` annotation, or by registering a handler on a `SynchronousSubscriptionModel` through the `Subscriptions` DSL with no Spring required, and the application service invokes it on the writer thread before `execute` returns. This is a distinct, decoupled mechanism from the existing asynchronous subscriptions, which are unchanged. The handler receives the just-written events with their stream version and global position filled in. Available on the blocking and reactive, stream and DCB application services. Rationale in [ADR 57](doc/architecture/decisions/0057-synchronous-subscriptions.md).
  * Transactions are opt-in and best-effort by default. Configure a `TransactionExecutor` on the application service (`builder(...).transactionExecutor(...)`) to make the write and the handlers commit atomically, with a Spring-backed executor (`TransactionTemplate` or `TransactionalOperator`) or a native MongoDB `ClientSession`-backed one, and a handler's own `@Transactional` composes with it. Without a transaction executor a synchronous handler still runs before `execute` returns, but the write has already committed, so a throwing handler does not roll it back.
  * Enabling synchronous subscriptions adds one read per event-producing write, to recover the global position, paid only while at least one synchronous subscription is registered.
...
* **The `ResumeBehavior` and `StartupMode` enums move out of `@Subscription`, `@StreamSubscription`, `@DcbSubscription` and `@Projection`, where each declared its own identical copy, and become the shared top-level types `org.occurrent.annotation.ResumeBehavior` and `org.occurrent.annotation.StartupMode`.** This is a breaking change for a 0.30.0 caller referencing a nested enum, for example `Subscription.ResumeBehavior` or `DcbSubscription.StartupMode`, and the `org.occurrent.UpgradeToOccurrent_0_31` OpenRewrite recipe rewrites those references for you. See the [upgrade guide](doc/migration/upgrading-to-0.31.0.md), the Breaking changes section below, and the rationale in [ADR 60](doc/architecture/decisions/0060-unify-resumebehavior-and-startupmode-enums.md).
  * `StartPosition` is hoisted the same way and becomes `org.occurrent.annotation.StartPosition`, shared by `@Subscription`, `@DcbSubscription`, `@Projection` and `@Snapshot`, and `@DcbSubscription`'s `DcbStartPosition` is renamed to it, with the constants unchanged. `@StreamSubscription` keeps its own `StartPosition` because its `BEGINNING_OF_TIME` constant genuinely differs.
  * The `Capability` and `Mode` enums shared between `@Projection` and `@Snapshot` are hoisted to `org.occurrent.annotation.Capability` and `org.occurrent.annotation.Mode` as well. Those two only exist on the unreleased annotations, so they are not a breaking change.
...
* **The Spring Boot annotation machinery moves out of the MongoDB starters into store-neutral modules, so a second event store can reuse it instead of copying it.** `@Subscription`, `@StreamSubscription`, `@DcbSubscription`, `@SynchronousSubscription`, `@Projection`, `@Snapshot` and `@Saga` are registered by the same code as before, now published as `occurrent-blocking-spring-boot-autoconfigure` and `occurrent-reactor-spring-boot-autoconfigure`, with the shared pieces in `occurrent-spring-boot-autoconfigure`, renamed from `occurrent-mongodb-spring-boot-autoconfigure`. Nothing changes for an application that uses a starter. `@EnableOccurrent` and `@EnableOccurrentReactive` stay where they are, every annotation behaves identically, and every `occurrent.*` property keeps its name. Resolves [#409](https://github.com/johanhaleby/occurrent/issues/409). See [the upgrade guide](doc/migration/upgrading-to-0.31.0.md) and the rationale in [ADR 72](doc/architecture/decisions/0072-store-neutral-spring-boot-annotation-modules.md).
  * Two things change for a 0.30.0 caller, and the `org.occurrent.UpgradeToOccurrent_0_31` OpenRewrite recipe rewrites both for you. `OccurrentProperties` and the seven other public types in `org.occurrent.springboot.mongo.common` move to `org.occurrent.springboot.common`, and the autoconfigure artifact changes coordinate.
  * The zero-config MongoDB defaults behind a store-less `@Projection`, `@Snapshot` or `@Saga` still work, supplied by the MongoDB starter through new provider interfaces rather than hardcoded, which is what lets a SQL starter offer its own.
...
* Renamed the `SubscriptionPosition` type family to `Checkpoint` to stop overloading "position" for two different
  ...
  API change, and there is no deprecated alias.
