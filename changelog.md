...
* **The blocking `CatchupProjectionFeed.accept(..)` and `DomainEventFeed.accept(..)` now return only once the event is applied, where 0.33.0 returned as soon as a catch-up replay had buffered it.** A listener acknowledges the message once `accept(..)` returns, so it acknowledged an event that was only held in memory, and if the application stopped or crashed before the replay finished, the broker never sent that event again. A rolling deploy during a long startup replay was enough. Before the feed goes live, `accept(..)` now waits until the catch-up has applied the event, and throws an `IllegalStateException` when it was not applied, because the catch-up was stopped before the feed went live, the catch-up failed, or the waiting thread was interrupted. Do not acknowledge the message when it throws, and the broker delivers it again. Resolves [#1135](https://github.com/johanhaleby/occurrent/issues/1135).
  * A long replay keeps the listener thread waiting. A Kafka consumer that waits past its `max.poll.interval.ms`, five minutes by default, is taken out of its group and the record is delivered again, which costs a redelivery rather than the event.
  * An event fed before the catch-up starts waits as well, so never call `accept(..)` on the thread that is about to call `catchUp()` or `goLive()`. Nothing else applies the buffered events, so that thread waits for good. An `accept(..)` called on the replay's own thread, from inside the view the replay is updating, throws rather than waiting.
  * `DomainEventFeed.acceptCloudEvent(..)`, which the Kafka and RabbitMQ bridges call, never buffered an event and is unchanged.
