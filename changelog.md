...
* `CommandDispatchers.decider(...)` and `DcbCommandDispatchers.decider(...)` now write a saga reaction's commands as one append per target instead of one per command. A reaction issuing three commands against the same stream, or inside the same `DcbCriteria` boundary, used to read that stream and append to it three times, and a failure on the third left the first two applied. Now the decider sees the run in one go, each command decides against what the ones before it decided, and a failure leaves nothing written. Only consecutive commands are folded, since dispatch stays in order, so two commands to one target separated by one to another stay three appends. This is the batch seam from [ADR 76](doc/architecture/decisions/0076-batch-command-dispatch-seam.md) being used by Occurrent's own dispatchers. There is nothing to change in your code. What you may notice is fewer stream version bumps, and no partial progress when one command in a reaction fails. On the DCB side a command the decider has no boundary for now fails the whole batch before anything is appended, rather than after the earlier commands were already written. Resolves [#480](https://github.com/johanhaleby/occurrent/issues/480).
