 ...
 * {@code save} or {@code writeVersion} ADR 116 added, and inserts a throwing stub for each missing member plus a
 * review comment, so the class compiles again. This is only the signature half of the 0.33.0 break. The recipe
 * cannot know whether a store can evaluate a write condition for real, so every stub throws
 * {@link UnsupportedOperationException} (a {@link reactor.core.publisher.Mono#error(Throwable)} carrying one on
 * the reactor stack) and is left marked for a manual pass, the same best-effort-plus-marker shape as
 * {@link MigrateEventStoreWriteStreamToList}. Java only, rewrite-kotlin has no recipe for inserting a member into
 * a class body, so a Kotlin implementer still needs the manual steps in doc/migration/upgrading-to-0.33.0.md.
 */
public class AddCheckpointStorageConditionalWriteStubs extends Recipe {
    ...
    private static final String BLOCKING_SAVE_STUB = """
            /* TODO [%s]: this always refuses a conditional write. Evaluate `condition` for real, or keep refusing every condition but `any()` if this storage cannot evaluate one. See doc/migration/upgrading-to-0.33.0.md. */
            @Override
    ...
    private static final String BLOCKING_WRITE_VERSION_STUB = """
            /* TODO [%s]: this never reports a stored version. Return the version a condition is judged against, or OptionalLong.empty() if this storage cannot evaluate one. See doc/migration/upgrading-to-0.33.0.md. */
            @Override
    ...
    private static final String REACTOR_WRITE_VERSION_STUB = """
            /* TODO [%s]: this never reports a stored version. Signal the version a condition is judged against, or an empty Mono if this storage cannot evaluate one. See doc/migration/upgrading-to-0.33.0.md. */
            @Override
    ...
    // directly. These shapes only have to match the real types closely enough for a signature to bind. Nothing in
    // a stub body calls a member of Checkpoint or CheckpointWriteCondition beyond Object's toString.
    ...
    public String getDescription() {
        return "Finds a class implementing the blocking or reactor `CheckpointStorage` that is missing the " +
               ...
               "inserts a throwing stub for each missing member, marked with a review comment, so the class " +
               "compiles again. Filling in real behaviour is a manual pass, see " +
